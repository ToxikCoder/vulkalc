...
VKAPI_CALL debug_report_CreateDebugReportCallbackEXT(
        VkInstance
instance,
...
// 3rd, allocate memory for a unique handle for each callback:
    ...
    {
#endif
        pCallbacks = *callbacks = ((VkDebugReportCallbackEXT * )
        malloc(
    ...
    while(pNext)
    {
           ...
           VK_STRUCTURE_TYPE_DEBUG_REPORT_CREATE_INFO_EXT)
        {
            ...
            *pCallbacks++ = (VkDebugReportCallbackEXT)(uintptr_t)
            pInfos++;
        }
...
static VKAPI_ATTR void VKAPI_CALL
debug_report_DebugReportMessageEXT(
        VkInstance
instance,
...
memset(pNewDbgFuncNode,
0, sizeof(VkLayerDbgFunctionNode));

pNewDbgFuncNode->
pfnMsgCallback = pCreateInfo->pfnCallback;
pNewDbgFuncNode->
msgFlags = pCreateInfo->flags;
pNewDbgFuncNode->
pUserData = pCreateInfo->pUserData;
pNewDbgFuncNode->
pNext = inst->DbgFunctionHead;
inst->
DbgFunctionHead = pNewDbgFuncNode;
...
DestroyDebugReportCallbackEXT(
        icd_term
->instance, icd_info[storage_idx], pAllocator);
}
storage_idx++;
}
...
terminator_DestroyDebugReportCallback(
        VkInstance
instance,
VkDebugReportCallbackEXT callback,
const VkAllocationCallbacks* pAllocator
...
icd_term->
DebugReportMessageEXT(icd_term
->instance, flags, objType,
