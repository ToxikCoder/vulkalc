...
VKAPI_ATTR VkResult
VKAPI_CALL vkSetInstanceDispatch(VkInstance
instance,
void* object
) {
...
loader_set_dispatch(object, inst
->disp);
return
VK_SUCCESS;
}
...
VKAPI_ATTR VkResult
VKAPI_CALL vkSetDeviceDispatch(VkDevice
device,
void* object
) {
...
static VKAPI_ATTR PFN_vkVoidFunction
VKAPI_CALL
        loader_gpdpa_instance_internal(VkInstance
inst,
const char* pName
) {
// inst is not wrapped
...
if (
loader_phys_dev_ext_gpa(loader_get_instance(inst), pName,
true, NULL, &addr))
return
addr;
...
static VKAPI_ATTR PFN_vkVoidFunction
VKAPI_CALL
        loader_gpdpa_instance_terminator(VkInstance
inst,
const char* pName
) {
// inst is not wrapped
...
static VKAPI_ATTR PFN_vkVoidFunction
VKAPI_CALL
        loader_gpa_instance_internal(VkInstance
inst,
const char* pName
) {
if (!
...
VKAPI_CALL
        loader_gpa_device_internal(VkDevice
device,
const char* pName
) {
struct loader_device* dev;
...
return (PFN_vkVoidFunction)
terminator_vkCreateSharedSwapchainsKHR;
} else if (!
strcmp(pName,
    ...
    for(uint32_t layer = 0; layer < inst->activated_layer_list.count; ++layer)
    {
        ...
        if(layer_prop_list[layer].interface_version > 1)
        {
            ...
            if(NULL != functions->get_physical_device_proc_addr &&
               NULL !=
               functions->get_physical_device_proc_addr((VkInstance)
                inst,
                        funcName)) {
                return true;
            }
...
VKAPI_CALL terminator_CreateInstance(
        const VkInstanceCreateInfo* pCreateInfo,
...
VKAPI_ATTR VkResult
VKAPI_CALL terminator_CreateDevice(
        VkPhysicalDevice
physicalDevice,
const VkDeviceCreateInfo* pCreateInfo,
...
loader_log(icd_term
->this_instance, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
...
loader_log(icd_term
->this_instance, VK_DEBUG_REPORT_WARNING_BIT_EXT,
...
loader_add_logical_device(icd_term
->this_instance, icd_term, dev);

/* Init dispatch pointer in new device object */
loader_init_dispatch(* pDevice,
&dev->loader_dispatch);

out:
if (NULL != icd_exts.list) {
loader_destroy_generic_list(icd_term
->this_instance,
(struct loader_generic_list *)&icd_exts);
}
...
VkResult setupLoaderTrampPhysDevs(VkInstance
instance) {
VkResult res = VK_SUCCESS;
...
memset(local_phys_devs,
0, sizeof(VkPhysicalDevice) * total_count);
...
loader_set_dispatch((
void *)new_phys_devs[new_idx], inst->disp);
new_phys_devs[new_idx]->
this_instance = inst;
new_phys_devs[new_idx]->
phys_dev = local_phys_devs[new_idx];
}
}
...
inst->
phys_dev_count_tramp = total_count;
inst->
phys_devs_tramp = new_phys_devs;
}
...
VkResult setupLoaderTermPhysDevs(struct loader_instance* inst)
{
    ...
    while(NULL != icd_term)
    {
        ...
        icd_phys_dev_array[i].phys_devs =
                (VkPhysicalDevice * )
        loader_stack_alloc(icd_phys_dev_array[i].count *
...
VKAPI_ATTR VkResult
VKAPI_CALL terminator_EnumeratePhysicalDevices(
        VkInstance
instance,
...
icd_term->
GetPhysicalDeviceProperties(phys_dev_term
->phys_dev,
pProperties);
}
}
...
GetPhysicalDeviceQueueFamilyProperties(
        phys_dev_term
->phys_dev, pQueueFamilyPropertyCount, pProperties);
}
}
...
icd_term->
GetPhysicalDeviceMemoryProperties(phys_dev_term
->phys_dev,
pProperties);
}
}
...
icd_term->
GetPhysicalDeviceFeatures(phys_dev_term
->phys_dev, pFeatures);
}
}
...
icd_term->
GetPhysicalDeviceFormatProperties(phys_dev_term
->phys_dev,
format, pFormatInfo);
}
}
...
GetPhysicalDeviceSparseImageFormatProperties(
        phys_dev_term
->phys_dev, format, type, samples, usage, tiling,
pNumProperties, pProperties);
}
}

VKAPI_ATTR VkResult
VKAPI_CALL terminator_EnumerateDeviceExtensionProperties(
        VkPhysicalDevice
physicalDevice,
...
if (!
loader_init_layer_list(icd_term
->this_instance,
&implicit_layer_list)) {
res = VK_ERROR_OUT_OF_HOST_MEMORY;
goto
...
loader_add_layer_implicit(
        icd_term
->this_instance, VK_LAYER_TYPE_INSTANCE_IMPLICIT,
...
for (
uint32_t i = 0;
i < implicit_layer_list.
count;
i++) {
for (
uint32_t j = 0;
j < implicit_layer_list.list[i].device_extension_list.
count;
j++) {
res = loader_add_to_ext_list(icd_term->this_instance, &all_exts,
                             1,
...
i++) {
*pPropertyCount +=
implicit_layer_list.list[i].device_extension_list.
count;
}
res = VK_SUCCESS;
}
...
VKAPI_ATTR VkResult
VKAPI_CALL terminator_EnumerateDeviceLayerProperties(
        VkPhysicalDevice
physicalDevice,
