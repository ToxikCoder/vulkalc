 ...
 */
LOADER_EXPORT VKAPI_ATTR PFN_vkVoidFunction
VKAPI_CALL
...
return
trampolineGetProcAddr(ptr_instance, pName
);
}
...
LOADER_EXPORT VKAPI_ATTR VkResult
...
VKAPI_CALL vkEnumerateInstanceLayerProperties(
...
if (pAllocator) {
ptr_instance->
alloc_callbacks = *pAllocator;
}

if (ptr_instance->num_tmp_callbacks > 0) {
// Setup the temporary callback(s) here to catch cleanup issues:
if (!
util_CreateDebugReportCallbacks(ptr_instance, pAllocator,
        ptr_instance
->num_tmp_callbacks,
ptr_instance->tmp_dbg_create_infos,
ptr_instance->tmp_callbacks)) {
...
loader_log(inst, VK_DEBUG_REPORT_INFORMATION_BIT_EXT,
0,
...
disp->
GetPhysicalDeviceFeatures(unwrapped_phys_dev, pFeatures
);
}
...
disp = loader_get_instance_layer_dispatch(physicalDevice);
disp->
GetPhysicalDeviceFormatProperties(unwrapped_pd, format, pFormatInfo
);
}
...
disp = loader_get_instance_layer_dispatch(physicalDevice);
disp->
GetPhysicalDeviceProperties(unwrapped_phys_dev, pProperties
);
}
...
disp = loader_get_instance_layer_dispatch(physicalDevice);
disp->
GetPhysicalDeviceMemoryProperties(unwrapped_phys_dev,
        pMemoryProperties
);
}
...
capacity = inst->activated_layer_list.capacity;
dev->activated_layer_list.
count = inst->activated_layer_list.count;
dev->activated_layer_list.
list =
...
memcpy(dev
->activated_layer_list.list, inst->activated_layer_list.list,
...
memset(&local_ext_list, 0, sizeof(local_ext_list));
if (
vk_string_validate(MaxLoaderStringLength, pLayerName
) ==
VK_STRING_ERROR_NONE) {
if (
strcmp(pLayerName, std_validation_str
) == 0) {
struct loader_layer_list local_list;
memset(&local_list, 0, sizeof(local_list));
for (
uint32_t i = 0;
i < sizeof(std_validation_names) /
sizeof(std_validation_names[0]);
i++) {
...
count = count;
enabled_layers->
capacity = enabled_layers->count *
           sizeof(struct loader_layer_properties);
enabled_layers->
list = loader_instance_heap_alloc(inst, enabled_layers->capacity,
                                  VK_SYSTEM_ALLOCATION_SCOPE_INSTANCE);
...
for (
uint32_t i = 0;
i < inst->activated_layer_list.
count;
j++) {

if (
loader_find_layer_name_array(
        inst
->activated_layer_list.list[i].info.layerName,
...
device,
uint32_t queueNodeIndex, uint32_t
queueIndex,
VkQueue* pQueue
) {
const VkLayerDispatchTable* disp;
...
LOADER_EXPORT VKAPI_ATTR VkResult
VKAPI_CALL vkQueueWaitIdle(VkQueue
queue) {
const VkLayerDispatchTable* disp;
...
LOADER_EXPORT VKAPI_ATTR VkResult
VKAPI_CALL vkDeviceWaitIdle(VkDevice
device) {
const VkLayerDispatchTable* disp;
...
device,
VkDeviceMemory mem, VkDeviceSize
offset,
VkDeviceSize size, VkFlags
flags,
void** ppData
) {
const VkLayerDispatchTable* disp;
...
return disp->
BindBufferMemory(device, buffer, mem, offset
);
}
...
return disp->
BindImageMemory(device, image, mem, offset
);
}
...
return disp->
CreateShaderModule(device, pCreateInfo, pAllocator, pShader
);
}
...
return disp->
FreeDescriptorSets(device, descriptorPool, descriptorSetCount,
        pDescriptorSets
);
}
...
disp->
CmdSetViewport(commandBuffer, firstViewport, viewportCount,
        pViewports
);
}
...
disp->
CmdDraw(commandBuffer, vertexCount, instanceCount, firstVertex,
        firstInstance
);
}
...
disp->
CmdDrawIndexedIndirect(commandBuffer, buffer, offset, drawCount,
        stride
);
}
...
disp->
CmdCopyBuffer(commandBuffer, srcBuffer, dstBuffer, regionCount,
        pRegions
);
}
...
disp->
CmdExecuteCommands(commandBuffer, commandBuffersCount,
        pCommandBuffers
);
}
