...
// Destroy memRef lists and free all memory
    static void DestroyQueueDataStructures(VkDevice device)
    {
        ...
        while(queue != device_data->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_QUEUE_EXT].end())
        {
            ...
            log_msg(device_data->report_data, VK_DEBUG_REPORT_INFORMATION_BIT_EXT, queue->second->object_type,
                    queue->second->handle,
...
// Check Queue type flags for selected queue operations
    static void ValidateQueueFlags(VkQueue queue, const char* function)
    {
        ...
        if(queue_item != device_data->queue_info_map.end())
        {
            ...
            if(pQueueInfo != NULL)
            {
                layer_data* instance_data = get_my_data_ptr(get_dispatch_key(device_data->physical_device),
                                                            layer_data_map);
                if((instance_data->queue_family_properties[pQueueInfo->queue_node_index].queueFlags &
    ...
    static void
    AllocateCommandBuffer(VkDevice device, const VkCommandPool command_pool, const VkCommandBuffer command_buffer,
                          VkDebugReportObjectTypeEXT object_type, VkCommandBufferLevel level)
    {
        ...
        log_msg(device_data->report_data, VK_DEBUG_REPORT_INFORMATION_BIT_EXT, object_type,
                ...
                __LINE__, OBJTRACK_NONE, LayerName, "OBJ[0x%" PRIxLEAST64 "] : CREATE %s object 0x%" PRIxLEAST64,
                object_track_index++,
    ...
    static bool ValidateCommandBuffer(VkDevice device, VkCommandPool command_pool, VkCommandBuffer command_buffer)
    {
           ...
           device_data->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT].end())
        {
            ...
            {
                skip_call |= log_msg(device_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, pNode->object_type,
                                     object_handle,
        ...
        else
        {
            skip_call |=
                    log_msg(device_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                            ...
                            object_handle, __LINE__, VALIDATION_ERROR_00097, LayerName,
                            "Invalid %s Object 0x%" PRIxLEAST64 ". %s",
                                      ...
                                      VkDebugReportObjectTypeEXT object_type)
    {
        ...
        device_data
                ->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT][reinterpret_cast<uint64_t&>(descriptor_set)] =
                pNewObjNode;
        device_data->num_objects[object_type]++;
        device_data->num_total_objects++;
    }
        ...
        else
        {
            skip_call |=
                    log_msg(device_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                            ...
                            object_handle, __LINE__, VALIDATION_ERROR_00920, LayerName,
                            "Invalid %s Object 0x%" PRIxLEAST64 ". %s",
    ...
    static void CreateQueue(VkDevice device, VkQueue vkObj, VkDebugReportObjectTypeEXT object_type)
    {
        ...
        log_msg(device_data->report_data, VK_DEBUG_REPORT_INFORMATION_BIT_EXT, object_type,
                reinterpret_cast<uint64_t>(vkObj), __LINE__,
                OBJTRACK_NONE, LayerName, "OBJ[0x%" PRIxLEAST64 "] : CREATE %s object 0x%" PRIxLEAST64,
                object_track_index++,
                object_name[object_type], reinterpret_cast<uint64_t>(vkObj));

        OBJTRACK_NODE* p_obj_node = NULL;
        auto queue_item = device_data->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_QUEUE_EXT]
                .find(reinterpret_cast<uint64_t>(vkObj));
        if(queue_item == device_data->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_QUEUE_EXT].end())
        {
            p_obj_node = new OBJTRACK_NODE;
            device_data
                    ->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_QUEUE_EXT][reinterpret_cast<uint64_t>(vkObj)] = p_obj_node;
            device_data->num_objects[object_type]++;
            device_data->num_total_objects++;
        }
        else
    ...
    static void
    CreateSwapchainImageObject(VkDevice dispatchable_object, VkImage swapchain_image, VkSwapchainKHR swapchain)
    {
                              ...
                              enum UNIQUE_VALIDATION_ERROR_CODE expected_default_allocator_code)
    {
        ...
        if(object_handle != VK_NULL_HANDLE)
        {
            ...
            if(item != device_data->object_map[object_type].end())
            {
                ...
                log_msg(device_data->report_data, VK_DEBUG_REPORT_INFORMATION_BIT_EXT, pNode->object_type,
                        object_handle, __LINE__,
                        OBJTRACK_NONE, LayerName,
                        "OBJ_STAT Destroy %s obj 0x%" PRIxLEAST64 " (%" PRIu64 " total objs remain & %" PRIu64 " %s objs).",
                        object_name[pNode->object_type], reinterpret_cast<uint64_t&>(object),
                        device_data->num_total_objects,
                ...
                if(allocated_with_custom && !custom_allocator &&
                   expected_custom_allocator_code != VALIDATION_ERROR_UNDEFINED)
                {
                    ...
                    log_msg(device_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, object_type, object_handle,
                            __LINE__,
                            ...
                            object_name[object_type], object_handle,
                            validation_error_map[expected_custom_allocator_code]);
                }
                        ...
                        expected_default_allocator_code != VALIDATION_ERROR_UNDEFINED)
                {
                    log_msg(device_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, object_type, object_handle,
                            ...
                            object_name[object_type], object_handle,
                            validation_error_map[expected_default_allocator_code]);
                }
            ...
            {
                log_msg(device_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT) 0,
                        object_handle, __LINE__,
    ...
    template<typename T1, typename T2>
    static bool
    ValidateObject(T1 dispatchable_object, T2 object, VkDebugReportObjectTypeEXT object_type, bool null_allowed,
                   enum UNIQUE_VALIDATION_ERROR_CODE error_code)
    {
        ...
        if(device_data->object_map[object_type].find(object_handle) == device_data->object_map[object_type].end())
        {
            ...
            {
                const char* error_msg = (error_code == VALIDATION_ERROR_UNDEFINED) ? ""
                ...
                return log_msg(device_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, object_type, object_handle,
                               __LINE__,
                               error_code, LayerName, "Invalid %s Object 0x%" PRIxLEAST64 ". %s",
                               object_name[object_type],
    ...
    {
        layer_data* device_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
        for(auto item = device_data->object_map[object_type].begin();
            item != device_data->object_map[object_type].end();)
        {
            OBJTRACK_NODE* object_info = item->second;
            log_msg(device_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, object_info->object_type,
...
{
    if (!
    layer_enable_tmp_callbacks(instance_data
    ->report_data, instance_data->num_tmp_callbacks,
...
ValidateObject(instance, instance, VK_DEBUG_REPORT_OBJECT_TYPE_INSTANCE_EXT,
true, VALIDATION_ERROR_00021);
...
for (
auto iit = instance_data->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT].begin();
iit != instance_data->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT].
...
log_msg(instance_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, pNode->object_type, pNode->handle, __LINE__,
OBJTRACK_OBJECT_LEAK, LayerName, "OBJ ERROR : %s object 0x%" PRIxLEAST64 " has not been destroyed.",
string_VkDebugReportObjectTypeEXT(pNode
->object_type), pNode->handle);
// Semaphore:
...
instance_data->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT].
...
if (callback_setup) {
layer_disable_tmp_callbacks(instance_data
->report_data, instance_data->num_tmp_callbacks, instance_data->tmp_callbacks);
}
if (instance_data->num_tmp_callbacks > 0) {
layer_free_tmp_callbacks(instance_data
->tmp_dbg_create_infos, instance_data->tmp_callbacks);
instance_data->
num_tmp_callbacks = 0;
}

// Clean up logging callback, if any
while (instance_data->logging_callback.

size()

> 0) {
VkDebugReportCallbackEXT callback = instance_data->logging_callback.back();
layer_destroy_msg_callback(instance_data
->report_data, callback, pAllocator);
instance_data->logging_callback.

pop_back();

}

layer_debug_report_destroy_instance(instance_data
->report_data);
layer_data_map.
erase(key);

instanceExtMap.
erase(pInstanceTable);
lock.
...
std::unique_lock<std::mutex> lock(global_lock);
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
true, VALIDATION_ERROR_00052);
...
skip_call |=
ValidateObject(physicalDevice, physicalDevice, VK_DEBUG_REPORT_OBJECT_TYPE_PHYSICAL_DEVICE_EXT,
false,
...
VKAPI_CALL GetInstanceProcAddr(VkInstance instance, const char* pName);
...
VKAPI_CALL GetPhysicalDeviceProcAddr(VkInstance instance, const char* funcName);

VKAPI_ATTR VkResult

VKAPI_CALL EnumerateInstanceExtensionProperties(const char* pLayerName, uint32_t* pPropertyCount,
                                                VkExtensionProperties* pProperties);

VKAPI_ATTR VkResult

VKAPI_CALL EnumerateInstanceLayerProperties(uint32_t* pPropertyCount, VkLayerProperties* pProperties);
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00635);
if (pMemoryRanges) {
for (
uint32_t idx0 = 0;
idx0<memoryRangeCount;
++idx0) {
if (pMemoryRanges[idx0].memory) {
skip_call |=
ValidateObject(device, pMemoryRanges[idx0]
.memory, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT,
...
VkResult result =
        get_dispatch_table(ot_device_table_map, device)
                ->InvalidateMappedMemoryRanges(device, memoryRangeCount, pMemoryRanges);
return
result;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00654);
skip_call |=
ValidateObject(device, memory, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT,
false, VALIDATION_ERROR_00655);
}
if (skip_call) {
return;
}
...
std::lock_guard<std::mutex> lock(global_lock);
skip_call |=
ValidateObject(device, buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false, VALIDATION_ERROR_00799);
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00798);
skip_call |=
ValidateObject(device, memory, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT,
false, VALIDATION_ERROR_00800);
}
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00807);
skip_call |=
ValidateObject(device, image, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_00808);
skip_call |=
ValidateObject(device, memory, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT,
false, VALIDATION_ERROR_00809);
}
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
...
skip_call |=
ValidateObject(device, buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false, VALIDATION_ERROR_00784);
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00783);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00787);
skip_call |=
ValidateObject(device, image, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_00788);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_01610);
skip_call |=
ValidateObject(device, image, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_01611);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00176);
skip_call |=
ValidateObject(device, fence, VK_DEBUG_REPORT_OBJECT_TYPE_FENCE_EXT,
true, VALIDATION_ERROR_00177);
}
if (skip_call) {
return;
}
...
++idx0) {
skip_call |=
ValidateObject(device, pFences[idx0], VK_DEBUG_REPORT_OBJECT_TYPE_FENCE_EXT,
false, VALIDATION_ERROR_00187);
}
}
}
if (skip_call) {
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00202);
skip_call |=
ValidateObject(device, semaphore, VK_DEBUG_REPORT_OBJECT_TYPE_SEMAPHORE_EXT,
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00216);
skip_call |=
ValidateObject(device, event, VK_DEBUG_REPORT_OBJECT_TYPE_EVENT_EXT,
true, VALIDATION_ERROR_00217);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00220);
skip_call |=
ValidateObject(device, event, VK_DEBUG_REPORT_OBJECT_TYPE_EVENT_EXT,
false, VALIDATION_ERROR_00221);
}
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00223);
skip_call |=
ValidateObject(device, event, VK_DEBUG_REPORT_OBJECT_TYPE_EVENT_EXT,
false, VALIDATION_ERROR_00224);
}
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00227);
skip_call |=
ValidateObject(device, event, VK_DEBUG_REPORT_OBJECT_TYPE_EVENT_EXT,
false, VALIDATION_ERROR_00228);
}
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_01015);
skip_call |=
ValidateObject(device, queryPool, VK_DEBUG_REPORT_OBJECT_TYPE_QUERY_POOL_EXT,
true, VALIDATION_ERROR_01016);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_01054);
skip_call |=
ValidateObject(device, queryPool, VK_DEBUG_REPORT_OBJECT_TYPE_QUERY_POOL_EXT,
false, VALIDATION_ERROR_01055);
}
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
...
skip_call |=
ValidateObject(device, buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
true, VALIDATION_ERROR_00680);
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00679);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00683);
if (pCreateInfo) {
skip_call |=
ValidateObject(device, pCreateInfo
->buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, false, VALIDATION_ERROR_00699);
}
}
if (skip_call) {
return
...
std::lock_guard<std::mutex> lock(global_lock);
skip_call |=
ValidateObject(device, bufferView, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_VIEW_EXT,
true, VALIDATION_ERROR_00705);
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00704);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00746);
skip_call |=
ValidateObject(device, image, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
true, VALIDATION_ERROR_00747);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00750);
if (pCreateInfo) {
skip_call |=
ValidateObject(device, pCreateInfo
->image, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT, false, VALIDATION_ERROR_00763);
}
}
if (skip_call) {
return
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00779);
skip_call |=
ValidateObject(device, imageView, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_VIEW_EXT,
true, VALIDATION_ERROR_00780);
}
if (skip_call) {
return;
}
...
VkResult result =
        get_dispatch_table(ot_device_table_map, device)
                ->CreateShaderModule(device, pCreateInfo, pAllocator, pShaderModule);
{
std::lock_guard<std::mutex> lock(global_lock);
if (result == VK_SUCCESS) {
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00481);
skip_call |=
ValidateObject(device, shaderModule, VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
true, VALIDATION_ERROR_00482);
}
if (skip_call) {
return;
}
...
VkResult result =
        get_dispatch_table(ot_device_table_map, device)
                ->CreatePipelineCache(device, pCreateInfo, pAllocator, pPipelineCache);
{
std::lock_guard<std::mutex> lock(global_lock);
if (result == VK_SUCCESS) {
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00585);
skip_call |=
ValidateObject(device, pipelineCache, VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_CACHE_EXT,
true, VALIDATION_ERROR_00586);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00572);
skip_call |=
ValidateObject(device, dstCache, VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_CACHE_EXT,
false, VALIDATION_ERROR_00573);
if (pSrcCaches) {
for (
uint32_t idx0 = 0;
idx0<srcCacheCount;
++idx0) {
skip_call |=
ValidateObject(device, pSrcCaches[idx0], VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_CACHE_EXT,
false,
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00558);
skip_call |=
ValidateObject(device, pipeline, VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_EXT,
true, VALIDATION_ERROR_00559);
}
if (skip_call) {
return;
}
...
VkResult result =
        get_dispatch_table(ot_device_table_map, device)
                ->CreatePipelineLayout(device, pCreateInfo, pAllocator, pPipelineLayout);
{
std::lock_guard<std::mutex> lock(global_lock);
if (result == VK_SUCCESS) {
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00885);
skip_call |=
ValidateObject(device, pipelineLayout, VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_LAYOUT_EXT,
true, VALIDATION_ERROR_00886);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00840);
skip_call |=
ValidateObject(device, sampler, VK_DEBUG_REPORT_OBJECT_TYPE_SAMPLER_EXT,
true, VALIDATION_ERROR_00841);
}
if (skip_call) {
return;
}
...
for (
uint32_t idx1 = 0;
idx1<pCreateInfo->pBindings[idx0].
descriptorCount;
++idx1) {
skip_call |=
ValidateObject(device, pCreateInfo
->pBindings[idx0].pImmutableSamplers[idx1],
...
VkResult result =
        get_dispatch_table(ot_device_table_map, device)
                ->CreateDescriptorSetLayout(device, pCreateInfo, pAllocator, pSetLayout);
{
std::lock_guard<std::mutex> lock(global_lock);
if (result == VK_SUCCESS) {
...
std::lock_guard<std::mutex> lock(global_lock);
skip_call |=
ValidateObject(device, descriptorSetLayout, VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT_EXT,
true,
VALIDATION_ERROR_00858);
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00857);
}
if (skip_call) {
return;
}
...
VkResult result =
        get_dispatch_table(ot_device_table_map, device)
                ->CreateDescriptorPool(device, pCreateInfo, pAllocator, pDescriptorPool);
{
std::lock_guard<std::mutex> lock(global_lock);
if (result == VK_SUCCESS) {
...
false, VALIDATION_ERROR_00930);
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00929);
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
}
...
while (itr != device_data->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT].
...
DestroyObject(device, (VkDescriptorSet)((*del_itr).first), VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT,
nullptr,
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00933);
if (pDescriptorCopies) {
for (
uint32_t idx0 = 0;
idx0<descriptorCopyCount;
++idx0) {
if (pDescriptorCopies[idx0].dstSet) {
skip_call |=
ValidateObject(device, pDescriptorCopies[idx0]
.dstSet,
...
for (
uint32_t idx1 = 0;
idx1<descriptorWriteCount;
++idx1) {
if (pDescriptorWrites[idx1].dstSet) {
skip_call |=
ValidateObject(device, pDescriptorWrites[idx1]
.dstSet,
...
for (
uint32_t idx2 = 0;
idx2<pDescriptorWrites[idx1].
descriptorCount;
++idx2) {
skip_call |=
ValidateObject(device, pDescriptorWrites[idx1]
.pTexelBufferView[idx2],
...
for (
uint32_t idx3 = 0;
idx3<pDescriptorWrites[idx1].
descriptorCount;
++idx3) {
if (pDescriptorWrites[idx1].pImageInfo[idx3].imageView) {
skip_call |=
ValidateObject(device, pDescriptorWrites[idx1]
.pImageInfo[idx3].imageView,
...
for (
uint32_t idx4 = 0;
idx4<pDescriptorWrites[idx1].
descriptorCount;
++idx4) {
if (pDescriptorWrites[idx1].pBufferInfo[idx4].buffer) {
skip_call |=
ValidateObject(device, pDescriptorWrites[idx1]
.pBufferInfo[idx4].buffer,
...
skip_call |=
ValidateObject(device, pCreateInfo
->pAttachments[idx0], VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_VIEW_EXT,
...
skip_call |=
ValidateObject(device, pCreateInfo
->renderPass, VK_DEBUG_REPORT_OBJECT_TYPE_RENDER_PASS_EXT, false,
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00425);
skip_call |=
ValidateObject(device, framebuffer, VK_DEBUG_REPORT_OBJECT_TYPE_FRAMEBUFFER_EXT,
true, VALIDATION_ERROR_00426);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00396);
skip_call |=
ValidateObject(device, renderPass, VK_DEBUG_REPORT_OBJECT_TYPE_RENDER_PASS_EXT,
true, VALIDATION_ERROR_00397);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00449);
skip_call |=
ValidateObject(device, renderPass, VK_DEBUG_REPORT_OBJECT_TYPE_RENDER_PASS_EXT,
false, VALIDATION_ERROR_00450);
}
if (skip_call) {
return;
}
...
std::lock_guard<std::mutex> lock(global_lock);
skip_call |=
ValidateObject(device, commandPool, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_POOL_EXT,
false, VALIDATION_ERROR_00074);
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00073);
}
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
...
VKAPI_CALL BeginCommandBuffer(VkCommandBuffer command_buffer, const VkCommandBufferBeginInfo* begin_info)
{
    layer_data* device_data = get_my_data_ptr(get_dispatch_key(command_buffer), layer_data_map);
    bool skip_call = false;
    {
        std::lock_guard<std::mutex> lock(global_lock);
        skip_call |= ValidateObject(command_buffer, command_buffer, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
                                    false,
    ...
    VkResult result = get_dispatch_table(ot_device_table_map, command_buffer)
...
VKAPI_CALL EndCommandBuffer(VkCommandBuffer commandBuffer)
{
    bool skip_call = false;
    {
        std::lock_guard<std::mutex> lock(global_lock);
...
VKAPI_CALL ResetCommandBuffer(VkCommandBuffer commandBuffer, VkCommandBufferResetFlags flags)
{
    bool skip_call = false;
    {
        std::lock_guard<std::mutex> lock(global_lock);
...
std::lock_guard<std::mutex> lock(global_lock);
skip_call |=
ValidateObject(commandBuffer, commandBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
false,
VALIDATION_ERROR_00599);
skip_call |=
ValidateObject(commandBuffer, pipeline, VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_EXT,
false, VALIDATION_ERROR_00601);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(commandBuffer, layout, VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_LAYOUT_EXT,
false, VALIDATION_ERROR_00981);
if (pDescriptorSets) {
for (
uint32_t idx0 = 0;
idx0<descriptorSetCount;
++idx0) {
skip_call |=
ValidateObject(commandBuffer, pDescriptorSets[idx0], VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT,
...
std::lock_guard<std::mutex> lock(global_lock);
skip_call |=
ValidateObject(commandBuffer, buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false, VALIDATION_ERROR_01354);
skip_call |=
ValidateObject(commandBuffer, commandBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
false,
...
++idx0) {
skip_call |=
ValidateObject(commandBuffer, pBuffers[idx0], VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false,
...
skip_call |=
ValidateObject(commandBuffer, dstBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false, VALIDATION_ERROR_01168);
skip_call |=
ValidateObject(commandBuffer, srcBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false, VALIDATION_ERROR_01167);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(commandBuffer, dstImage, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_01189);
skip_call |=
ValidateObject(commandBuffer, srcImage, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_01187);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(commandBuffer, dstImage, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_01294);
skip_call |=
ValidateObject(commandBuffer, srcImage, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_01292);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(commandBuffer, dstBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false, VALIDATION_ERROR_01256);
skip_call |=
ValidateObject(commandBuffer, srcImage, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_01254);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(commandBuffer, image, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
...
skip_call |=
ValidateObject(commandBuffer, dstImage, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_01330);
skip_call |=
ValidateObject(commandBuffer, srcImage, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT,
false, VALIDATION_ERROR_01328);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(commandBuffer, event, VK_DEBUG_REPORT_OBJECT_TYPE_EVENT_EXT,
false, VALIDATION_ERROR_00233);
}
if (skip_call) {
return;
}
...
for (
uint32_t idx0 = 0;
idx0<bufferMemoryBarrierCount;
++idx0) {
if (pBufferMemoryBarriers[idx0].buffer) {
skip_call |=
ValidateObject(commandBuffer, pBufferMemoryBarriers[idx0]
.buffer,
...
++idx1) {
skip_call |=
ValidateObject(commandBuffer, pEvents[idx1], VK_DEBUG_REPORT_OBJECT_TYPE_EVENT_EXT,
false,
...
for (
uint32_t idx2 = 0;
idx2<imageMemoryBarrierCount;
++idx2) {
if (pImageMemoryBarriers[idx2].image) {
skip_call |=
ValidateObject(commandBuffer, pImageMemoryBarriers[idx2]
.image,
...
for (
uint32_t idx1 = 0;
idx1<imageMemoryBarrierCount;
++idx1) {
if (pImageMemoryBarriers[idx1].image) {
skip_call |=
ValidateObject(commandBuffer, pImageMemoryBarriers[idx1]
.image,
...
skip_call |=
ValidateObject(commandBuffer, queryPool, VK_DEBUG_REPORT_OBJECT_TYPE_QUERY_POOL_EXT,
false, VALIDATION_ERROR_01036);
}
if (skip_call) {
return;
}
...
skip_call |=
ValidateObject(commandBuffer, dstBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false, VALIDATION_ERROR_01070);
skip_call |=
ValidateObject(commandBuffer, queryPool, VK_DEBUG_REPORT_OBJECT_TYPE_QUERY_POOL_EXT,
false, VALIDATION_ERROR_01069);
}
if (skip_call) {
return;
}
...
if (pRenderPassBegin) {
skip_call |=
ValidateObject(commandBuffer, pRenderPassBegin
...
for (
uint32_t idx0 = 0;
idx0<commandBufferCount;
++idx0) {
skip_call |=
ValidateObject(commandBuffer, pCommandBuffers[idx0], VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
...
skip_call |=
ValidateObject(instance, instance, VK_DEBUG_REPORT_OBJECT_TYPE_INSTANCE_EXT,
false, VALIDATION_ERROR_01847);
skip_call |=
ValidateObject(instance, surface, VK_DEBUG_REPORT_OBJECT_TYPE_SURFACE_KHR_EXT,
true, VALIDATION_ERROR_01848);
}
if (skip_call) {
return;
}
...
get_dispatch_table(ot_instance_table_map, instance
)->
...
skip_call |=
ValidateObject(physicalDevice, surface, VK_DEBUG_REPORT_OBJECT_TYPE_SURFACE_KHR_EXT,
false, VALIDATION_ERROR_01891);
}
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_01918);
if (pCreateInfo) {
skip_call |=
ValidateObject(device, pCreateInfo
->oldSwapchain, VK_DEBUG_REPORT_OBJECT_TYPE_SWAPCHAIN_KHR_EXT, true,
VALIDATION_ERROR_01935);
layer_data* device_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
skip_call |=
ValidateObject(device_data
->physical_device, pCreateInfo->surface,
...
VkResult result =
        get_dispatch_table(ot_device_table_map, device)
                ->CreateSwapchainKHR(device, pCreateInfo, pAllocator, pSwapchain);
{
std::lock_guard<std::mutex> lock(global_lock);
if (result == VK_SUCCESS) {
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_01954);
skip_call |=
ValidateObject(device, fence, VK_DEBUG_REPORT_OBJECT_TYPE_FENCE_EXT,
true, VALIDATION_ERROR_01957);
skip_call |=
ValidateObject(device, semaphore, VK_DEBUG_REPORT_OBJECT_TYPE_SEMAPHORE_EXT,
true, VALIDATION_ERROR_01956);
skip_call |=
ValidateObject(device, swapchain, VK_DEBUG_REPORT_OBJECT_TYPE_SWAPCHAIN_KHR_EXT,
false, VALIDATION_ERROR_01955);
}
if (skip_call) {
return
VK_ERROR_VALIDATION_FAILED_EXT;
...
i++) {
skip_call |=
ValidateObject(device, pCreateInfos[i]
.oldSwapchain, VK_DEBUG_REPORT_OBJECT_TYPE_SWAPCHAIN_KHR_EXT,
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateDebugReportCallbackEXT(VkInstance
instance,
...
if (pLayerName && !
strcmp(pLayerName, globalLayerProps
.layerName))
...
PFN_vkCreateDevice fpCreateDevice = (PFN_vkCreateDevice) fpGetInstanceProcAddr(phy_dev_data->instance,
                                                                               "vkCreateDevice");
...
device_data->
report_data = layer_debug_report_create_device(phy_dev_data->report_data, *pDevice);
layer_init_device_dispatch_table(* pDevice, & device_data
...
i++) {
instance_data->queue_family_properties.
emplace_back(pQueueFamilyProperties[i]);
}
}
}
                          ...
                          VkInstance* pInstance)
{
    ...
    layer_copy_tmp_callbacks(pCreateInfo->pNext, &instance_data->num_tmp_callbacks,
                             &instance_data->tmp_dbg_create_infos,
                             &instance_data->tmp_callbacks);

    instance_data->report_data = debug_report_create_instance(pInstanceTable, *pInstance,
...
i++) {
CreateObject(instance, pPhysicalDevices[i], VK_DEBUG_REPORT_OBJECT_TYPE_PHYSICAL_DEVICE_EXT,
nullptr);
}
}
}
...
skip |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00621);
skip |=
ValidateObject(device, memory, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT,
true, VALIDATION_ERROR_00622);
lock.
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00630);
skip_call |=
ValidateObject(device, memory, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT,
false, VALIDATION_ERROR_00631);
lock.
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00650);
skip_call |=
ValidateObject(device, memory, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_MEMORY_EXT,
false, VALIDATION_ERROR_00651);
lock.
                           ...
                           VkFence fence)
{
    ...
    VkResult result = get_dispatch_table(ot_device_table_map, queue)
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00084);
skip_call |=
ValidateObject(device, pAllocateInfo
->commandPool, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_POOL_EXT, false,
...
commandBufferCount;
i++) {
AllocateCommandBuffer(device, pAllocateInfo
->commandPool, pCommandBuffers[i],
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00908);
skip_call |=
ValidateObject(device, pAllocateInfo
->descriptorPool, VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_POOL_EXT, false,
...
skip_call |=
ValidateObject(device, pAllocateInfo
->pSetLayouts[i], VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT_EXT,
...
descriptorSetCount;
i++) {
AllocateDescriptorSet(device, pAllocateInfo
->descriptorPool, pDescriptorSets[i],
VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT);
}
lock.
...
std::unique_lock<std::mutex> lock(global_lock);
ValidateObject(device, commandPool, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_POOL_EXT,
false, VALIDATION_ERROR_00099);
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00098);
for (
uint32_t i = 0;
i<commandBufferCount;
i++) {
skip_call |=
ValidateCommandBuffer(device, commandPool, pCommandBuffers[i]
);
}
...
i<commandBufferCount;
i++) {
DestroyObject(device, pCommandBuffers[i], VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
nullptr,
VALIDATION_ERROR_UNDEFINED, VALIDATION_ERROR_UNDEFINED);
}
...
while (itr != device_data->swapchainImageMap.
...
skip_call |=
ValidateObject(device, descriptorPool, VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_POOL_EXT,
false, VALIDATION_ERROR_00924);
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00923);
for (
uint32_t i = 0;
i<descriptorSetCount;
i++) {
skip_call |=
ValidateDescriptorSet(device, descriptorPool, pDescriptorSets[i]
);
}
...
i<descriptorSetCount;
i++) {
DestroyObject(device, pDescriptorSets[i], VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT,
nullptr,
VALIDATION_ERROR_UNDEFINED, VALIDATION_ERROR_UNDEFINED);
}
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00904);
skip_call |=
ValidateObject(device, descriptorPool, VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_POOL_EXT,
true, VALIDATION_ERROR_00905);
lock.
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00080);
skip_call |=
ValidateObject(device, commandPool, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_POOL_EXT,
true, VALIDATION_ERROR_00081);
lock.
...
auto del_itr = itr;
while (itr != device_data->object_map[VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT].
...
skip_call |=
ValidateCommandBuffer(device, commandPool,
reinterpret_cast
<VkCommandBuffer>((* del_itr)
.first));
...
skip_call |=
ValidateObject(device, device, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
false, VALIDATION_ERROR_00519);
if (pCreateInfos) {
for (
uint32_t idx0 = 0;
idx0<createInfoCount;
++idx0) {
if (pCreateInfos[idx0].basePipelineHandle) {
skip_call |=
ValidateObject(device, pCreateInfos[idx0]
.basePipelineHandle, VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_EXT,
true, VALIDATION_ERROR_00529);
}
if (pCreateInfos[idx0].layout) {
skip_call |=
ValidateObject(device, pCreateInfos[idx0]
.layout, VK_DEBUG_REPORT_OBJECT_TYPE_PIPELINE_LAYOUT_EXT,
false, VALIDATION_ERROR_00546);
}
if (pCreateInfos[idx0].pStages) {
for (
uint32_t idx1 = 0;
idx1<pCreateInfos[idx0].
stageCount;
++idx1) {
...
skip_call |=
ValidateObject(device, pCreateInfos[idx0]
.stage.module, VK_DEBUG_REPORT_OBJECT_TYPE_SHADER_MODULE_EXT,
...
VKAPI_ATTR void VKAPI_CALL
CmdDebugMarkerEndEXT(VkCommandBuffer
commandBuffer) {
bool skip_call = VK_FALSE;
std::unique_lock<std::mutex> lock(global_lock);
skip_call |=
ValidateObject(commandBuffer, commandBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
false, VALIDATION_ERROR_02022);
lock.
...
pDisplayCount;
displays++) {
CreateObject(physicalDevice, pDisplays[displays], VK_DEBUG_REPORT_OBJECT_TYPE_DISPLAY_KHR_EXT,
nullptr);
}
}
return
...
skip |=
ValidateObject(physicalDevice, display, VK_DEBUG_REPORT_OBJECT_TYPE_DISPLAY_KHR_EXT,
false,
...
skip |=
ValidateObject(physicalDevice, mode, VK_DEBUG_REPORT_OBJECT_TYPE_DISPLAY_MODE_KHR_EXT,
false,
...
VKAPI_ATTR void VKAPI_CALL
CmdDrawIndirectCountAMD(VkCommandBuffer
commandBuffer,
VkBuffer buffer, VkDeviceSize
offset,
...
skip_call |=
ValidateObject(commandBuffer, commandBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
false, VALIDATION_ERROR_01771);
skip_call |=
ValidateObject(commandBuffer, buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false, VALIDATION_ERROR_01772);
lock.
...
VKAPI_ATTR void VKAPI_CALL
CmdDrawIndexedIndirectCountAMD(VkCommandBuffer
commandBuffer,
VkBuffer buffer, VkDeviceSize
offset,
VkBuffer countBuffer, VkDeviceSize
...
skip_call |=
ValidateObject(commandBuffer, commandBuffer, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
false, VALIDATION_ERROR_01783);
skip_call |=
ValidateObject(commandBuffer, buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
false, VALIDATION_ERROR_01784);
lock.
...
if (!
skip_call&& dev_data
->dispatch_table.CmdProcessCommandsNVX) {
dev_data->dispatch_table.
...
result = dev_data->dispatch_table
...
VKAPI_ATTR VkResult
VKAPI_CALL DisplayPowerControlEXT(VkDevice
device,
VkDisplayKHR display,
...
VKAPI_ATTR VkResult
VKAPI_CALL RegisterDeviceEventEXT(VkDevice
device,
...
VKAPI_ATTR VkResult
VKAPI_CALL RegisterDisplayEventEXT(VkDevice
device,
VkDisplayKHR display,
...
VKAPI_ATTR VkResult
VKAPI_CALL GetSwapchainCounterEXT(VkDevice
device,
VkSwapchainKHR swapchain,
...
if (
get_dispatch_table(ot_instance_table_map, instance
)->GetPhysicalDeviceProcAddr == NULL) {
return NULL;
}
return
get_dispatch_table(ot_instance_table_map, instance
...
*pMsgCallback) {
return
object_tracker::CreateDebugReportCallbackEXT(instance, pCreateInfo, pAllocator, pMsgCallback
);
}
...
return
object_tracker::EnumerateInstanceLayerProperties(pCount, pProperties
);
}
...
== VK_NULL_HANDLE);
return
object_tracker::EnumerateDeviceLayerProperties(VK_NULL_HANDLE, pCount, pProperties
);
}
...
return
object_tracker::GetDeviceProcAddr(dev, funcName
);
}
...
return
object_tracker::GetInstanceProcAddr(instance, funcName
);
}
...
== VK_NULL_HANDLE);
return
object_tracker::EnumerateDeviceExtensionProperties(VK_NULL_HANDLE, pLayerName, pCount, pProperties
);
}
...
return
object_tracker::GetPhysicalDeviceProcAddr(instance, funcName
);
}
