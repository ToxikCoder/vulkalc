...
loader_platform_thread_lock_mutex(& globalLock);
if (
screenshotFramesReceived&& screenshotFrames
.
...
swapchainMap.
insert(make_pair(*pSwapchain, swapchainMapElem));
...
QueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo)
{
    ...
    DeviceMapStruct * devMap = get_dev_info((VkDevice)
    queue);
    assert(devMap);
    VkLayerDispatchTable* pDisp = devMap->device_dispatch_table;
...
VKAPI_CALL SpecifyScreenshotFrames(const char* frameList)
{
...
return NULL;
return pDisp->
GetDeviceProcAddr(dev, funcName
);
}
...
return
screenshot::EnumerateInstanceLayerProperties(pCount, pProperties
);
}
...
return
screenshot::EnumerateDeviceLayerProperties(VK_NULL_HANDLE, pCount,
        pProperties
);
}
...
assert(physicalDevice == VK_NULL_HANDLE);
return
screenshot::EnumerateDeviceExtensionProperties(
...
return
screenshot::GetDeviceProcAddr(dev, funcName
);
}
...
return
screenshot::GetInstanceProcAddr(instance, funcName
);
}
