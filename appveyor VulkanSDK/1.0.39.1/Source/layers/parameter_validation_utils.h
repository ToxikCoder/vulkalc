...
*/
    template<typename T>
    bool ValidateGreaterThan(debug_report_data* report_data, const char* api_name, const ParameterName& parameter_name,
                             T value,
                             T lower_bound)
    {
        ...
        {
            skip_call |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT) 0, 0,
                                 __LINE__, 1, LayerName,
                                 "%s: parameter %s must be greater than %d", api_name,
                                 parameter_name.get_name().c_str(), lower_bound);
        }
 ...
 */
    static bool
    validate_required_pointer(debug_report_data* report_data, const char* apiName, const ParameterName& parameterName,
                              const void* value)
    {
        bool skip_call = false;
 ...
 */
    ...
    bool validate_array(debug_report_data* report_data, const char* apiName, const ParameterName& countName,
                        const ParameterName& arrayName, T count, const void* array, bool countRequired,
                        bool arrayRequired)
    {
        bool skip_call = false;
...
*/
                        ...
                        bool countValueRequired, bool arrayRequired)
    {
        ...
        if(count == NULL)
        {
            if(countPtrRequired)
            {
                skip_call |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                     ...
                                     REQUIRED_PARAMETER, LayerName, "%s: required parameter %s specified as NULL",
                                     apiName,
                                     countName.get_name().c_str());
            }
        }
        ...
        {
            skip_call |= validate_array(report_data, apiName, countName, arrayName, (*count), array, countValueRequired,
                                        arrayRequired);
        }
 ...
 */
    ...
    bool validate_struct_type_array(debug_report_data* report_data, const char* apiName, const ParameterName& countName,
                                    const ParameterName& arrayName, const char* sTypeName, uint32_t count,
                                    const T* array,
                                    VkStructureType sType, bool countRequired, bool arrayRequired)
    {
        ...
        {
            skip_call |= validate_array(report_data, apiName, countName, arrayName, count, array, countRequired,
                                        arrayRequired);
        }
        else
        {
            ...
            for(uint32_t i = 0; i < count; ++i)
            {
                if(array[i].sType != sType)
                {
                    skip_call |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                         ...
                                         "%s: parameter %s[%d].sType must be %s", apiName,
 ...
 */
    ...
    bool validate_struct_type_array(debug_report_data* report_data, const char* apiName, const ParameterName& countName,
                                    const ParameterName& arrayName, const char* sTypeName, uint32_t* count,
                                    const T* array,
                                    VkStructureType sType, bool countPtrRequired, bool countValueRequired,
                                    bool arrayRequired)
    {
        bool skip_call = false;
        ...
        {
            skip_call |= validate_struct_type_array(report_data, apiName, countName, arrayName, sTypeName, (*count),
                                                    array, sType,
...
*/
    template<typename T>
    bool
    validate_required_handle(debug_report_data* report_data, const char* api_name, const ParameterName& parameter_name,
                             T value)
    {
        bool skip_call = false;
        ...
        if(value == VK_NULL_HANDLE)
        {
            skip_call |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                                 __LINE__,
                                 REQUIRED_PARAMETER, LayerName, "%s: required parameter %s specified as VK_NULL_HANDLE",
                                 api_name,
 ...
 */
    static bool
    validate_string_array(debug_report_data* report_data, const char* apiName, const ParameterName& countName,
            ...
            for(uint32_t i = 0; i < count; ++i)
            {
                if(array[i] == NULL)
                {
                    skip_call |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                         ...
                                         "%s: required parameter %s[%d] specified as NULL",
 ...
 */
    static bool
    validate_struct_pnext(debug_report_data* report_data, const char* api_name, const ParameterName& parameter_name,
                    ...
                    if(std::find(start, end, current->sType) == end)
                    {
                        ...
                        if(type_name == UnsupportedStructureTypeString)
                        {
                            ...
                            skip_call |= log_msg(report_data, VK_DEBUG_REPORT_WARNING_BIT_EXT,
                                                 ...
                                                 0, __LINE__, INVALID_STRUCT_PNEXT, LayerName, message.c_str(),
                                                 api_name,
                                                 parameter_name.get_name().c_str(), current->sType,
                                                 allowed_struct_names,
                                                 header_version, parameter_name.get_name().c_str());
                        }
                        else
                        {
                                ...
                                VkBool32 value)
    {
        ...
        if((value != VK_TRUE) && (value != VK_FALSE))
        {
            skip_call |= log_msg(report_data, VK_DEBUG_REPORT_WARNING_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT,
                                 0, __LINE__,
                                 UNRECOGNIZED_VALUE, LayerName, "%s: value of %s (%d) is neither VK_TRUE nor VK_FALSE",
                                 apiName,
...
*/
    template<typename T>
    static bool
    validate_ranged_enum_array(debug_report_data* report_data, const char* apiName, const ParameterName& countName,
...
*/
    static bool
    validate_reserved_flags(debug_report_data* report_data, const char* api_name, const ParameterName& parameter_name,
                            VkFlags value)
    {
        bool skip_call = false;
        ...
        if(value != 0)
        {
            skip_call |=
                    log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                            __LINE__,
                            RESERVED_PARAMETER, LayerName, "%s: parameter %s must be 0", api_name,
                            parameter_name.get_name().c_str());
        }
        ...
        else if((value & (~all_flags)) != 0)
        {
            skip_call |=
                    log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                            __LINE__,
                            UNRECOGNIZED_VALUE, LayerName,
                            "%s: value of %s contains flag bits that are not recognized members of %s",
...
*/
    static bool
    validate_flags_array(debug_report_data* report_data, const char* api_name, const ParameterName& count_name,
            ...
            for(uint32_t i = 0; i < count; ++i)
            {
                if(array[i] == 0)
                {
                    ...
                    if(array_required)
                    {
                        skip_call |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                             VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                                             __LINE__, REQUIRED_PARAMETER, LayerName,
                                             "%s: value of %s[%d] must not be 0", api_name,
                                             array_name.get_name().c_str(), i);
                    }
                }
                else if((array[i] & (~all_flags)) != 0)
                {
                    skip_call |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                         VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                                         __LINE__, UNRECOGNIZED_VALUE, LayerName,
                                         "%s: value of %s[%d] contains flag bits that are not recognized members of %s",
                                         api_name,
...
*/
    static std::string get_result_description(VkResult result)
    {
        ...
        {
            case VK_SUCCESS:
                return "a command completed successfully";
            case VK_NOT_READY:
            ...
            case VK_EVENT_RESET:
                return "an event is unsignalled";
            case VK_INCOMPLETE:
            ...
            case VK_ERROR_LAYER_NOT_PRESENT:
                return "the specified layer does not exist";
            case VK_ERROR_EXTENSION_NOT_PRESENT:
                return "the specified extension does not exist";
            case VK_ERROR_FEATURE_NOT_PRESENT:
                return "the requested feature is not available on this device";
            case VK_ERROR_INCOMPATIBLE_DRIVER:
                return "a Vulkan driver could not be found";
            case VK_ERROR_TOO_MANY_OBJECTS:
                return "too many objects of the type have already been created";
            case VK_ERROR_FORMAT_NOT_SUPPORTED:
                return "the requested format is not supported on this device";
            case VK_ERROR_SURFACE_LOST_KHR:
                return "a surface is no longer available";
            case VK_ERROR_NATIVE_WINDOW_IN_USE_KHR:
                return "the requested window is already connected to another "
                        "VkSurfaceKHR object, or some other non-Vulkan surface object";
            case VK_SUBOPTIMAL_KHR:
                return "an image became available, and the swapchain no longer "
                        "matches the surface properties exactly, but can still be used to "
                        "present to the surface successfully.";
            case VK_ERROR_OUT_OF_DATE_KHR:
                return "a surface has changed in such a way that it is no "
            ...
            case VK_ERROR_VALIDATION_FAILED_EXT:
                return "API validation has detected an invalid use of the API";
    ...
    static void validate_result(debug_report_data* report_data, const char* apiName, VkResult result)
    {
        if(result < 0 && result != VK_ERROR_VALIDATION_FAILED_EXT)
        {
            ...
            else
            {
                ...
                log_msg(report_data, VK_DEBUG_REPORT_WARNING_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                        __LINE__,
                        FAILURE_RETURN_CODE, LayerName, "%s: returned %s, indicating that %s", apiName,
                        resultName.c_str(),
                        resultDesc.c_str());
            }
        }
