                                              ...
                                              VkDevice device)
    {
        ...
        else
        {
            ...
            log_msg(my_instance_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                    ...
                    "vkCreateDevice() called with a non-valid VkPhysicalDevice. %s",
                    validation_error_map[VALIDATION_ERROR_00031]);
        }
                                         ...
                                         VkPhysicalDevice physical_device, const char* function)
    {
        ...
        if(queue_family_index >= queue_family_count)
        {
            skip_call = log_msg(my_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                VK_DEBUG_REPORT_OBJECT_TYPE_PHYSICAL_DEVICE_EXT,
                                reinterpret_cast<uint64_t>(physical_device), __LINE__,
...
for (
auto it = pInstance->physicalDevices.begin();
it != pInstance->physicalDevices.
...
reinterpret_cast
<uint64_t>(pPhysicalDevice
->pDevice->device), __LINE__, VALIDATION_ERROR_00018,
swapchain_layer_name,
...
for (
auto it = pInstance->surfaces.begin();
it != pInstance->surfaces.
...
if (pSurface) {
log_msg(my_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_INSTANCE_EXT,
reinterpret_cast
<uint64_t>(pInstance
->instance), __LINE__, VALIDATION_ERROR_00018, swapchain_layer_name,
...
&& pQueueFamilyProperties) {
pPhysicalDevice->
gotQueueFamilyPropertyCount = true;
pPhysicalDevice->
...
pPhysicalDevice->
displayPlanePropertyCount = *pPropertyCount;
pPhysicalDevice->
gotDisplayPlanePropertyCount = true;
}
...
skip_call |=
log_msg(my_data
->report_data, VK_DEBUG_REPORT_WARNING_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_INSTANCE_EXT,
reinterpret_cast
<uint64_t>(pPhysicalDevice
->pInstance->instance), __LINE__,
...
if (pPhysicalDevice->
gotDisplayPlanePropertyCount&& planeIndex
>= pPhysicalDevice->displayPlanePropertyCount) {
skip_call |=
log_msg(my_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_INSTANCE_EXT,
reinterpret_cast
<uint64_t>(pPhysicalDevice
->pInstance->instance), __LINE__, VALIDATION_ERROR_01857,
swapchain_layer_name,
...
result = my_data->instance_dispatch_table
        ->GetDisplayPlaneSupportedDisplaysKHR(physicalDevice, planeIndex, pDisplayCount,
                                              pDisplays);
...
VKAPI_CALL CreateDisplayPlaneSurfaceKHR(VkInstance instance, const VkDisplaySurfaceCreateInfoKHR* pCreateInfo,
                                        const VkAllocationCallbacks* pAllocator, VkSurfaceKHR* pSurface)
{
    VkResult result = VK_SUCCESS;
    bool skip_call = false;
...
pSurface->pInstance->surfaces.
erase(surface);
}
if (!pSurface->swapchains.
...
reinterpret_cast
<uint64_t>(instance),
__LINE__, VALIDATION_ERROR_01844, swapchain_layer_name,
...
for (
auto it = pSurface->swapchains.begin();
it != pSurface->swapchains.
...
->device_dispatch_table, fpGetDeviceProcAddr);
...
reinterpret_cast
<uint64_t>(device),
__LINE__, VALIDATION_ERROR_00049, swapchain_layer_name,
...
for (
auto it = pDevice->swapchains.begin();
it != pDevice->swapchains.
...
reinterpret_cast
<uint64_t>(pPhysicalDevice
->physicalDevice), __LINE__, SWAPCHAIN_DID_NOT_QUERY_QUEUE_FAMILIES,
swapchain_layer_name,
...
skip_call |=
ValidateQueueFamilyIndex(my_data, queueFamilyIndex, pPhysicalDevice
->numOfQueueFamilies,
...
(VkBool32 * )
malloc(pPhysicalDevice
->numOfQueueFamilies * sizeof(VkBool32));
if (pSurface->pQueueFamilyIndexSupport != NULL) {
pSurface->
numQueueFamilyIndexSupport = pPhysicalDevice->numOfQueueFamilies;
}
}
}
if (pSurface->numQueueFamilyIndexSupport) {
...
static bool validateCreateSwapchainKHR(VkDevice
device,
const VkSwapchainCreateInfoKHR* pCreateInfo, VkSwapchainKHR
*pSwapchain) {
...
i++) {
skip_call |=
ValidateQueueFamilyIndex(my_data, pCreateInfo
->pQueueFamilyIndices[i], pPhysicalDevice->numOfQueueFamilies,
...
reinterpret_cast
<uint64_t>(device),
__LINE__, VALIDATION_ERROR_01922, swapchain_layer_name,
...
reinterpret_cast
<uint64_t>(device),
__LINE__, VALIDATION_ERROR_02338, swapchain_layer_name,
...
sharingModeStr(pCreateInfo
->imageSharingMode), validation_error_map[VALIDATION_ERROR_02338]);
}
...
reinterpret_cast
<uint64_t>(device),
__LINE__, VALIDATION_ERROR_02337, swapchain_layer_name,
"vkCreateSwapchainKHR() called with a supported pCreateInfo->sharingMode of (i.e. %s), but with a "
"bad value(s) for pCreateInfo->queueFamilyIndexCount or pCreateInfo->pQueueFamilyIndices). %s",
sharingModeStr(pCreateInfo
->imageSharingMode), validation_error_map[VALIDATION_ERROR_02337]);
}
}
...
pSwapchain->pDevice->swapchains.
erase(swapchain);
}
if (pSwapchain->pSurface) {
pSwapchain->pSurface->swapchains.
erase(swapchain);
}
...
skip_call |=
log_msg(my_data
->report_data, VK_DEBUG_REPORT_WARNING_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_DEVICE_EXT,
reinterpret_cast
<uint64_t>(device),
__LINE__, SWAPCHAIN_PRIOR_COUNT, swapchain_layer_name,
...
reinterpret_cast
<uint64_t>(device),
__LINE__, SWAPCHAIN_INVALID_COUNT, swapchain_layer_name,
...
if (pLayerName && !
strcmp(pLayerName, swapchain_layer
.layerName))
...
*pMsgCallback) {
return
swapchain::CreateDebugReportCallbackEXT(instance, pCreateInfo, pAllocator, pMsgCallback
);
}
...
return
swapchain::EnumerateInstanceLayerProperties(pCount, pProperties
);
}
...
== VK_NULL_HANDLE);
return
swapchain::EnumerateDeviceLayerProperties(VK_NULL_HANDLE, pCount, pProperties
);
}
...
== VK_NULL_HANDLE);
return
swapchain::EnumerateDeviceExtensionProperties(VK_NULL_HANDLE, pLayerName, pCount, pProperties
);
}
...
return
swapchain::GetDeviceProcAddr(dev, funcName
);
}
...
return
swapchain::GetInstanceProcAddr(instance, funcName
);
}
...
return
swapchain::GetPhysicalDeviceProcAddr(instance, funcName
);
}
