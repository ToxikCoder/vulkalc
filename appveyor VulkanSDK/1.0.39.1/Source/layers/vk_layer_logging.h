...
static inline bool debug_report_log_msg(const debug_report_data* debug_data, VkFlags msgFlags,
                                        VkDebugReportObjectTypeEXT objectType, uint64_t srcObject, size_t location,
                                        int32_t msgCode,
    ...
    while(cur_callback)
    {
        if(cur_callback->msgCallback == callback)
        {
            ...
            debug_report_log_msg(debug_data, VK_DEBUG_REPORT_DEBUG_BIT_EXT,
                                 ...
                                 reinterpret_cast<uint64_t&>(cur_callback->msgCallback), 0,
...
static inline void RemoveAllMessageCallbacks(debug_report_data* debug_data, VkLayerDbgFunctionNode** list_head)
{
    ...
    while(current_callback)
    {
        ...
        debug_report_log_msg(debug_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_DEBUG_REPORT_EXT,
                             (uint64_t) current_callback->msgCallback, 0, VK_DEBUG_REPORT_ERROR_CALLBACK_REF_EXT,
                             "DebugReport",
...
static inline debug_report_data*
layer_debug_report_create_device(debug_report_data* instance_debug_data, VkDevice device)
{
                                                 ...
                                                 VkDebugReportCallbackEXT* pCallback)
{
    ...
    debug_report_log_msg(debug_data, VK_DEBUG_REPORT_DEBUG_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_DEBUG_REPORT_EXT,
                         (uint64_t) *pCallback, 0, VK_DEBUG_REPORT_ERROR_CALLBACK_REF_EXT, "DebugReport",
                         "Added callback");
    return VK_SUCCESS;
}

static inline PFN_vkVoidFunction
debug_report_get_instance_proc_addr(debug_report_data* debug_data, const char* funcName)
{
...
static VkResult
layer_copy_tmp_callbacks(const void* pChain, uint32_t* num_callbacks, VkDebugReportCallbackCreateInfoEXT** infos,
                         VkDebugReportCallbackEXT** callbacks)
{
    ...
    VkDebugReportCallbackEXT* pCallbacks = *callbacks = ((VkDebugReportCallbackEXT*) malloc(
...
VKAPI_CALL log_callback(VkFlags msgFlags, VkDebugReportObjectTypeEXT objType, uint64_t srcObject,
...
VKAPI_CALL win32_debug_output_msg(VkFlags msgFlags, VkDebugReportObjectTypeEXT objType,
                                  ...
                                  const char* pLayerPrefix, const char* pMsg, void* pUserData)
{
    ...
    _snprintf(buf, sizeof(buf) - 1,
              "%s (%s): object: 0x%" PRIxPTR " type: %d location: "
