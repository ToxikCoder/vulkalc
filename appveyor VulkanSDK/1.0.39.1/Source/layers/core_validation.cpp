...
defined(__GNUC__)
#pragma
GCC diagnostic ignored "-Wwrite-strings"
#endif
#if
defined(__GNUC__)
#pragma
GCC diagnostic warning "-Wwrite-strings"
#endif
#include
"core_validation.h"
#include
...
<android/log.h>
#define
LOGCONSOLE(...) ((void)__android_log_print(ANDROID_LOG_INFO, "DS", __VA_ARGS__))
#else
#define
LOGCONSOLE(...)                                                                                                            \
    {
...
spirv_inst_iter &operator*() {
return *this; }
spirv_inst_iter const &operator*() const {
return *this; }
};
...
return spirv_inst_iter(words.begin(), words.begin() + 5); } // First insn
spirv_inst_iter end() const {
return spirv_inst_iter(words.begin(), words.end()); }         // Just past last insn
...
cb_node->object_bindings.insert({
reinterpret_cast<uint64_t &>(image_state->image), VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT
});
...
imgpair = {
image, false, VkImageSubresource()
};
auto imgsubIt = pCB->imageLayoutMap.find(imgpair);
if(imgsubIt == pCB->imageLayoutMap.end())
return false;
...
semaphore, pSemaphore->signaler.first, pSemaphore->signaler.second
});
...
static void PostCallRecordGetFenceStatus(layer_data *dev_data, VkFence fence) {
RetireFence(dev_data, fence); }
...
static void PostCallRecordDestroyFence(layer_data *dev_data, VkFence fence) {
dev_data->fenceMap.erase(fence); }
...
static void PostCallRecordDestroySemaphore(layer_data *dev_data, VkSemaphore sema) {
dev_data->semaphoreMap.erase(sema); }
...
static const char *memory_type[3] = {
"texel",
"uniform",
"storage"
};
...
{
reinterpret_cast<uint64_t &>(rp_state->renderPass), VK_DEBUG_REPORT_OBJECT_TYPE_RENDER_PASS_EXT
}, cb_state);
}
}
}
...
static inline void updateResourceTrackingOnDraw(GLOBAL_CB_NODE *pCB) {
...
static const VkPipelineStageFlags stage_flag_bit_array[] = {
...
struct comparer {
bool operator()(struct span i, struct span j) {
...
if(SetSparseMemBinding(dev_data, {
sparse_binding.memory, sparse_binding.memoryOffset, sparse_binding.size
},
...
if(SetSparseMemBinding(dev_data, {
sparse_binding.memory, sparse_binding.memoryOffset, size
},
...
[=](VkImage image) {
return getImageState(dev_data, image)->acquired; });
...
if(result == VK_SUCCESS) {
surface_state->gpu_queue_support[{
