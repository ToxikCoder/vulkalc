...
namespace image
{
                                 ...
                                 const VkAllocationCallbacks* pAllocator, VkDebugReportCallbackEXT* pMsgCallback)
    {
        ...
        VkResult res = my_data->instance_dispatch_table
...
layer_data* my_data = get_my_data_ptr(get_dispatch_key(instance), layer_data_map);
my_data->instance_dispatch_table->
DebugReportMessageEXT(instance, flags, objType, object, location, msgCode, pLayerPrefix,
        pMsg
);
}
...
VKAPI_CALL CreateDevice(VkPhysicalDevice physicalDevice,
...
GetPhysicalDeviceFormatProperties(device_data
->physicalDevice, pCreateInfo->format,
&properties);
if ((pCreateInfo->tiling == VK_IMAGE_TILING_LINEAR) && (properties.linearTilingFeatures == 0)) {
std::stringstream ss;
ss << "vkCreateImage format parameter (" <<
string_VkFormat(pCreateInfo
->format) << ") is an unsupported format";
skip_call |=
log_msg(device_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT,
0, __LINE__, VALIDATION_ERROR_02150, "IMAGE", "%s. %s", ss.
...
log_msg(device_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT,
0, __LINE__, VALIDATION_ERROR_02155, "IMAGE", "%s. %s", ss.
...
std::stringstream ss;
ss << "vkCreateImage: VkFormat for TILING_OPTIMAL image (" <<
string_VkFormat(pCreateInfo
->format)
...
c_str(), validation_error_map[VALIDATION_ERROR_02158]
...
std::stringstream ss;
ss << "vkCreateImage: VkFormat for TILING_LINEAR image (" <<
string_VkFormat(pCreateInfo
->format)
<< ") does not support requested Image usage type VK_IMAGE_USAGE_COLOR_ATTACHMENT";
skip_call |=
...
c_str(), validation_error_map[VALIDATION_ERROR_02153]
...
c_str(), validation_error_map[VALIDATION_ERROR_02159]

);
}
if ((pCreateInfo->tiling == VK_IMAGE_TILING_LINEAR) &&
((properties.
linearTilingFeatures& VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT
) == 0)) {
std::stringstream ss;
ss << "vkCreateImage: VkFormat for TILING_LINEAR image (" <<
string_VkFormat(pCreateInfo
...
c_str(), validation_error_map[VALIDATION_ERROR_02154]
...
->format, pCreateInfo->imageType, pCreateInfo->tiling, pCreateInfo->usage, pCreateInfo->flags,
...
if (failedMinSize) {
skip_call |=
log_msg(phy_dev_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT, 0, __LINE__,
...
if ((pCreateInfo->
samples& ImageFormatProperties
.sampleCounts) == 0) {
skip_call |=
log_msg(phy_dev_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_IMAGE_EXT, 0, __LINE__,
VALIDATION_ERROR_02138, "Image", "CreateImage samples %s is not supported by format 0x%.8X. %s",
string_VkSampleCountFlagBits(pCreateInfo
->samples), ImageFormatProperties.sampleCounts,
validation_error_map[VALIDATION_ERROR_02138]);
}

if (pCreateInfo->initialLayout !=
VK_IMAGE_LAYOUT_UNDEFINED&& pCreateInfo
...
device_data->imageMap[*pImage] =
IMAGE_STATE(pCreateInfo);
}
return
result;
}
...
device_data->imageMap.
erase(image);
lock.

unlock();

device_data->device_dispatch_table->
...
IMAGE_RENDERPASS_INVALID_ATTACHMENT, "IMAGE", "%s", ss.
...
if (image_state) {
if (
vk_format_is_depth_or_stencil(image_state
->format)) {
...
vk_format_is_compressed(image_state
->format)) {
...
if (!skipCall) {
device_data->device_dispatch_table->
CmdClearDepthStencilImage(commandBuffer, image, imageLayout, pDepthStencil, rangeCount,
        pRanges
);
}
}
...
static bool RangesIntersect(int32_t start, uint32_t start_offset, int32_t end, uint32_t end_offset)
{
    ...
    uint32_t intersection_max = std::min(static_cast<uint32_t>(start) + start_offset,
                                         static_cast<uint32_t>(end) + end_offset);
...
static bool RegionIntersects(const VkImageCopy* src, const VkImageCopy* dst, VkImageType type)
{
    ...
    if((src->srcSubresource.mipLevel == dst->dstSubresource.mipLevel) &&
       (RangesIntersect(src->srcSubresource.baseArrayLayer, src->srcSubresource.layerCount,
                        dst->dstSubresource.baseArrayLayer,
...
if (
src_image_entry&& dst_image_entry
) {
...
skip |=
log_msg(device_data
->report_data, VK_DEBUG_REPORT_WARNING_BIT_EXT,
VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT, reinterpret_cast<uint64_t &>(command_buffer),
__LINE__, IMAGE_MISMATCHED_IMAGE_ASPECT, "IMAGE", "%s", ss.
...
c_str(), validation_error_map[VALIDATION_ERROR_01198]
...
aspectMask& VK_IMAGE_ASPECT_METADATA_BIT
) ||
...
c_str(), validation_error_map[VALIDATION_ERROR_01222]
...
c_str(), validation_error_map[VALIDATION_ERROR_01199]
...
c_str(), validation_error_map[VALIDATION_ERROR_01223]
...
c_str(), validation_error_map[VALIDATION_ERROR_01224]
...
if (
ExceedsBounds(& regions[i]
.srcOffset, &regions[i].extent, src_image_entry)) {
std::stringstream ss;
...
c_str(), validation_error_map[VALIDATION_ERROR_01175]
...
if (
ExceedsBounds(& regions[i]
.dstOffset, &regions[i].extent, dst_image_entry)) {
std::stringstream ss;
...
c_str(), validation_error_map[VALIDATION_ERROR_01176]
...
j++) {
if (
RegionIntersects(& regions[i], & regions[j], src_image_entry
->imageType)) {
std::stringstream ss;
...
c_str(), validation_error_map[VALIDATION_ERROR_01177]
...
if (
vk_format_is_depth_or_stencil(src_image_entry
->format) ||
vk_format_is_depth_or_stencil(dst_image_entry
->format)) {
...
if (!skip) {
device_data->device_dispatch_table->
CmdCopyImage(commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout,
    ...
    for(uint32_t i = 0; i < regionCount; i++)
    {
        ...
        if(image_info)
        {
            ...
            if(vk_safe_modulo(pRegions[i].bufferOffset, texel_size) != 0)
            {
                ...
                "). %s",
                        function, i, pRegions[i]
                        .bufferOffset, texel_size, validation_error_map[VALIDATION_ERROR_01263]);
            }
            ...
            if(vk_safe_modulo(pRegions[i].bufferOffset, 4) != 0)
            {
                skip |= log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                ...
                                "%s(): pRegion[%d] bufferOffset 0x%" PRIxLEAST64 " must be a multiple of 4. %s",
                                function, i,
            ...
            if(aspect_mask_bits.count() != 1)
            {
                skip |=
                        log_msg(dev_data->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                ...
                                "%s: aspectMasks for imageSubresource in each region must have only a single bit set. %s",
                                function,
...
if (!
PreCallValidateCmdCopyImageToBuffer(device_data, srcImage, regionCount, pRegions,
"vkCmdCopyImageToBuffer()")) {
device_data->device_dispatch_table->
CmdCopyImageToBuffer(commandBuffer, srcImage, srcImageLayout, dstBuffer, regionCount,
        pRegions
);
}
}
...
if (!
PreCallValidateCmdCopyBufferToImage(device_data, dstImage, regionCount, pRegions,
"vkCmdCopyBufferToImage()")) {
device_data->device_dispatch_table->
CmdCopyBufferToImage(commandBuffer, srcBuffer, dstImage, dstImageLayout, regionCount,
        pRegions
);
}
}
...
log_msg(device_data
->report_data, VK_DEBUG_REPORT_WARNING_BIT_EXT,
VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT, reinterpret_cast
...
if (
vk_format_is_uint(srcFormat)
!=
vk_format_is_uint(dstFormat)
) {
std::stringstream ss;
...
<< "Source format is " <<
string_VkFormat(srcFormat)
<< " Destination format is " <<
string_VkFormat(dstFormat);
skipCall |=
log_msg(device_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
(uint64_t)commandBuffer, __LINE__, VALIDATION_ERROR_02191, "IMAGE", "%s. %s", ss.
...
if (
vk_format_is_sint(srcFormat)
!=
vk_format_is_sint(dstFormat)
) {
std::stringstream ss;
...
log_msg(device_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
(uint64_t)commandBuffer, __LINE__, VALIDATION_ERROR_02190, "IMAGE", "%s. %s", ss.
...
if (
vk_format_is_depth_or_stencil(srcFormat)
||
...
log_msg(device_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
(uint64_t)commandBuffer, __LINE__, VALIDATION_ERROR_02192, "IMAGE", "%s. %s", ss.
...
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT, (uint64_t)commandBuffer, __LINE__,
IMAGE_INVALID_IMAGE_ASPECT, "IMAGE", "%s", ss.
...
if (
vk_format_is_depth_and_stencil(srcFormat)
) {
...
vk_format_is_stencil_only(srcFormat)
) {
...
vk_format_is_depth_only(srcFormat)
) {
...
log_msg(device_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
(uint64_t)commandBuffer, __LINE__, VALIDATION_ERROR_02193, "IMAGE", "%s. %s", ss.
...
log_msg(device_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_COMMAND_BUFFER_EXT,
(uint64_t)commandBuffer, __LINE__, IMAGE_INVALID_FILTER, "IMAGE", "%s", ss.
...
if (!skipCall) {
device_data->device_dispatch_table->
CmdBlitImage(commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout,
...
++i) {
VkImageMemoryBarrier const* const barrier = (VkImageMemoryBarrier
const *const) & pImageMemoryBarriers[i];
...
skipCall |=
log_msg(device_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT)0, 0,
__LINE__, IMAGE_INVALID_IMAGE_RESOURCE, "IMAGE", "%s", ss.
...
device_data->device_dispatch_table->
CmdPipelineBarrier(commandBuffer, srcStageMask, dstStageMask, dependencyFlags,
...
if (!skipCall) {
device_data->device_dispatch_table->
CmdResolveImage(commandBuffer, srcImage, srcImageLayout, dstImage, dstImageLayout,
        regionCount, pRegions
);
}
}
...
VKAPI_ATTR VkResult
VKAPI_CALL
        EnumerateDeviceLayerProperties(VkPhysicalDevice
physicalDevice,
uint32_t* pCount, VkLayerProperties
*pProperties) {
return util_GetLayerProperties(1, &global_layer, pCount, pProperties);
}
...
*pMsgCallback) {
return
image::CreateDebugReportCallbackEXT(instance, pCreateInfo, pAllocator, pMsgCallback
);
}
...
return
image::EnumerateInstanceLayerProperties(pCount, pProperties
);
}
...
return
image::EnumerateDeviceLayerProperties(VK_NULL_HANDLE, pCount, pProperties
);
}
...
assert(physicalDevice == VK_NULL_HANDLE);
return
image::EnumerateDeviceExtensionProperties(VK_NULL_HANDLE, pLayerName, pCount, pProperties
);
}
...
return
image::GetDeviceProcAddr(dev, funcName
);
}
...
return
image::GetInstanceProcAddr(instance, funcName
);
}
...
return
image::GetPhysicalDeviceProcAddr(instance, funcName
);
}
