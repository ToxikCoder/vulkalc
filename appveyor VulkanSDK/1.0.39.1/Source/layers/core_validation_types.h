...
namespace std
{
    ...
    {
        size_t operator()(VK_OBJECT obj) const NOEXCEPT
        {
...
struct DESCRIPTOR_POOL_STATE : BASE_NODE
{
    ...
    {
        if(createInfo.poolSizeCount)
        { // Shadow type struct from ptr into local struct
...
{
    return a.mem == b.mem && a.offset == b.offset && a.size == b.size;
}
...
struct DEVICE_MEM_INFO : public BASE_NODE
{
    ...
    DEVICE_MEM_INFO(void* disp_object, const VkDeviceMemory in_mem, const VkMemoryAllocateInfo* p_alloc_info)
            : object(disp_object), global_valid(false), mem(in_mem), alloc_info(*p_alloc_info), mem_range{},
              shadow_copy_base(0),
...
public:
    ...
    // Default constructor
    PIPELINE_STATE()
            : pipeline{}, graphicsPipelineCI{}, computePipelineCI{}, active_shaders(0), duplicate_shaders(0),
              active_slots(),
        ...
        if(pCreateInfo->pVertexInputState)
        {
            ...
            if(pVICI->vertexBindingDescriptionCount)
            {
                this->vertexBindingDescriptions = std::vector<VkVertexInputBindingDescription>(
                        ...
                        pVICI->pVertexBindingDescriptions + pVICI->vertexBindingDescriptionCount);
            }
...
namespace core_validation
{
    ...
    bool
    ValidateMemoryIsBoundToBuffer(const layer_data*, const BUFFER_STATE*, const char*, UNIQUE_VALIDATION_ERROR_CODE);
