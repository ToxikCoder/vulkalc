...
namespace unique_objects
{
    ...
    {
        layer_debug_actions(instance_data->report_data, instance_data->logging_callback, pAllocator,
...
// Handle CreateDevice Extensions
    static void createDeviceRegisterExtensions(const VkDeviceCreateInfo* pCreateInfo, VkDevice device)
    {
        ...
        device_data->device_dispatch_table->CreateSwapchainKHR = (PFN_vkCreateSwapchainKHR) gpa(device,
                              ...
                              VkInstance* pInstance)
    {
        ...
        if(!layer_copy_tmp_callbacks(pCreateInfo->pNext, &instance_data->num_tmp_callbacks,
        ...
        if(instance_data->num_tmp_callbacks > 0)
        {
            layer_disable_tmp_callbacks(instance_data->report_data, instance_data->num_tmp_callbacks,
                                        instance_data->tmp_callbacks);
            layer_free_tmp_callbacks(instance_data->tmp_dbg_create_infos, instance_data->tmp_callbacks);
            instance_data->num_tmp_callbacks = 0;
        }
...
VKAPI_CALL CreateDevice(VkPhysicalDevice gpu, const VkDeviceCreateInfo* pCreateInfo,
...
VKAPI_CALL EnumerateDeviceLayerProperties(VkPhysicalDevice physicalDevice, uint32_t* pCount,
...
VKAPI_CALL EnumerateDeviceExtensionProperties(VkPhysicalDevice physicalDevice, const char* pLayerName,
                                              uint32_t* pCount, VkExtensionProperties* pProperties)
{
    ...
    layer_data* instance_data = get_my_data_ptr(key, layer_data_map);
    return instance_data->instance_dispatch_table
...
reinterpret_cast
<const VkDedicatedAllocationMemoryAllocateInfoNV*>(orig_pnext)
);
...
safe_dedicated_allocate_info->
image = reinterpret_cast<VkImage&>(device_data->unique_id_mapping[local_image]);
}
...
input_pnext->
pNext = reinterpret_cast<GenericHeader*>(safe_dedicated_allocate_info.get());
...
++idx0) {
local_pCreateInfos[idx0].
initialize(& pCreateInfos[idx0]);
if (pCreateInfos[idx0].basePipelineHandle) {
local_pCreateInfos[idx0].
basePipelineHandle =
(VkPipeline)
my_device_data
->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pCreateInfos[idx0]
.basePipelineHandle)];
}
...
layout =
(VkPipelineLayout)
my_device_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pCreateInfos[idx0]
.layout)];
}
if (pCreateInfos[idx0].stage.module) {
local_pCreateInfos[idx0].stage.
module =
(VkShaderModule)
my_device_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pCreateInfos[idx0]
...
my_device_data->unique_id_mapping[reinterpret_cast<uint64_t &>(pipelineCache)];
}

VkResult result = my_device_data->device_dispatch_table->CreateComputePipelines(
        device, pipelineCache, createInfoCount, (const VkComputePipelineCreateInfo*) local_pCreateInfos, pAllocator,
        pPipelines);
delete[]
local_pCreateInfos;
{
...
local_pCreateInfos[idx0].pStages[idx1].
module =
(VkShaderModule)
my_device_data
->unique_id_mapping[reinterpret_cast
...
VkResult result = my_device_data->device_dispatch_table->CreateGraphicsPipelines(
        device, pipelineCache, createInfoCount, (const VkGraphicsPipelineCreateInfo*) local_pCreateInfos, pAllocator,
        pPipelines);
delete[]
local_pCreateInfos;
{
...
oldSwapchain =
(VkSwapchainKHR)
my_map_data->unique_id_mapping[reinterpret_cast
...
surface =
(VkSurfaceKHR)
instance_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pCreateInfo
->surface)];
}
...
my_device_data->unique_id_mapping[reinterpret_cast<uint64_t &>(swapchain)];
}
VkResult result =
...
VKAPI_CALL GetPhysicalDeviceDisplayPropertiesKHR(VkPhysicalDevice physicalDevice, uint32_t* pPropertyCount,
            ...
            for(uint32_t idx0 = 0; idx0 < *pPropertyCount; ++idx0)
            {
                ...
                if(pProperties[idx0].display)
                {
                    local_pProperties[idx0].display =
                            (VkDisplayKHR) my_map_data
                                    ->unique_id_mapping[reinterpret_cast<const uint64_t&>(pProperties[idx0].display)];
                }
            }
        }
    }
...
VKAPI_CALL GetDisplayPlaneSupportedDisplaysKHR(VkPhysicalDevice physicalDevice, uint32_t planeIndex,
                                               uint32_t* pDisplayCount, VkDisplayKHR* pDisplays)
{
    layer_data* my_map_data = get_my_data_ptr(get_dispatch_key(physicalDevice), layer_data_map);
    VkResult result = my_map_data->instance_dispatch_table
...
VKAPI_CALL GetDisplayModePropertiesKHR(VkPhysicalDevice physicalDevice, VkDisplayKHR display,
                                       uint32_t* pPropertyCount, VkDisplayModePropertiesKHR* pProperties)
{
    ...
    if(result == VK_SUCCESS && pProperties)
    {
        for(uint32_t idx0 = 0; idx0 < *pPropertyCount; ++idx0)
        {
            ...
            uint64_t unique_id = global_unique_id++;
            my_map_data->unique_id_mapping[unique_id] = reinterpret_cast<uint64_t&>(local_pProperties[idx0]
                    .displayMode);
...
VKAPI_CALL GetDisplayPlaneCapabilitiesKHR(VkPhysicalDevice physicalDevice, VkDisplayModeKHR mode,
...
*pMsgCallback) {
return
unique_objects::CreateDebugReportCallbackEXT(instance, pCreateInfo, pAllocator, pMsgCallback
);
}
...
return
unique_objects::EnumerateInstanceLayerProperties(pCount, pProperties
);
}
...
== VK_NULL_HANDLE);
return
unique_objects::EnumerateDeviceLayerProperties(VK_NULL_HANDLE, pCount, pProperties
);
}
...
return
unique_objects::GetDeviceProcAddr(dev, funcName
);
}
...
return
unique_objects::GetInstanceProcAddr(instance, funcName
);
}
...
return
unique_objects::GetPhysicalDeviceProcAddr(instance, funcName
);
}
