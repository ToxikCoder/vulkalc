...
cvdescriptorset::DescriptorSetLayout::DescriptorSetLayout(const VkDescriptorSetLayoutCreateInfo* p_create_info,
        ...
        : layout_(layout), binding_count_(p_create_info->bindingCount), descriptor_count_(0),
          dynamic_descriptor_count_(0)
{
    ...
    for(uint32_t i = 0; i < binding_count_; ++i)
    {
        ...
        { // out-of-order binding number, need to insert into vector in-order
    ...
    for(uint32_t i = 0; i < create_info->bindingCount; ++i)
    {
        if(!bindings.insert(create_info->pBindings[i].binding).second)
        {
            skip |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                            __LINE__,
                            VALIDATION_ERROR_02345, "DS",
                            "duplicated binding number in VkDescriptorSetLayoutBinding. %s",
...
bool cvdescriptorset::DescriptorSetLayout::IsCompatible(const DescriptorSetLayout* rh_ds_layout,
                                                        std::string* error_msg) const
{
    ...
    if(descriptor_count_ != rh_ds_layout->descriptor_count_)
    {
        ...
        error_str << "DescriptorSetLayout " << layout_ << " has " << descriptor_count_
                  << " descriptors, but DescriptorSetLayout "
                  << rh_ds_layout->GetDescriptorSetLayout() << " has " << rh_ds_layout->descriptor_count_
                  << " descriptors.";
    ...
    for(auto binding : bindings_)
    {
        ...
        {
            std::stringstream error_str;
            error_str << "Binding " << binding.binding << " for DescriptorSetLayout " << layout_
                      << " has a descriptorCount of "
...
bool cvdescriptorset::DescriptorSetLayout::VerifyUpdateConsistency(uint32_t current_binding, uint32_t offset,
                                                                   ...
                                                                   std::string* error_msg) const
{
    ...
    while(update_count > binding_remaining)
    { // While our updates overstep current binding
        ...
        if(!IsNextBindingConsistent(current_binding++))
        {
            ...
            error_str << "Attempting " << type << " descriptor set " << set << " binding #" << orig_binding << " with #"
                      << update_count
                      << " descriptors being updated but this update oversteps the bounds of this binding and the "
                    ...
                    if(descriptor_class == GeneralBuffer)
                    {
                        ...
                        else
                        {
                            for(auto mem_binding : buffer_node->GetBoundMemory())
                            {
                                ...
                                {
                                    std::stringstream error_str;
                                    error_str << "Descriptor in binding #" << binding << " at global descriptor index "
                                              << i
                        ...
                        if(descriptors_[i]->IsDynamic())
                        {
                            ...
                            if(VK_WHOLE_SIZE == range)
                            {
                                ...
                                {
                                    std::stringstream error_str;
                                    error_str << "Dynamic descriptor in binding #" << binding
                                              << " at global descriptor index " << i
                                              << " uses buffer " << buffer
                                              << " with update range of VK_WHOLE_SIZE has dynamic offset " << dyn_offset
                                              << " combined with offset " << desc_offset
                                              << " that oversteps the buffer size of "
                                              << buffer_size << ".";
                                    *error = error_str.str();
                                    return false;
                                }
        ...
        if(descriptors_[start_idx]->IsStorage())
        {
            if(Image == descriptors_[start_idx]->descriptor_class)
            {
                for(uint32_t i = 0; i < p_layout_->GetDescriptorCountFromBinding(binding); ++i)
                {
                    ...
                    {
                        image_set->insert(
                                static_cast<ImageDescriptor*>(descriptors_[start_idx + i].get())->GetImageView());
                        num_updates++;
                    }
                }
            }
            else if(TexelBuffer == descriptors_[start_idx]->descriptor_class)
            {
                for(uint32_t i = 0; i < p_layout_->GetDescriptorCountFromBinding(binding); ++i)
                {
                    if(descriptors_[start_idx + i]->updated)
                    {
                        auto bufferview = static_cast<TexelDescriptor*>(descriptors_[start_idx + i].get())
                                ->GetBufferView();
            ...
            else if(GeneralBuffer == descriptors_[start_idx]->descriptor_class)
            {
                for(uint32_t i = 0; i < p_layout_->GetDescriptorCountFromBinding(binding); ++i)
                {
                    ...
                    {
                        buffer_set->insert(
                                static_cast<BufferDescriptor*>(descriptors_[start_idx + i].get())->GetBuffer());
                        num_updates++;
                    }
                }
            }
...
void cvdescriptorset::DescriptorSet::InvalidateBoundCmdBuffers()
{
    core_validation::invalidateCommandBuffers(device_data_, cb_bindings,
                                              {reinterpret_cast<uint64_t&>(set_),
...
bool cvdescriptorset::DescriptorSet::ValidateCopyUpdate(const debug_report_data* report_data,
                                                        const VkCopyDescriptorSet* update,
                                                        ...
                                                        std::string* error_msg)
{
    ...
    if((src_start_idx + update->descriptorCount) > src_set->GetTotalDescriptorCount())
    {
        ...
        *error_code = VALIDATION_ERROR_00965;
        ...
        error_str << "Attempting copy update from descriptorSet " << update->srcSet << " binding#" << update->srcBinding
                  ...
                  << " plus update array offset of " << update->srcArrayElement << " and update of "
                  << update->descriptorCount
                  << " descriptors oversteps total number of descriptors in set: "
    ...
    if((dst_start_idx + update->descriptorCount) > p_layout_->GetTotalDescriptorCount())
    {
        ...
        *error_code = VALIDATION_ERROR_00967;
        ...
        error_str << "Attempting copy update to descriptorSet " << set_ << " binding#" << update->dstBinding
                  ...
                  << " plus update array offset of " << update->dstArrayElement << " and update of "
                  << update->descriptorCount
                  << " descriptors oversteps total number of descriptors in set: "
                  << p_layout_->GetTotalDescriptorCount();
        *error_msg = error_str.str();
        return false;
    }
    ...
    *error_code = VALIDATION_ERROR_00968;
    ...
    if((!src_set->GetLayout()
            ->VerifyUpdateConsistency(update->srcBinding, update->srcArrayElement, update->descriptorCount,
                                      "copy update from", src_set->GetSet(), error_msg)) ||
       (!p_layout_->VerifyUpdateConsistency(update->dstBinding, update->dstArrayElement, update->descriptorCount,
                                            "copy update to",
                                            set_, error_msg)))
    {
        return false;
                                                       ...
                                                       const std::map<uint32_t, descriptor_req>& binding_req_map)
{
    ...
    cb_node->object_bindings
            .insert({reinterpret_cast<uint64_t&>(set_), VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_SET_EXT});
...
cvdescriptorset::SamplerDescriptor::SamplerDescriptor(const VkSampler* immut) : sampler_(VK_NULL_HANDLE),
                                                                                immutable_(false)
{
    updated = false;
    descriptor_class = PlainSampler;
    ...
    *error_code = VALIDATION_ERROR_00943;
    ...
    switch(image_layout)
    {
        case VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL:
            ...
            if((aspect_mask & VK_IMAGE_ASPECT_COLOR_BIT) != VK_IMAGE_ASPECT_COLOR_BIT)
            {
                ...
                error_str << "ImageView (" << image_view
                          << ") uses layout VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL but does "
        ...
        case VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL:
            ...
            if(!ds)
            {
                std::stringstream error_str;
                error_str << "ImageView (" << image_view << ") has layout " << string_VkImageLayout(image_layout)
                          << " but the image format is " << string_VkFormat(format)
            ...
            if(ds)
            {
                if(aspect_mask & VK_IMAGE_ASPECT_DEPTH_BIT)
                {
                    if(aspect_mask & VK_IMAGE_ASPECT_STENCIL_BIT)
                    {
                        // both  must NOT be set
                        std::stringstream error_str;
                        error_str << "ImageView (" << image_view << ") has layout "
    ...
    if(!error_usage_bit.empty())
    {
        ...
        error_str << "ImageView (" << image_view << ") with usage mask 0x" << usage
                  << " being used for a descriptor update of type " << string_VkDescriptorType(type)
                  << " does not have "
                  << error_usage_bit << " set.";
        *error_msg = error_str.str();
        return false;
...
void cvdescriptorset::SamplerDescriptor::BindCommandBuffer(const core_validation::layer_data* dev_data,
...
cvdescriptorset::ImageSamplerDescriptor::ImageSamplerDescriptor()
        : sampler_(VK_NULL_HANDLE), immutable_(false), image_view_(VK_NULL_HANDLE),
...
cvdescriptorset::ImageSamplerDescriptor::ImageSamplerDescriptor(const VkSampler* immut)
        : sampler_(VK_NULL_HANDLE), immutable_(true), image_view_(VK_NULL_HANDLE),
          image_layout_(VK_IMAGE_LAYOUT_UNDEFINED)
{
    updated = false;
    descriptor_class = ImageSampler;
...
void cvdescriptorset::ImageDescriptor::BindCommandBuffer(const core_validation::layer_data* dev_data,
                                                         GLOBAL_CB_NODE* cb_node)
{
    // Add binding for image
    auto iv_state = getImageViewState(dev_data, image_view_);
...
void cvdescriptorset::BufferDescriptor::BindCommandBuffer(const core_validation::layer_data* dev_data,
                                                          GLOBAL_CB_NODE* cb_node)
{
...
cvdescriptorset::TexelDescriptor::TexelDescriptor(const VkDescriptorType type) : buffer_view_(VK_NULL_HANDLE),
                                                                                 storage_(false)
{
    updated = false;
...
void cvdescriptorset::TexelDescriptor::BindCommandBuffer(const core_validation::layer_data* dev_data,
                                                         GLOBAL_CB_NODE* cb_node)
{
...
bool cvdescriptorset::DescriptorSet::ValidateWriteUpdate(const debug_report_data* report_data,
                                                         ...
                                                         std::string* error_msg)
{
    ...
    else
    {
        ...
        if(0 == p_layout_->GetDescriptorCountFromBinding(update->dstBinding))
        {
            *error_code = VALIDATION_ERROR_02348;
            std::stringstream error_str;
            error_str << "DescriptorSet " << set_ << " cannot updated binding " << update->dstBinding
    ...
    if(type != update->descriptorType)
    {
        *error_code = VALIDATION_ERROR_00937;
        std::stringstream error_str;
        error_str << "Attempting write update to descriptor set " << set_ << " binding #" << update->dstBinding
                  << " with type "
                  << string_VkDescriptorType(type) << " but update type is "
...
// If there's an error, update the error_msg string with details and return false, else return true
bool
cvdescriptorset::DescriptorSet::ValidateBufferUpdate(VkDescriptorBufferInfo const* buffer_info, VkDescriptorType type,
                                                     UNIQUE_VALIDATION_ERROR_CODE* error_code,
                                                     std::string* error_msg) const
{
    ...
    if(buffer_info->offset > buffer_node->createInfo.size)
    {
        *error_code = VALIDATION_ERROR_00959;
        std::stringstream error_str;
        error_str << "VkDescriptorBufferInfo offset of " << buffer_info->offset << " is greater than buffer "
                  << buffer_node->buffer
        ...
        if(buffer_info->range > (buffer_node->createInfo.size - buffer_info->offset))
        {
            *error_code = VALIDATION_ERROR_00961;
            ...
            error_str << "VkDescriptorBufferInfo range is " << buffer_info->range
                      << " which is greater than buffer size ("
    ...
    switch(update->descriptorType)
    {
        case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
        {
            for(uint32_t di = 0; di < update->descriptorCount; ++di)
            {
                ...
                if(!ValidateImageUpdate(image_view, image_layout, update->descriptorType, device_data_, error_code,
                                        error_msg))
                {
                    std::stringstream error_str;
        ...
        case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
        {
            for(uint32_t di = 0; di < update->descriptorCount; ++di)
            {
                ...
                if(!ValidateBufferUsage(getBufferState(device_data_, buffer), update->descriptorType, error_code,
                                        error_msg))
                {
                    std::stringstream error_str;
                    error_str << "Attempted write update to texel buffer descriptor failed due to: "
                              << error_msg->c_str();
                    *error_msg = error_str.str();
                    return false;
                }
...
// Verify that the contents of the update are ok, but don't perform actual update
bool cvdescriptorset::DescriptorSet::VerifyCopyUpdateContents(const VkCopyDescriptorSet* update,
                                                              const DescriptorSet* src_set,
    ...
    switch(src_set->descriptors_[index]->descriptor_class)
    {
        case PlainSampler:
        {
            for(uint32_t di = 0; di < update->descriptorCount; ++di)
            {
                ...
                {
                    auto update_sampler = static_cast<SamplerDescriptor*>(src_set->descriptors_[index + di].get())
                            ->GetSampler();
        ...
        case ImageSampler:
        {
            for(uint32_t di = 0; di < update->descriptorCount; ++di)
            {
                auto img_samp_desc = static_cast<const ImageSamplerDescriptor*>(src_set->descriptors_[index + di]
                        .get());
        ...
        case TexelBuffer:
        {
            for(uint32_t di = 0; di < update->descriptorCount; ++di)
            {
                auto buffer_view = static_cast<TexelDescriptor*>(src_set->descriptors_[index + di].get())
                        ->GetBufferView();
                auto bv_state = getBufferViewState(device_data_, buffer_view);
                if(!bv_state)
                {
                ...
                {
                    std::stringstream error_str;
                    error_str << "Attempted copy update to texel buffer descriptor failed due to: "
                              << error_msg->c_str();
                    *error_msg = error_str.str();
                    return false;
                }
                                                     ...
                                                     AllocateDescriptorSetsData* ds_data)
{
        ...
        if(!layout)
        {
            skip_call |=
                    log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                            ...
                            reinterpret_cast<const uint64_t&>(p_alloc_info->pSetLayouts[i]), __LINE__,
    ...
    for(uint32_t i = 0; i < VK_DESCRIPTOR_TYPE_RANGE_SIZE; i++)
    {
        if(ds_data->required_descriptors_by_type[i] > pool_state->availableDescriptorTypeCount[i])
        {
            skip_call |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
                                 VK_DEBUG_REPORT_OBJECT_TYPE_DESCRIPTOR_POOL_EXT,
                                 reinterpret_cast<const uint64_t&>(pool_state->pool), __LINE__, VALIDATION_ERROR_00912,
                                 "DS",
                                 ...
                                 reinterpret_cast<uint64_t&>(pool_state->pool),
                                 pool_state->availableDescriptorTypeCount[i],
                                                    ...
                                                    const core_validation::layer_data* dev_data)
{
    ...
    for(uint32_t i = 0; i < p_alloc_info->descriptorSetCount; i++)
    {
        auto new_ds = new cvdescriptorset::DescriptorSet(descriptor_sets[i], p_alloc_info->descriptorPool,
                                                         ds_data->layout_nodes[i],
                                                         dev_data);
