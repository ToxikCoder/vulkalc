...
VK_LAYER_EXPORT bool vk_format_is_depth_or_stencil(VkFormat
format) {
return (
vk_format_is_depth_and_stencil(format)
||
...
VK_LAYER_EXPORT bool vk_format_is_depth_and_stencil(VkFormat
format) {
bool is_ds = false;
...
VK_LAYER_EXPORT bool vk_format_is_stencil_only(VkFormat
format) {
return (format == VK_FORMAT_S8_UINT); }

// Return true if format is a depth-only format
VK_LAYER_EXPORT bool vk_format_is_depth_only(VkFormat
format) {
bool is_depth = false;
...
VK_LAYER_EXPORT bool vk_format_is_norm(VkFormat
format) {
bool is_norm = false;
...
VK_LAYER_EXPORT bool vk_format_is_int(VkFormat
format) {
return (
...
VK_LAYER_EXPORT bool vk_format_is_uint(VkFormat
format) {
bool is_uint = false;
...
VK_LAYER_EXPORT bool vk_format_is_sint(VkFormat
format) {
bool is_sint = false;
...
VK_LAYER_EXPORT bool vk_format_is_float(VkFormat
format) {
bool is_float = false;
...
VK_LAYER_EXPORT bool vk_format_is_srgb(VkFormat
format) {
bool is_srgb = false;
...
VK_LAYER_EXPORT bool vk_format_is_compressed(VkFormat
format) {
switch (format) {
case
VK_FORMAT_BC1_RGB_UNORM_BLOCK:
...
VK_LAYER_EXPORT VkExtent2D
vk_format_compressed_block_size(VkFormat
format) {
...
VK_LAYER_EXPORT VkFormatCompatibilityClass
vk_format_get_compatibility_class(VkFormat
format) {
auto item = vk_format_table.find(format);
if (item != vk_format_table.
...
return
VK_FORMAT_COMPATIBILITY_CLASS_NONE_BIT;
}

// Return size, in bytes, of a pixel of the specified format
VK_LAYER_EXPORT size_t
vk_format_get_size(VkFormat
format) {
auto item = vk_format_table.find(format);
if (item != vk_format_table.
...
VK_LAYER_EXPORT VkDeviceSize
vk_safe_modulo(VkDeviceSize
dividend,
VkDeviceSize divisor
) {
...
VK_LAYER_EXPORT VkStringErrorFlags

vk_string_validate(const int max_length, const char* utf8)
{
...
VK_LAYER_EXPORT void
layer_debug_actions(debug_report_data* report_data, std::vector<VkDebugReportCallbackEXT>& logging_callback,
