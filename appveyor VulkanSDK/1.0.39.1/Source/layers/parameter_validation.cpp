...
namespace parameter_validation
{
    ...
    VKAPI_CALL CreateDebugReportCallbackEXT(VkInstance instance,
                                            const VkDebugReportCallbackCreateInfoEXT* pCreateInfo,
                                            const VkAllocationCallbacks* pAllocator,
                                            VkDebugReportCallbackEXT* pMsgCallback)
    {
        auto data = get_my_data_ptr(get_dispatch_key(instance), instance_layer_data_map);
        VkResult result = data->dispatch_table
                .CreateDebugReportCallbackEXT(instance, pCreateInfo, pAllocator, pMsgCallback);
    ...
    )
{
    auto data = get_my_data_ptr(get_dispatch_key(instance), instance_layer_data_map);
    data->dispatch_table.
    DestroyDebugReportCallbackEXT(instance, msgCallback, pAllocator
    );

    layer_destroy_msg_callback(data
    ->report_data, msgCallback, pAllocator);
}

VKAPI_ATTR void VKAPI_CALL
DebugReportMessageEXT(VkInstance
instance,
VkDebugReportFlagsEXT flags,
        VkDebugReportObjectTypeEXT
objType,
uint64_t object, size_t
location,
int32_t msgCode,
...
auto data = get_my_data_ptr(get_dispatch_key(instance), instance_layer_data_map);
data->dispatch_table.
DebugReportMessageEXT(instance, flags, objType, object, location, msgCode, pLayerPrefix, pMsg
);
}
...
static bool ValidateEnumerator(VkQueueFlagBits const& enumerator)
{
    VkQueueFlagBits allFlags =
            (VkQueueFlagBits)(
                    VK_QUEUE_TRANSFER_BIT | VK_QUEUE_COMPUTE_BIT | VK_QUEUE_SPARSE_BINDING_BIT | VK_QUEUE_GRAPHICS_BIT);
    if(enumerator & (~allFlags))
    {
        return false;
    }
...
static bool ValidateEnumerator(VkMemoryPropertyFlagBits const& enumerator)
{
    VkMemoryPropertyFlagBits allFlags = (VkMemoryPropertyFlagBits)(
            VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT |
...
static bool ValidateEnumerator(VkImageCreateFlagBits const& enumerator)
{
    VkImageCreateFlagBits allFlags = (VkImageCreateFlagBits)(
            VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT | VK_IMAGE_CREATE_SPARSE_ALIASED_BIT |
...
static bool ValidateEnumerator(VkPipelineCreateFlagBits const& enumerator)
{
    VkPipelineCreateFlagBits allFlags = (VkPipelineCreateFlagBits)(
            VK_PIPELINE_CREATE_DERIVATIVE_BIT | VK_PIPELINE_CREATE_ALLOW_DERIVATIVES_BIT |
...
static bool ValidateEnumerator(VkShaderStageFlagBits const& enumerator)
{
    VkShaderStageFlagBits allFlags = (VkShaderStageFlagBits)(
            VK_SHADER_STAGE_ALL | VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_GEOMETRY_BIT |
...
static bool ValidateEnumerator(VkPipelineStageFlagBits const& enumerator)
{
    VkPipelineStageFlagBits allFlags = (VkPipelineStageFlagBits)(
            ...
            VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_GEOMETRY_SHADER_BIT |
                            ...
                            const char* validateString)
{
    ...
    else if(result & VK_STRING_ERROR_LENGTH)
    {
        ...
        skip = log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0, __LINE__,
                       INVALID_USAGE, LayerName, "%s: string %s exceeds max length %d", apiName,
    ...
    {
        skip = log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0, __LINE__,
                       INVALID_USAGE, LayerName, "%s: string %s contains invalid characters or is badly formed",
                       apiName,
                       stringName.get_name().c_str());
    }
    return skip;
                                        ...
                                        uint32_t index)
{
    ...
    {
        skip |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT)
        0, 0, __LINE__, 1, LayerName,
...
static bool
validate_queue_family_indices(layer_data* device_data, const char* function_name, const char* parameter_name,
...
static void
CheckInstanceRegisterExtensions(const VkInstanceCreateInfo* pCreateInfo, instance_layer_data* instance_data);

VKAPI_ATTR VkResult

VKAPI_CALL CreateInstance(const VkInstanceCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator,
                          VkInstance* pInstance)
{
    ...
    if(result == VK_SUCCESS)
    {
        ...
        my_instance_data->report_data =
                debug_report_create_instance(&my_instance_data->dispatch_table, *pInstance,
        ...
        if(pCreateInfo->pApplicationInfo)
        {
            ...
            {
                validate_string(my_instance_data->report_data, "vkCreateInstance",
                                "pCreateInfo->VkApplicationInfo->pEngineName",
...
if (!
layer_enable_tmp_callbacks(my_data
->report_data, my_data->num_tmp_callbacks, my_data->tmp_dbg_create_infos,
...
skip |=
parameter_validation_vkDestroyInstance(my_data
->report_data, pAllocator);

// Disable and cleanup the temporary callback(s):
if (callback_setup) {
layer_disable_tmp_callbacks(my_data
->report_data, my_data->num_tmp_callbacks, my_data->tmp_callbacks);
}
if (my_data->num_tmp_callbacks > 0) {
layer_free_tmp_callbacks(my_data
->tmp_dbg_create_infos, my_data->tmp_callbacks);
my_data->
num_tmp_callbacks = 0;
}

if (!skip) {
my_data->dispatch_table.
DestroyInstance(instance, pAllocator
);

// Clean up logging callback, if any
while (my_data->logging_callback.

size()

> 0) {
VkDebugReportCallbackEXT callback = my_data->logging_callback.back();
layer_destroy_msg_callback(my_data
->report_data, callback, pAllocator);
...
layer_debug_report_destroy_instance(my_data
->report_data);
...
VKAPI_ATTR VkResult
VKAPI_CALL EnumeratePhysicalDevices(VkInstance
instance,
...
skip |=
parameter_validation_vkEnumeratePhysicalDevices(my_data
->report_data, pPhysicalDeviceCount, pPhysicalDevices);

if (!skip) {
result = my_data->dispatch_table.EnumeratePhysicalDevices(instance, pPhysicalDeviceCount, pPhysicalDevices);
validate_result(my_data
->report_data, "vkEnumeratePhysicalDevices", result);
}
return
result;
}
...
skip |=
parameter_validation_vkGetPhysicalDeviceFeatures(my_data
->report_data, pFeatures);
...
VKAPI_ATTR void VKAPI_CALL
GetPhysicalDeviceFormatProperties(VkPhysicalDevice
physicalDevice,
VkFormat format,
        VkFormatProperties
*pFormatProperties) {
...
skip |=
parameter_validation_vkGetPhysicalDeviceFormatProperties(my_data
->report_data, format, pFormatProperties);
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceImageFormatProperties(VkPhysicalDevice
physicalDevice,
VkFormat format,
        VkImageType
type,
...
skip |=
parameter_validation_vkGetPhysicalDeviceImageFormatProperties(my_data
->report_data, format, type, tiling, usage,
flags, pImageFormatProperties);

if (!skip) {
result = my_data->dispatch_table
        .GetPhysicalDeviceImageFormatProperties(physicalDevice, format, type, tiling, usage, flags,
                                                pImageFormatProperties);

validate_result(my_data
->report_data, "vkGetPhysicalDeviceImageFormatProperties", result);
}
...
skip |=
parameter_validation_vkGetPhysicalDeviceProperties(my_data
->report_data, pProperties);
...
VKAPI_ATTR void VKAPI_CALL
GetPhysicalDeviceQueueFamilyProperties(VkPhysicalDevice
physicalDevice,
...
skip |=
parameter_validation_vkGetPhysicalDeviceQueueFamilyProperties(my_data
->report_data, pQueueFamilyPropertyCount,
pQueueFamilyProperties);
...
skip |=
parameter_validation_vkGetPhysicalDeviceMemoryProperties(my_data
->report_data, pMemoryProperties);
...
static void validateDeviceCreateInfo(VkPhysicalDevice
physicalDevice,
...
for (
uint32_t j = 0;
j<pCreateInfo->pQueueCreateInfos[i].
queueCount;
++j) {
...
if (pCreateInfo->pQueueCreateInfos[i].queueFamilyIndex >= properties.
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateDevice(VkPhysicalDevice
physicalDevice,
const VkDeviceCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkDevice
*pDevice) {
/*
...
assert(my_instance_data
!= nullptr);

skip |=
parameter_validation_vkCreateDevice(my_instance_data
->report_data, pCreateInfo, pAllocator, pDevice);
...
i++) {
skip |=
validate_string(my_instance_data
->report_data, "vkCreateDevice", "pCreateInfo->ppEnabledLayerNames",
...
// Advance the link info for the next element on the chain
chain_info->u.
pLayerInfo = chain_info->u.pLayerInfo->pNext;

result = fpCreateDevice(physicalDevice, pCreateInfo, pAllocator, pDevice);

validate_result(my_instance_data
->report_data, "vkCreateDevice", result);

if (result == VK_SUCCESS) {
layer_data* my_device_data = get_my_data_ptr(get_dispatch_key(*pDevice), layer_data_map);
assert(my_device_data
!= nullptr);

my_device_data->
report_data = layer_debug_report_create_device(my_instance_data->report_data, *pDevice);
layer_init_device_dispatch_table(* pDevice, & my_device_data
->dispatch_table, fpGetDeviceProcAddr);
...
my_instance_data->dispatch_table.
GetPhysicalDeviceProperties(physicalDevice, & device_properties
);
memcpy(& my_device_data
->device_limits, &device_properties.limits, sizeof(VkPhysicalDeviceLimits));
my_device_data->
...
my_device_data->
physical_device_features = *pCreateInfo->pEnabledFeatures;
} else {
memset(& my_device_data
->physical_device_features, 0, sizeof(VkPhysicalDeviceFeatures));
}
}
}
...
skip |=
parameter_validation_vkDestroyDevice(my_data
->report_data, pAllocator);
...
static bool PreGetDeviceQueue(VkDevice
device,
...
validate_queue_family_index(my_device_data,
"vkGetDeviceQueue", "queueFamilyIndex", queueFamilyIndex);
...
skip |=
parameter_validation_vkGetDeviceQueue(my_data
->report_data, queueFamilyIndex, queueIndex, pQueue);
...
VKAPI_CALL QueueSubmit(VkQueue queue, uint32_t submitCount, const VkSubmitInfo* pSubmits, VkFence fence)
{
    VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
    bool skip = false;
...
VKAPI_CALL QueueWaitIdle(VkQueue queue)
{
    layer_data* my_data = get_my_data_ptr(get_dispatch_key(queue), layer_data_map);
    assert(my_data != NULL);
...
VKAPI_ATTR VkResult
VKAPI_CALL DeviceWaitIdle(VkDevice
device) {
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
assert(my_data
!= NULL);
...
validate_result(my_data
->report_data, "vkDeviceWaitIdle", result);
...
VKAPI_ATTR VkResult
VKAPI_CALL AllocateMemory(VkDevice
device,
const VkMemoryAllocateInfo* pAllocateInfo,
const VkAllocationCallbacks* pAllocator, VkDeviceMemory
*pMemory) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkAllocateMemory(my_data
->report_data, pAllocateInfo, pAllocator, pMemory);

if (!skip) {
result = my_data->dispatch_table.AllocateMemory(device, pAllocateInfo, pAllocator, pMemory);

validate_result(my_data
->report_data, "vkAllocateMemory", result);
}
...
skip |=
parameter_validation_vkFreeMemory(my_data
->report_data, memory, pAllocator);
...
skip |=
parameter_validation_vkMapMemory(my_data
->report_data, memory, offset, size, flags, ppData);

if (!skip) {
result = my_data->dispatch_table.MapMemory(device, memory, offset, size, flags, ppData);

validate_result(my_data
->report_data, "vkMapMemory", result);
}
...
skip |=
parameter_validation_vkUnmapMemory(my_data
->report_data, memory);
...
VKAPI_ATTR VkResult
VKAPI_CALL FlushMappedMemoryRanges(VkDevice
device,
...
skip |=
parameter_validation_vkFlushMappedMemoryRanges(my_data
->report_data, memoryRangeCount, pMemoryRanges);

if (!skip) {
result = my_data->dispatch_table.FlushMappedMemoryRanges(device, memoryRangeCount, pMemoryRanges);

validate_result(my_data
->report_data, "vkFlushMappedMemoryRanges", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL InvalidateMappedMemoryRanges(VkDevice
device,
uint32_t memoryRangeCount,
const VkMappedMemoryRange* pMemoryRanges
) {
...
skip |=
parameter_validation_vkInvalidateMappedMemoryRanges(my_data
->report_data, memoryRangeCount, pMemoryRanges);
...
validate_result(my_data
->report_data, "vkInvalidateMappedMemoryRanges", result);
}
...
VKAPI_ATTR void VKAPI_CALL
GetDeviceMemoryCommitment(VkDevice
device,
VkDeviceMemory memory,
        VkDeviceSize
...
skip |=
parameter_validation_vkGetDeviceMemoryCommitment(my_data
->report_data, memory, pCommittedMemoryInBytes);
...
VKAPI_ATTR VkResult
VKAPI_CALL BindBufferMemory(VkDevice
device,
...
skip |=
parameter_validation_vkBindBufferMemory(my_data
->report_data, buffer, memory, memoryOffset);

if (!skip) {
result = my_data->dispatch_table.BindBufferMemory(device, buffer, memory, memoryOffset);

validate_result(my_data
->report_data, "vkBindBufferMemory", result);
}
...
skip |=
parameter_validation_vkBindImageMemory(my_data
->report_data, image, memory, memoryOffset);

if (!skip) {
result = my_data->dispatch_table.BindImageMemory(device, image, memory, memoryOffset);

validate_result(my_data
->report_data, "vkBindImageMemory", result);
}
...
VKAPI_ATTR void VKAPI_CALL
GetBufferMemoryRequirements(VkDevice
device,
VkBuffer buffer,
...
skip |=
parameter_validation_vkGetBufferMemoryRequirements(my_data
->report_data, buffer, pMemoryRequirements);
...
device,
VkImage image, VkMemoryRequirements
*pMemoryRequirements) {
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
assert(my_data
...
skip |=
parameter_validation_vkGetImageMemoryRequirements(my_data
->report_data, image, pMemoryRequirements);
...
static bool PostGetImageSparseMemoryRequirements(VkDevice
device,
VkImage image, uint32_t
*pNumRequirements,
VkSparseImageMemoryRequirements* pSparseMemoryRequirements
) {
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
if (pSparseMemoryRequirements != nullptr) {
if ((pSparseMemoryRequirements->formatProperties.
aspectMask&
...
VKAPI_ATTR void VKAPI_CALL
GetImageSparseMemoryRequirements(VkDevice
device,
VkImage image, uint32_t
...
skip |=
parameter_validation_vkGetImageSparseMemoryRequirements(my_data
->report_data, image, pSparseMemoryRequirementCount,
...
static bool PostGetPhysicalDeviceSparseImageFormatProperties(VkPhysicalDevice
physicalDevice,
VkFormat format, VkImageType
type,
VkSampleCountFlagBits samples, VkImageUsageFlags
usage,
VkImageTiling tiling,
        uint32_t
*pNumProperties,
...
VK_IMAGE_ASPECT_METADATA_BIT)) == 0) {
log_msg(my_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0, __LINE__, 1,
LayerName,
...
VKAPI_ATTR void VKAPI_CALL
GetPhysicalDeviceSparseImageFormatProperties(VkPhysicalDevice
physicalDevice,
VkFormat format,
        VkImageType
type,
...
skip |=
parameter_validation_vkGetPhysicalDeviceSparseImageFormatProperties(my_data
->report_data, format, type, samples,
usage, tiling, pPropertyCount, pProperties);

if (!skip) {
my_data->dispatch_table.
GetPhysicalDeviceSparseImageFormatProperties(physicalDevice, format, type, samples, usage, tiling,
...
VKAPI_CALL QueueBindSparse(VkQueue queue, uint32_t bindInfoCount, const VkBindSparseInfo* pBindInfo,
                           VkFence fence)
{
    VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
    bool skip = false;
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateFence(VkDevice
device,
const VkFenceCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkFence
*pFence) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreateFence(my_data
->report_data, pCreateInfo, pAllocator, pFence);

if (!skip) {
result = my_data->dispatch_table.CreateFence(device, pCreateInfo, pAllocator, pFence);

validate_result(my_data
->report_data, "vkCreateFence", result);
}
...
skip |=
parameter_validation_vkDestroyFence(my_data
->report_data, fence, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL ResetFences(VkDevice
device,
uint32_t fenceCount,
...
skip |=
parameter_validation_vkResetFences(my_data
->report_data, fenceCount, pFences);

if (!skip) {
result = my_data->dispatch_table.ResetFences(device, fenceCount, pFences);

validate_result(my_data
->report_data, "vkResetFences", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetFenceStatus(VkDevice
device,
VkFence fence
) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
...
skip |=
parameter_validation_vkGetFenceStatus(my_data
->report_data, fence);
...
validate_result(my_data
->report_data, "vkGetFenceStatus", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL WaitForFences(VkDevice
device,
uint32_t fenceCount,
const VkFence* pFences, VkBool32
waitAll,
uint64_t timeout
) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
...
skip |=
parameter_validation_vkWaitForFences(my_data
->report_data, fenceCount, pFences, waitAll, timeout);

if (!skip) {
result = my_data->dispatch_table.WaitForFences(device, fenceCount, pFences, waitAll, timeout);

validate_result(my_data
->report_data, "vkWaitForFences", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateSemaphore(VkDevice
device,
const VkSemaphoreCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkSemaphore
*pSemaphore) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreateSemaphore(my_data
->report_data, pCreateInfo, pAllocator, pSemaphore);

if (!skip) {
result = my_data->dispatch_table.CreateSemaphore(device, pCreateInfo, pAllocator, pSemaphore);

validate_result(my_data
->report_data, "vkCreateSemaphore", result);
}
...
skip |=
parameter_validation_vkDestroySemaphore(my_data
->report_data, semaphore, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateEvent(VkDevice
device,
const VkEventCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkEvent
*pEvent) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreateEvent(my_data
->report_data, pCreateInfo, pAllocator, pEvent);

if (!skip) {
result = my_data->dispatch_table.CreateEvent(device, pCreateInfo, pAllocator, pEvent);

validate_result(my_data
->report_data, "vkCreateEvent", result);
}
...
skip |=
parameter_validation_vkDestroyEvent(my_data
->report_data, event, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL GetEventStatus(VkDevice
device,
VkEvent event
) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
...
skip |=
parameter_validation_vkGetEventStatus(my_data
->report_data, event);
...
validate_result(my_data
->report_data, "vkGetEventStatus", result);
}
...
skip |=
parameter_validation_vkSetEvent(my_data
->report_data, event);
...
VKAPI_ATTR VkResult
VKAPI_CALL ResetEvent(VkDevice
device,
VkEvent event
) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
...
skip |=
parameter_validation_vkResetEvent(my_data
->report_data, event);
...
validate_result(my_data
->report_data, "vkResetEvent", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateQueryPool(VkDevice
device,
const VkQueryPoolCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkQueryPool
*pQueryPool) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* device_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreateQueryPool(device_data
->report_data, pCreateInfo, pAllocator, pQueryPool);
...
((pCreateInfo->
pipelineStatistics& (~AllVkQueryPipelineStatisticFlagBits)
) != 0)) {
skip |=
...
skip |=
parameter_validation_vkDestroyQueryPool(my_data
->report_data, queryPool, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL GetQueryPoolResults(VkDevice
device,
VkQueryPool queryPool, uint32_t
firstQuery,
uint32_t queryCount,
        size_t
dataSize,
void* pData, VkDeviceSize
stride,
...
skip |=
parameter_validation_vkGetQueryPoolResults(my_data
->report_data, queryPool, firstQuery, queryCount, dataSize,
pData, stride, flags);
...
validate_result(my_data
->report_data, "vkGetQueryPoolResults", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateBuffer(VkDevice
device,
const VkBufferCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkBuffer
*pBuffer) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* device_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreateBuffer(report_data, pCreateInfo, pAllocator, pBuffer
);

if (pCreateInfo != nullptr) {
// Buffer size must be greater than 0 (error 00663)
skip |=
ValidateGreaterThan(report_data,
"vkCreateBuffer", "pCreateInfo->size", static_cast
<uint32_t>(pCreateInfo
->size), 0u);
...
skip |=
parameter_validation_vkDestroyBuffer(my_data
->report_data, buffer, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateBufferView(VkDevice
device,
...
skip |=
parameter_validation_vkCreateBufferView(my_data
->report_data, pCreateInfo, pAllocator, pView);

if (!skip) {
result = my_data->dispatch_table.CreateBufferView(device, pCreateInfo, pAllocator, pView);

validate_result(my_data
->report_data, "vkCreateBufferView", result);
}
...
skip |=
parameter_validation_vkDestroyBufferView(my_data
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateImage(VkDevice
device,
...
skip |=
parameter_validation_vkCreateImage(report_data, pCreateInfo, pAllocator, pImage
);
...
skip |=
ValidateGreaterThan(report_data,
"vkCreateImage", "pCreateInfo->extent.width", pCreateInfo->extent.width, 0u);
skip |=
ValidateGreaterThan(report_data,
"vkCreateImage", "pCreateInfo->extent.height", pCreateInfo->extent.height, 0u);
skip |=
ValidateGreaterThan(report_data,
"vkCreateImage", "pCreateInfo->extent.depth", pCreateInfo->extent.depth, 0u);

// mipLevels must be greater than 0
skip |=
ValidateGreaterThan(report_data,
"vkCreateImage", "pCreateInfo->mipLevels", pCreateInfo->mipLevels, 0u);

// arrayLayers must be greater than 0
skip |=
ValidateGreaterThan(report_data,
"vkCreateImage", "pCreateInfo->arrayLayers", pCreateInfo->arrayLayers, 0u);

// If imageType is VK_IMAGE_TYPE_1D, both extent.height and extent.depth must be 1
if ((pCreateInfo->imageType == VK_IMAGE_TYPE_1D) && (pCreateInfo->extent.height != 1) && (pCreateInfo->extent.depth != 1)) {
skip |=
log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT)
0, 0, __LINE__,
...
skip |=
log_msg(
        report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, (VkDebugReportObjectTypeEXT)
0, 0, __LINE__, VALIDATION_ERROR_02130,
LayerName,
...
skip |=
parameter_validation_vkDestroyImage(my_data
->report_data, image, pAllocator);
...
static bool PreGetImageSubresourceLayout(VkDevice
device,
...
VKAPI_ATTR void VKAPI_CALL
GetImageSubresourceLayout(VkDevice
device,
VkImage image,
const VkImageSubresource* pSubresource,
        VkSubresourceLayout
...
skip |=
parameter_validation_vkGetImageSubresourceLayout(my_data
->report_data, image, pSubresource, pLayout);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateImageView(VkDevice
device,
const VkImageViewCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkImageView
*pView) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreateImageView(report_data, pCreateInfo, pAllocator, pView
);
...
validate_result(my_data
->report_data, "vkCreateImageView", result);
}
...
skip |=
parameter_validation_vkDestroyImageView(my_data
->report_data, imageView, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateShaderModule(VkDevice
device,
...
skip |=
parameter_validation_vkCreateShaderModule(my_data
->report_data, pCreateInfo, pAllocator, pShaderModule);
...
validate_result(my_data
->report_data, "vkCreateShaderModule", result);
}
...
skip |=
parameter_validation_vkDestroyShaderModule(my_data
->report_data, shaderModule, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreatePipelineCache(VkDevice
device,
const VkPipelineCacheCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkPipelineCache
*pPipelineCache) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreatePipelineCache(my_data
->report_data, pCreateInfo, pAllocator, pPipelineCache);
...
validate_result(my_data
->report_data, "vkCreatePipelineCache", result);
}
...
skip |=
parameter_validation_vkDestroyPipelineCache(my_data
->report_data, pipelineCache, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPipelineCacheData(VkDevice
device,
VkPipelineCache pipelineCache, size_t
*pDataSize,
void* pData
) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
...
skip |=
parameter_validation_vkGetPipelineCacheData(my_data
->report_data, pipelineCache, pDataSize, pData);

if (!skip) {
result = my_data->dispatch_table.GetPipelineCacheData(device, pipelineCache, pDataSize, pData);

validate_result(my_data
->report_data, "vkGetPipelineCacheData", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL MergePipelineCaches(VkDevice
device,
...
skip |=
parameter_validation_vkMergePipelineCaches(my_data
->report_data, dstCache, srcCacheCount, pSrcCaches);

if (!skip) {
result = my_data->dispatch_table.MergePipelineCaches(device, dstCache, srcCacheCount, pSrcCaches);

validate_result(my_data
->report_data, "vkMergePipelineCaches", result);
}
...
static bool PreCreateGraphicsPipelines(VkDevice
device,
...
if (pCreateInfos->pRasterizationState->
...
ParameterName("pCreateInfos[%i].pStages[%i].pName", ParameterName::IndexVector{
i, j}),
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateGraphicsPipelines(VkDevice
device,
VkPipelineCache pipelineCache, uint32_t
createInfoCount,
const VkGraphicsPipelineCreateInfo* pCreateInfos,
const VkAllocationCallbacks* pAllocator, VkPipeline
*pPipelines) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* device_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreateGraphicsPipelines(report_data, pipelineCache, createInfoCount, pCreateInfos,
...
skip |=
validate_struct_pnext(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pTessellationState->pNext", ParameterName::IndexVector{
i}), NULL,
...
validate_reserved_flags(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pTessellationState->flags", ParameterName::IndexVector{
i}),
...
skip |=
validate_struct_pnext(report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pViewportState->pNext", ParameterName::IndexVector{
i}),
NULL, pCreateInfos[i].pViewportState->pNext, 0, NULL, GeneratedHeaderVersion);

skip |=
validate_reserved_flags(report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pViewportState->flags", ParameterName::IndexVector{
i}),
...
state_index<pCreateInfos[i].pDynamicState->
dynamicStateCount;
++state_index) {
...
if (!
has_dynamic_viewport&& (pCreateInfos[i]
...
if (!
has_dynamic_scissor&& (pCreateInfos[i]
...
validate_struct_pnext(report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pMultisampleState->pNext", ParameterName::IndexVector{
i}),
...
validate_reserved_flags(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pMultisampleState->flags", ParameterName::IndexVector{
i}),
...
validate_bool32(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pMultisampleState->sampleShadingEnable", ParameterName::IndexVector{
i}),
...
validate_array(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pMultisampleState->rasterizationSamples", ParameterName::IndexVector{
i}),
ParameterName("pCreateInfos[%i].pMultisampleState->pSampleMask", ParameterName::IndexVector{
i}),
...
validate_bool32(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pMultisampleState->alphaToCoverageEnable", ParameterName::IndexVector{
i}),
...
validate_bool32(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pMultisampleState->alphaToOneEnable", ParameterName::IndexVector{
i}),
...
validate_struct_pnext(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->pNext", ParameterName::IndexVector{
i}), NULL,
...
validate_reserved_flags(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->flags", ParameterName::IndexVector{
i}),
...
validate_bool32(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->depthTestEnable", ParameterName::IndexVector{
i}),
...
validate_bool32(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->depthWriteEnable", ParameterName::IndexVector{
i}),
pCreateInfos[i].pDepthStencilState->depthWriteEnable);

skip |=
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->depthCompareOp", ParameterName::IndexVector{
i}),
...
validate_bool32(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->depthBoundsTestEnable", ParameterName::IndexVector{
i}),
...
validate_bool32(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->stencilTestEnable", ParameterName::IndexVector{
i}),
...
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->front.failOp", ParameterName::IndexVector{
i}),
...
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->front.passOp", ParameterName::IndexVector{
i}),
"VkStencilOp", VK_STENCIL_OP_BEGIN_RANGE, VK_STENCIL_OP_END_RANGE,
...
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->front.depthFailOp", ParameterName::IndexVector{
i}),
"VkStencilOp", VK_STENCIL_OP_BEGIN_RANGE, VK_STENCIL_OP_END_RANGE,
...
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->front.compareOp", ParameterName::IndexVector{
i}),
"VkCompareOp", VK_COMPARE_OP_BEGIN_RANGE, VK_COMPARE_OP_END_RANGE,
...
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->back.failOp", ParameterName::IndexVector{
i}), "VkStencilOp",
...
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->back.passOp", ParameterName::IndexVector{
i}), "VkStencilOp",
...
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->back.depthFailOp", ParameterName::IndexVector{
i}),
"VkStencilOp", VK_STENCIL_OP_BEGIN_RANGE, VK_STENCIL_OP_END_RANGE,
...
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pDepthStencilState->back.compareOp", ParameterName::IndexVector{
i}),
"VkCompareOp", VK_COMPARE_OP_BEGIN_RANGE, VK_COMPARE_OP_END_RANGE,
...
validate_struct_pnext(report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pColorBlendState->pNext", ParameterName::IndexVector{
i}),
...
validate_reserved_flags(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pColorBlendState->flags", ParameterName::IndexVector{
i}),
...
validate_bool32(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pColorBlendState->logicOpEnable", ParameterName::IndexVector{
i}),
...
validate_array(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pColorBlendState->attachmentCount", ParameterName::IndexVector{
i}),
ParameterName("pCreateInfos[%i].pColorBlendState->pAttachments", ParameterName::IndexVector{
i}),
...
attachmentIndex<pCreateInfos[i].pColorBlendState->
attachmentCount;
++attachmentIndex) {
skip |=
validate_bool32(report_data,
"vkCreateGraphicsPipelines",
...
skip |=
validate_ranged_enum(report_data,
"vkCreateGraphicsPipelines",
...
validate_ranged_enum(
        report_data,
"vkCreateGraphicsPipelines",
ParameterName("pCreateInfos[%i].pColorBlendState->logicOp", ParameterName::IndexVector{
i}), "VkLogicOp",
...
result = device_data->dispatch_table
...
bool PreCreateComputePipelines(VkDevice
device,
...
uint32_t i = 0;
validate_string(data
->report_data, "vkCreateComputePipelines",
ParameterName("pCreateInfos[%i].stage.pName", ParameterName::IndexVector{
i}), pCreateInfos[i].stage.pName);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateComputePipelines(VkDevice
device,
VkPipelineCache pipelineCache, uint32_t
createInfoCount,
const VkComputePipelineCreateInfo* pCreateInfos,
...
skip |=
parameter_validation_vkCreateComputePipelines(my_data
->report_data, pipelineCache, createInfoCount, pCreateInfos,
pAllocator, pPipelines);
...
validate_result(my_data
->report_data, "vkCreateComputePipelines", result);
}
...
->report_data, pipeline, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreatePipelineLayout(VkDevice
device,
const VkPipelineLayoutCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkPipelineLayout
*pPipelineLayout) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreatePipelineLayout(my_data
->report_data, pCreateInfo, pAllocator, pPipelineLayout);
...
validate_result(my_data
->report_data, "vkCreatePipelineLayout", result);
}
...
skip |=
parameter_validation_vkDestroyPipelineLayout(my_data
->report_data, pipelineLayout, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateSampler(VkDevice
device,
const VkSamplerCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkSampler
*pSampler) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* device_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreateSampler(report_data, pCreateInfo, pAllocator, pSampler
);
...
skip |=
validate_ranged_enum(report_data,
"vkCreateSampler", "pCreateInfo->compareOp", "VkCompareOp",
...
skip |=
validate_ranged_enum(report_data,
"vkCreateSampler", "pCreateInfo->borderColor", "VkBorderColor",
...
skip |=
parameter_validation_vkDestroySampler(my_data
->report_data, sampler, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateDescriptorSetLayout(VkDevice
device,
...
skip |=
parameter_validation_vkCreateDescriptorSetLayout(report_data, pCreateInfo, pAllocator, pSetLayout
);
...
descriptor_index<pCreateInfo->pBindings[i].
descriptorCount;
++descriptor_index) {
...
stageFlags& (~AllVkShaderStageFlagBits)
) != 0)) {
skip |=
log_msg(
        report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT,
...
VKAPI_ATTR void VKAPI_CALL
DestroyDescriptorSetLayout(VkDevice
device,
...
skip |=
parameter_validation_vkDestroyDescriptorSetLayout(my_data
->report_data, descriptorSetLayout, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateDescriptorPool(VkDevice
device,
...
skip |=
parameter_validation_vkCreateDescriptorPool(my_data
->report_data, pCreateInfo, pAllocator, pDescriptorPool);
...
validate_result(my_data
->report_data, "vkCreateDescriptorPool", result);
}
...
VKAPI_ATTR void VKAPI_CALL
DestroyDescriptorPool(VkDevice
device,
...
skip |=
parameter_validation_vkDestroyDescriptorPool(my_data
->report_data, descriptorPool, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL ResetDescriptorPool(VkDevice
device,
VkDescriptorPool descriptorPool,
...
skip |=
parameter_validation_vkResetDescriptorPool(my_data
->report_data, descriptorPool, flags);

if (!skip) {
result = my_data->dispatch_table.ResetDescriptorPool(device, descriptorPool, flags);

validate_result(my_data
->report_data, "vkResetDescriptorPool", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL AllocateDescriptorSets(VkDevice
device,
...
skip |=
parameter_validation_vkAllocateDescriptorSets(my_data
->report_data, pAllocateInfo, pDescriptorSets);

if (!skip) {
result = my_data->dispatch_table.AllocateDescriptorSets(device, pAllocateInfo, pDescriptorSets);

validate_result(my_data
->report_data, "vkAllocateDescriptorSets", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL FreeDescriptorSets(VkDevice
device,
VkDescriptorPool descriptorPool, uint32_t
descriptorSetCount,
...
skip |=
parameter_validation_vkFreeDescriptorSets(report_data, descriptorPool, descriptorSetCount, pDescriptorSets
);
...
// validate_array()
skip |=
validate_array(report_data,
"vkFreeDescriptorSets", "descriptorSetCount", "pDescriptorSets", descriptorSetCount,
...
skip |=
parameter_validation_vkUpdateDescriptorSets(report_data, descriptorWriteCount, pDescriptorWrites,
...
for (
uint32_t descriptor_index = 0;
descriptor_index<pDescriptorWrites[i].
descriptorCount;
++descriptor_index) {
skip |=
validate_required_handle(report_data,
...
descriptorIndex<pDescriptorWrites[i].
descriptorCount;
++descriptorIndex) {
skip |=
validate_required_handle(report_data,
...
for (
uint32_t j = 0;
j<pDescriptorWrites[i].
descriptorCount;
j++) {
if (pDescriptorWrites[i].pBufferInfo != NULL) {
if (
vk_safe_modulo(pDescriptorWrites[i]
.pBufferInfo[j].offset, uniformAlignment) != 0) {
skip |=
log_msg(
        device_data
...
if (
vk_safe_modulo(pDescriptorWrites[i]
.pBufferInfo[j].offset, storageAlignment) != 0) {
skip |=
log_msg(
        device_data
->report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT,
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateFramebuffer(VkDevice
device,
const VkFramebufferCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkFramebuffer
*pFramebuffer) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkCreateFramebuffer(my_data
->report_data, pCreateInfo, pAllocator, pFramebuffer);

if (!skip) {
result = my_data->dispatch_table.CreateFramebuffer(device, pCreateInfo, pAllocator, pFramebuffer);

validate_result(my_data
->report_data, "vkCreateFramebuffer", result);
}
...
skip |=
parameter_validation_vkDestroyFramebuffer(my_data
->report_data, framebuffer, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateRenderPass(VkDevice
device,
...
skip |=
parameter_validation_vkCreateRenderPass(my_data
->report_data, pCreateInfo, pAllocator, pRenderPass);
skip |=
PreCreateRenderPass(my_data, pCreateInfo
);
...
validate_result(my_data
->report_data, "vkCreateRenderPass", result);
}
...
skip |=
parameter_validation_vkDestroyRenderPass(my_data
->report_data, renderPass, pAllocator);
...
VKAPI_ATTR void VKAPI_CALL
GetRenderAreaGranularity(VkDevice
device,
...
skip |=
parameter_validation_vkGetRenderAreaGranularity(my_data
->report_data, renderPass, pGranularity);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateCommandPool(VkDevice
device,
const VkCommandPoolCreateInfo* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkCommandPool
*pCommandPool) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
validate_queue_family_index(my_data,
"vkCreateCommandPool", "pCreateInfo->queueFamilyIndex", pCreateInfo->queueFamilyIndex);

skip |=
parameter_validation_vkCreateCommandPool(my_data
->report_data, pCreateInfo, pAllocator, pCommandPool);

if (!skip) {
result = my_data->dispatch_table.CreateCommandPool(device, pCreateInfo, pAllocator, pCommandPool);

validate_result(my_data
->report_data, "vkCreateCommandPool", result);
}
...
skip |=
parameter_validation_vkDestroyCommandPool(my_data
->report_data, commandPool, pAllocator);
...
VKAPI_ATTR VkResult
VKAPI_CALL ResetCommandPool(VkDevice
device,
VkCommandPool commandPool, VkCommandPoolResetFlags
flags) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
parameter_validation_vkResetCommandPool(my_data
->report_data, commandPool, flags);

if (!skip) {
result = my_data->dispatch_table.ResetCommandPool(device, commandPool, flags);

validate_result(my_data
->report_data, "vkResetCommandPool", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL AllocateCommandBuffers(VkDevice
device,
...
skip |=
parameter_validation_vkAllocateCommandBuffers(my_data
->report_data, pAllocateInfo, pCommandBuffers);

if (!skip) {
result = my_data->dispatch_table.AllocateCommandBuffers(device, pAllocateInfo, pCommandBuffers);

validate_result(my_data
->report_data, "vkAllocateCommandBuffers", result);
}
...
VKAPI_ATTR void VKAPI_CALL
FreeCommandBuffers(VkDevice
device,
VkCommandPool commandPool, uint32_t
commandBufferCount,
...
skip |=
parameter_validation_vkFreeCommandBuffers(report_data, commandPool, commandBufferCount, pCommandBuffers
);
...
skip |=
validate_array(report_data,
"vkFreeCommandBuffers", "commandBufferCount", "pCommandBuffers", commandBufferCount,
...
static bool
PreBeginCommandBuffer(layer_data* dev_data, VkCommandBuffer commandBuffer, const VkCommandBufferBeginInfo* pBeginInfo)
{
    bool skip = false;
    ...
    if(pInfo != NULL)
    {
        if((dev_data->physical_device_features.inheritedQueries == VK_FALSE) &&
           (pInfo->occlusionQueryEnable != VK_FALSE))
        {
        ...
        if((dev_data->physical_device_features.inheritedQueries != VK_FALSE) &&
           (pInfo->occlusionQueryEnable != VK_FALSE))
        {
            skip |= validate_flags(dev_data->report_data, "vkBeginCommandBuffer",
...
VKAPI_ATTR VkResult
VKAPI_CALL BeginCommandBuffer(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkBeginCommandBuffer(report_data, pBeginInfo
);

// Validation for parameters excluded from the generated validation code due to a 'noautovalidity' tag in vk.xml
// TODO: pBeginInfo->pInheritanceInfo must not be NULL if commandBuffer is a secondary command buffer
skip |=
validate_struct_type(report_data,
"vkBeginCommandBuffer", "pBeginInfo->pInheritanceInfo",
...
skip |=
validate_struct_pnext(report_data,
"vkBeginCommandBuffer", "pBeginInfo->pInheritanceInfo->pNext", NULL,
pBeginInfo->pInheritanceInfo->pNext, 0, NULL, GeneratedHeaderVersion);

skip |=
validate_bool32(report_data,
"vkBeginCommandBuffer", "pBeginInfo->pInheritanceInfo->occlusionQueryEnable",
...
skip |=
validate_flags(report_data,
"vkBeginCommandBuffer", "pBeginInfo->pInheritanceInfo->pipelineStatistics",
"VkQueryPipelineStatisticFlagBits", AllVkQueryPipelineStatisticFlagBits,
pBeginInfo->pInheritanceInfo->pipelineStatistics, false);
}

skip |=
PreBeginCommandBuffer(device_data, commandBuffer, pBeginInfo
);
...
VKAPI_ATTR VkResult
VKAPI_CALL EndCommandBuffer(VkCommandBuffer
commandBuffer) {
...
validate_result(my_data
->report_data, "vkEndCommandBuffer", result);
...
VKAPI_ATTR VkResult
VKAPI_CALL ResetCommandBuffer(VkCommandBuffer
commandBuffer,
...
validate_result(my_data
->report_data, "vkResetCommandBuffer", result);
}
...
VKAPI_ATTR void VKAPI_CALL
CmdBindPipeline(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkCmdBindPipeline(my_data
->report_data, pipelineBindPoint, pipeline);
...
static bool preCmdSetViewport(layer_data* my_data, uint32_t viewport_count, const VkViewport* viewports)
{
    ...
    bool skip =
            validate_array(report_data, "vkCmdSetViewport", "viewportCount", "pViewports", viewport_count, viewports,
                           true, true);
        ...
        for(uint32_t viewportIndex = 0; viewportIndex < viewport_count; ++viewportIndex)
        {
            ...
            if(viewport.width <= 0 || viewport.width > limits.maxViewportDimensions[0])
            {
                skip |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                                ...
                                viewport.width, limits.maxViewportDimensions[0],
                                validation_error_map[VALIDATION_ERROR_01448]);
            }
            ...
            if(invalid_height)
            {
                skip |= log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                                ...
                                validation_error_map[VALIDATION_ERROR_01449]);
            }
            ...
            if(viewport.x < limits.viewportBoundsRange[0] || viewport.x > limits.viewportBoundsRange[1])
            {
                skip |=
                        log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                                __LINE__,
                                VALIDATION_ERROR_01450, LayerName,
                                "vkCmdSetViewport %d: x (%f) exceeds permitted bounds (%f,%f). %s",
            ...
            if(viewport.y < limits.viewportBoundsRange[0] || viewport.y > limits.viewportBoundsRange[1])
            {
                skip |=
                        log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                                __LINE__,
                                VALIDATION_ERROR_01450, LayerName,
                                "vkCmdSetViewport %d: y (%f) exceeds permitted bounds (%f,%f). %s",
            ...
            if(viewport.x + viewport.width > limits.viewportBoundsRange[1])
            {
                skip |=
                        log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                                ...
                                "vkCmdSetViewport %d: x (%f) + width (%f) exceeds permitted bound (%f). %s",
                                viewportIndex, viewport.x,
                                viewport.width, limits.viewportBoundsRange[1],
                                validation_error_map[VALIDATION_ERROR_01451]);
            }
            ...
            if(viewport.y + viewport.height > limits.viewportBoundsRange[1])
            {
                skip |=
                        log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0,
                                ...
                                "vkCmdSetViewport %d: y (%f) + height (%f) exceeds permitted bound (%f). %s",
                                viewportIndex, viewport.y,
                                viewport.height, limits.viewportBoundsRange[1],
                                validation_error_map[VALIDATION_ERROR_01452]);
            }
        }
    }
...
VKAPI_ATTR void VKAPI_CALL
CmdSetViewport(VkCommandBuffer
commandBuffer,
...
skip |=
preCmdSetViewport(my_data, viewportCount, pViewports
);
...
commandBuffer,
uint32_t firstScissor, uint32_t
scissorCount,
...
skip |=
parameter_validation_vkCmdSetScissor(my_data
->report_data, firstScissor, scissorCount, pScissors);

for (
uint32_t scissorIndex = 0;
scissorIndex<scissorCount;
++scissorIndex) {
...
<int32_t>(pScissor
.extent.width) > (INT_MAX - pScissor.offset.x)) {
skip |=
log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT,
0, __LINE__,
...
<int32_t>(pScissor
.extent.height) > (INT_MAX - pScissor.offset.y)) {
skip |=
log_msg(report_data, VK_DEBUG_REPORT_ERROR_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT,
0, __LINE__,
...
VKAPI_ATTR void VKAPI_CALL
CmdSetLineWidth(VkCommandBuffer
commandBuffer,
float lineWidth
) {
layer_data* my_data = get_my_data_ptr(get_dispatch_key(commandBuffer), layer_data_map);
...
VKAPI_ATTR void VKAPI_CALL
CmdSetDepthBias(VkCommandBuffer
commandBuffer,
...
VKAPI_ATTR void VKAPI_CALL
CmdSetBlendConstants(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkCmdSetBlendConstants(my_data
->report_data, blendConstants);
...
VKAPI_ATTR void VKAPI_CALL
CmdSetDepthBounds(VkCommandBuffer
commandBuffer,
...
VKAPI_ATTR void VKAPI_CALL
CmdSetStencilCompareMask(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkCmdSetStencilCompareMask(my_data
->report_data, faceMask, compareMask);
...
commandBuffer,
VkStencilFaceFlags faceMask, uint32_t
writeMask) {
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(commandBuffer), layer_data_map);
assert(my_data
...
skip |=
parameter_validation_vkCmdSetStencilWriteMask(my_data
->report_data, faceMask, writeMask);
...
VKAPI_ATTR void VKAPI_CALL
CmdSetStencilReference(VkCommandBuffer
commandBuffer,
VkStencilFaceFlags faceMask, uint32_t
reference) {
bool skip = false;
...
->report_data, faceMask, reference);
...
VKAPI_ATTR void VKAPI_CALL
CmdBindDescriptorSets(VkCommandBuffer
commandBuffer,
VkPipelineBindPoint pipelineBindPoint,
        VkPipelineLayout
layout,
uint32_t firstSet, uint32_t
descriptorSetCount,
...
skip |=
parameter_validation_vkCmdBindDescriptorSets(my_data
->report_data, pipelineBindPoint, layout, firstSet, descriptorSetCount,
...
skip |=
parameter_validation_vkCmdBindIndexBuffer(my_data
->report_data, buffer, offset, indexType);
...
VKAPI_ATTR void VKAPI_CALL
CmdBindVertexBuffers(VkCommandBuffer
commandBuffer,
uint32_t firstBinding, uint32_t
bindingCount,
...
skip |=
parameter_validation_vkCmdBindVertexBuffers(my_data
->report_data, firstBinding, bindingCount, pBuffers, pOffsets);
...
static bool PreCmdDraw(VkCommandBuffer
commandBuffer,
...
log_msg(my_data
->report_data, VK_DEBUG_REPORT_WARNING_BIT_EXT, VK_DEBUG_REPORT_OBJECT_TYPE_UNKNOWN_EXT, 0, __LINE__,
...
VKAPI_ATTR void VKAPI_CALL
CmdDraw(VkCommandBuffer
commandBuffer,
uint32_t vertexCount, uint32_t
instanceCount,
uint32_t firstVertex, uint32_t
firstInstance) {
layer_data* my_data = get_my_data_ptr(get_dispatch_key(commandBuffer), layer_data_map);
...
VKAPI_ATTR void VKAPI_CALL
CmdDrawIndexed(VkCommandBuffer
commandBuffer,
uint32_t indexCount, uint32_t
instanceCount,
uint32_t firstIndex, int32_t
vertexOffset,
...
VKAPI_ATTR void VKAPI_CALL
CmdDrawIndirect(VkCommandBuffer
commandBuffer,
VkBuffer buffer, VkDeviceSize
offset,
uint32_t count,
...
skip |=
parameter_validation_vkCmdDrawIndirect(my_data
->report_data, buffer, offset, count, stride);
...
VKAPI_ATTR void VKAPI_CALL
CmdDrawIndexedIndirect(VkCommandBuffer
commandBuffer,
VkBuffer buffer, VkDeviceSize
offset,
...
skip |=
parameter_validation_vkCmdDrawIndexedIndirect(my_data
->report_data, buffer, offset, count, stride);
...
VKAPI_ATTR void VKAPI_CALL
CmdDispatchIndirect(VkCommandBuffer
commandBuffer,
VkBuffer buffer, VkDeviceSize
offset) {
bool skip = false;
...
skip |=
parameter_validation_vkCmdDispatchIndirect(my_data
->report_data, buffer, offset);
...
VKAPI_ATTR void VKAPI_CALL
CmdCopyBuffer(VkCommandBuffer
commandBuffer,
VkBuffer srcBuffer, VkBuffer
dstBuffer,
...
skip |=
parameter_validation_vkCmdCopyBuffer(my_data
->report_data, srcBuffer, dstBuffer, regionCount, pRegions);
...
static bool PreCmdCopyImage(VkCommandBuffer
commandBuffer,
const VkImageCopy* pRegions
) {
layer_data* my_data = get_my_data_ptr(get_dispatch_key(commandBuffer), layer_data_map);
if (pRegions != nullptr) {
if ((pRegions->srcSubresource.
aspectMask& (VK_IMAGE_ASPECT_COLOR_BIT
...
skip |=
parameter_validation_vkCmdCopyImage(my_data
->report_data, srcImage, srcImageLayout, dstImage, dstImageLayout,
...
static bool PreCmdBlitImage(VkCommandBuffer
commandBuffer,
...
dstImageLayout,
uint32_t regionCount,
const VkImageBlit* pRegions, VkFilter
filter) {
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(commandBuffer), layer_data_map);
assert(my_data
...
skip |=
parameter_validation_vkCmdBlitImage(my_data
->report_data, srcImage, srcImageLayout, dstImage, dstImageLayout,
...
static bool PreCmdCopyBufferToImage(VkCommandBuffer
commandBuffer,
const VkBufferImageCopy* pRegions
) {
layer_data* my_data = get_my_data_ptr(get_dispatch_key(commandBuffer), layer_data_map);
if (pRegions != nullptr) {
if ((pRegions->imageSubresource.
aspectMask& (VK_IMAGE_ASPECT_COLOR_BIT
| VK_IMAGE_ASPECT_DEPTH_BIT |
VK_IMAGE_ASPECT_STENCIL_BIT | VK_IMAGE_ASPECT_METADATA_BIT)) == 0) {
log_msg(my_data
...
VKAPI_ATTR void VKAPI_CALL
CmdCopyBufferToImage(VkCommandBuffer
commandBuffer,
VkBuffer srcBuffer, VkImage
dstImage,
VkImageLayout dstImageLayout, uint32_t
regionCount,
const VkBufferImageCopy* pRegions
) {
bool skip = false;
...
skip |=
parameter_validation_vkCmdCopyBufferToImage(my_data
->report_data, srcBuffer, dstImage, dstImageLayout, regionCount,
pRegions);
...
static bool PreCmdCopyImageToBuffer(VkCommandBuffer
commandBuffer,
const VkBufferImageCopy* pRegions
) {
layer_data* my_data = get_my_data_ptr(get_dispatch_key(commandBuffer), layer_data_map);
...
VKAPI_ATTR void VKAPI_CALL
CmdCopyImageToBuffer(VkCommandBuffer
commandBuffer,
VkImage srcImage, VkImageLayout
srcImageLayout,
VkBuffer dstBuffer, uint32_t
regionCount,
const VkBufferImageCopy* pRegions
) {
bool skip = false;
...
skip |=
parameter_validation_vkCmdCopyImageToBuffer(my_data
->report_data, srcImage, srcImageLayout, dstBuffer, regionCount,
pRegions);
...
VKAPI_ATTR void VKAPI_CALL
CmdUpdateBuffer(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkCmdUpdateBuffer(my_data
->report_data, dstBuffer, dstOffset, dataSize, pData);
...
VKAPI_ATTR void VKAPI_CALL
CmdFillBuffer(VkCommandBuffer
commandBuffer,
VkBuffer dstBuffer, VkDeviceSize
dstOffset,
VkDeviceSize size, uint32_t
data) {
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(commandBuffer), layer_data_map);
assert(my_data
...
skip |=
parameter_validation_vkCmdFillBuffer(my_data
->report_data, dstBuffer, dstOffset, size, data);
...
VKAPI_ATTR void VKAPI_CALL
CmdClearColorImage(VkCommandBuffer
commandBuffer,
VkImage image, VkImageLayout
imageLayout,
const VkClearColorValue* pColor, uint32_t
rangeCount,
...
skip |=
parameter_validation_vkCmdClearColorImage(my_data
->report_data, image, imageLayout, pColor, rangeCount, pRanges);
...
VKAPI_ATTR void VKAPI_CALL
CmdClearDepthStencilImage(VkCommandBuffer
commandBuffer,
VkImage image, VkImageLayout
imageLayout,
const VkClearDepthStencilValue* pDepthStencil, uint32_t
rangeCount,
const VkImageSubresourceRange* pRanges
) {
bool skip = false;
...
skip |=
parameter_validation_vkCmdClearDepthStencilImage(my_data
->report_data, image, imageLayout, pDepthStencil,
rangeCount, pRanges);
...
VKAPI_ATTR void VKAPI_CALL
CmdClearAttachments(VkCommandBuffer
commandBuffer,
uint32_t attachmentCount,
const VkClearAttachment* pAttachments, uint32_t
rectCount,
...
skip |=
parameter_validation_vkCmdClearAttachments(my_data
->report_data, attachmentCount, pAttachments, rectCount, pRects);
...
static bool PreCmdResolveImage(VkCommandBuffer
commandBuffer,
...
VKAPI_ATTR void VKAPI_CALL
CmdResolveImage(VkCommandBuffer
commandBuffer,
VkImage srcImage, VkImageLayout
srcImageLayout,
VkImage dstImage, VkImageLayout
...
skip |=
parameter_validation_vkCmdResolveImage(my_data
->report_data, srcImage, srcImageLayout, dstImage, dstImageLayout,
regionCount, pRegions);
...
skip |=
parameter_validation_vkCmdSetEvent(my_data
->report_data, event, stageMask);
...
skip |=
parameter_validation_vkCmdResetEvent(my_data
->report_data, event, stageMask);
...
skip |=
parameter_validation_vkCmdWaitEvents(my_data
->report_data, eventCount, pEvents, srcStageMask, dstStageMask,
...
VKAPI_ATTR void VKAPI_CALL
CmdPipelineBarrier(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkCmdPipelineBarrier(my_data
->report_data, srcStageMask, dstStageMask, dependencyFlags,
memoryBarrierCount, pMemoryBarriers, bufferMemoryBarrierCount,
pBufferMemoryBarriers, imageMemoryBarrierCount, pImageMemoryBarriers);
...
skip |=
parameter_validation_vkCmdBeginQuery(my_data
->report_data, queryPool, slot, flags);
...
skip |=
parameter_validation_vkCmdEndQuery(my_data
->report_data, queryPool, slot);
...
VKAPI_ATTR void VKAPI_CALL
CmdResetQueryPool(VkCommandBuffer
commandBuffer,
VkQueryPool queryPool, uint32_t
firstQuery,
uint32_t queryCount
...
skip |=
parameter_validation_vkCmdResetQueryPool(my_data
->report_data, queryPool, firstQuery, queryCount);
...
bool PostCmdWriteTimestamp(VkCommandBuffer
commandBuffer,
VkPipelineStageFlagBits pipelineStage, VkQueryPool
queryPool,
uint32_t slot
) {
...
VKAPI_ATTR void VKAPI_CALL
CmdWriteTimestamp(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkCmdWriteTimestamp(my_data
->report_data, pipelineStage, queryPool, query);
...
VKAPI_ATTR void VKAPI_CALL
CmdCopyQueryPoolResults(VkCommandBuffer
commandBuffer,
VkQueryPool queryPool, uint32_t
firstQuery,
uint32_t queryCount, VkBuffer
dstBuffer,
...
skip |=
parameter_validation_vkCmdCopyQueryPoolResults(my_data
->report_data, queryPool, firstQuery, queryCount, dstBuffer,
...
VKAPI_ATTR void VKAPI_CALL
CmdPushConstants(VkCommandBuffer
commandBuffer,
VkPipelineLayout layout, VkShaderStageFlags
stageFlags,
...
skip |=
parameter_validation_vkCmdPushConstants(my_data
->report_data, layout, stageFlags, offset, size, pValues);
...
VKAPI_ATTR void VKAPI_CALL
CmdBeginRenderPass(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkCmdBeginRenderPass(my_data
->report_data, pRenderPassBegin, contents);
...
VKAPI_ATTR void VKAPI_CALL
CmdNextSubpass(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkCmdNextSubpass(my_data
...
VKAPI_ATTR void VKAPI_CALL
CmdExecuteCommands(VkCommandBuffer
commandBuffer,
...
skip |=
parameter_validation_vkCmdExecuteCommands(my_data
->report_data, commandBufferCount, pCommandBuffers);
...
VKAPI_ATTR VkResult
VKAPI_CALL EnumerateDeviceLayerProperties(VkPhysicalDevice
physicalDevice,
uint32_t* pCount,
        VkLayerProperties
*pProperties) {
...
VKAPI_CALL EnumerateInstanceExtensionProperties(const char* pLayerName, uint32_t* pCount,
...
VKAPI_ATTR VkResult
VKAPI_CALL EnumerateDeviceExtensionProperties(VkPhysicalDevice
physicalDevice,
...
if (pLayerName && !
strcmp(pLayerName, global_layer
.layerName))
...
return
get_my_data_ptr(get_dispatch_key(physicalDevice), instance_layer_data_map
)
->dispatch_table.
EnumerateDeviceExtensionProperties(physicalDevice,
NULL, pCount, pProperties);
}

static bool
require_device_extension(layer_data* my_data, bool flag, char const* function_name, char const* extension_name)
{
    if(!flag)
    {
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateSwapchainKHR(VkDevice
device,
...
skip |=
require_device_extension(my_data, my_data
->enables.khr_swapchain_enabled, "vkCreateSwapchainKHR", VK_KHR_SWAPCHAIN_EXTENSION_NAME);

skip |=
parameter_validation_vkCreateSwapchainKHR(my_data
->report_data, pCreateInfo, pAllocator, pSwapchain);

if (!skip) {
result = my_data->dispatch_table.CreateSwapchainKHR(device, pCreateInfo, pAllocator, pSwapchain);

validate_result(my_data
->report_data, "vkCreateSwapchainKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetSwapchainImagesKHR(VkDevice
device,
VkSwapchainKHR swapchain, uint32_t
...
skip |=
require_device_extension(my_data, my_data
->enables.khr_swapchain_enabled, "vkGetSwapchainImagesKHR", VK_KHR_SWAPCHAIN_EXTENSION_NAME);

skip |=
parameter_validation_vkGetSwapchainImagesKHR(my_data
->report_data, swapchain, pSwapchainImageCount, pSwapchainImages);

if (!skip) {
result = my_data->dispatch_table.GetSwapchainImagesKHR(device, swapchain, pSwapchainImageCount, pSwapchainImages);

validate_result(my_data
->report_data, "vkGetSwapchainImagesKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL AcquireNextImageKHR(VkDevice
device,
VkSwapchainKHR swapchain, uint64_t
timeout,
VkSemaphore semaphore, VkFence
fence,
...
skip |=
require_device_extension(my_data, my_data
->enables.khr_swapchain_enabled, "vkAcquireNextImageKHR", VK_KHR_SWAPCHAIN_EXTENSION_NAME);

skip |=
parameter_validation_vkAcquireNextImageKHR(my_data
->report_data, swapchain, timeout, semaphore, fence, pImageIndex);

if (!skip) {
result = my_data->dispatch_table.AcquireNextImageKHR(device, swapchain, timeout, semaphore, fence, pImageIndex);

validate_result(my_data
->report_data, "vkAcquireNextImageKHR", result);
}
...
VKAPI_CALL QueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo)
{
    VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
    bool skip = false;
    layer_data* my_data = get_my_data_ptr(get_dispatch_key(queue), layer_data_map);
...
skip |=
require_device_extension(my_data, my_data
->enables.khr_swapchain_enabled, "vkDestroySwapchainKHR", VK_KHR_SWAPCHAIN_EXTENSION_NAME);
...
static bool
require_instance_extension(void* instance, bool instance_extension_enables::*flag, char const* function_name,
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceSurfaceSupportKHR(VkPhysicalDevice
physicalDevice,
...
skip |=
require_instance_extension(physicalDevice, & instance_extension_enables::surface_enabled,
"vkGetPhysicalDeviceSurfaceSupportKHR", VK_KHR_SURFACE_EXTENSION_NAME);

skip |=
parameter_validation_vkGetPhysicalDeviceSurfaceSupportKHR(my_data
->report_data, queueFamilyIndex, surface, pSupported);
...
validate_result(my_data
->report_data, "vkGetPhysicalDeviceSurfaceSupportKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceSurfaceCapabilitiesKHR(VkPhysicalDevice
physicalDevice,
...
skip |=
parameter_validation_vkGetPhysicalDeviceSurfaceCapabilitiesKHR(my_data
->report_data, surface, pSurfaceCapabilities);

if (!skip) {
result = my_data->dispatch_table.GetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, surface, pSurfaceCapabilities);

validate_result(my_data
->report_data, "vkGetPhysicalDeviceSurfaceCapabilitiesKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceSurfaceFormatsKHR(VkPhysicalDevice
physicalDevice,
VkSurfaceKHR surface,
        uint32_t
*pSurfaceFormatCount,
...
skip |=
parameter_validation_vkGetPhysicalDeviceSurfaceFormatsKHR(my_data
->report_data, surface, pSurfaceFormatCount,
pSurfaceFormats);
...
validate_result(my_data
->report_data, "vkGetPhysicalDeviceSurfaceFormatsKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceSurfacePresentModesKHR(VkPhysicalDevice
physicalDevice,
VkSurfaceKHR surface,
        uint32_t
*pPresentModeCount,
...
skip |=
parameter_validation_vkGetPhysicalDeviceSurfacePresentModesKHR(my_data
->report_data, surface, pPresentModeCount,
pPresentModes);
...
validate_result(my_data
->report_data, "vkGetPhysicalDeviceSurfacePresentModesKHR", result);
}
...
skip |=
require_instance_extension(instance, & instance_extension_enables::surface_enabled,
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateSharedSwapchainsKHR(VkDevice
device,
...
skip |=
require_device_extension(my_data, my_data
->enables.khr_display_swapchain_enabled, "vkCreateSharedSwapchainsKHR",
VK_KHR_DISPLAY_SWAPCHAIN_EXTENSION_NAME);

skip |=
parameter_validation_vkCreateSharedSwapchainsKHR(my_data
->report_data, swapchainCount, pCreateInfos, pAllocator,
pSwapchains);
...
validate_result(my_data
->report_data, "vkCreateSharedSwapchainsKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceDisplayPropertiesKHR(VkPhysicalDevice
physicalDevice,
...
skip |=
require_instance_extension(physicalDevice, & instance_extension_enables::display_enabled,
...
validate_result(my_data
->report_data, "vkGetPhysicalDeviceDisplayPropertiesKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceDisplayPlanePropertiesKHR(VkPhysicalDevice
physicalDevice,
uint32_t* pPropertyCount,
...
validate_result(my_data
->report_data, "vkGetPhysicalDeviceDisplayPlanePropertiesKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetDisplayPlaneSupportedDisplaysKHR(VkPhysicalDevice
physicalDevice,
uint32_t planeIndex,
        uint32_t
*pDisplayCount,
...
validate_result(my_data
->report_data, "vkGetDisplayPlaneSupportedDisplaysKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetDisplayModePropertiesKHR(VkPhysicalDevice
physicalDevice,
...
validate_result(my_data
->report_data, "vkGetDisplayModePropertiesKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateDisplayModeKHR(VkPhysicalDevice
physicalDevice,
VkDisplayKHR display,
...
validate_result(my_data
->report_data, "vkCreateDisplayModeKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetDisplayPlaneCapabilitiesKHR(VkPhysicalDevice
physicalDevice,
...
validate_result(my_data
->report_data, "vkGetDisplayPlaneCapabilitiesKHR", result);
}
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateDisplayPlaneSurfaceKHR(VkInstance
instance,
...
skip |=
require_instance_extension(instance, & instance_extension_enables::display_enabled,
...
validate_result(my_data
->report_data, "vkCreateDisplayPlaneSurfaceKHR", result);
}
...
VKAPI_ATTR void VKAPI_CALL
GetPhysicalDeviceFeatures2KHR(VkPhysicalDevice
physicalDevice,
...
skip_call |=
parameter_validation_vkGetPhysicalDeviceFeatures2KHR(my_data
->report_data, pFeatures);
...
VKAPI_ATTR void VKAPI_CALL
GetPhysicalDeviceProperties2KHR(VkPhysicalDevice
physicalDevice,
...
skip_call |=
parameter_validation_vkGetPhysicalDeviceProperties2KHR(my_data
->report_data, pProperties);
...
VKAPI_ATTR void VKAPI_CALL
GetPhysicalDeviceFormatProperties2KHR(VkPhysicalDevice
physicalDevice,
VkFormat format,
...
skip_call |=
parameter_validation_vkGetPhysicalDeviceFormatProperties2KHR(my_data
->report_data, format, pFormatProperties);
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceImageFormatProperties2KHR(
        VkPhysicalDevice
physicalDevice,
...
skip_call |=
parameter_validation_vkGetPhysicalDeviceImageFormatProperties2KHR(my_data
->report_data, pImageFormatInfo,
pImageFormatProperties);
...
result = my_data->dispatch_table.GetPhysicalDeviceImageFormatProperties2KHR(physicalDevice, pImageFormatInfo,
                                                                            pImageFormatProperties);
validate_result(my_data
->report_data, "vkGetPhysicalDeviceImageFormatProperties2KHR", result);
}
...
VKAPI_ATTR void VKAPI_CALL
GetPhysicalDeviceQueueFamilyProperties2KHR(VkPhysicalDevice
physicalDevice,
uint32_t* pQueueFamilyPropertyCount,
...
skip_call |=
parameter_validation_vkGetPhysicalDeviceQueueFamilyProperties2KHR(my_data
->report_data, pQueueFamilyPropertyCount,
pQueueFamilyProperties);

if (!skip_call) {
my_data->dispatch_table.
GetPhysicalDeviceQueueFamilyProperties2KHR(physicalDevice, pQueueFamilyPropertyCount,
        pQueueFamilyProperties
);
}
}
...
VKAPI_ATTR void VKAPI_CALL
GetPhysicalDeviceMemoryProperties2KHR(VkPhysicalDevice
physicalDevice,
...
skip_call |=
parameter_validation_vkGetPhysicalDeviceMemoryProperties2KHR(my_data
->report_data, pMemoryProperties);
...
GetPhysicalDeviceSparseImageFormatProperties2KHR(
        VkPhysicalDevice
physicalDevice,
const VkPhysicalDeviceSparseImageFormatInfo2KHR* pFormatInfo, uint32_t
*pPropertyCount,
...
skip_call |=
parameter_validation_vkGetPhysicalDeviceSparseImageFormatProperties2KHR(my_data
->report_data, pFormatInfo,
pPropertyCount, pProperties);
...
VKAPI_ATTR void VKAPI_CALL
TrimCommandPoolKHR(VkDevice
device,
VkCommandPool commandPool, VkCommandPoolTrimFlagsKHR
flags) {
bool skip_call = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
assert(my_data
...
skip_call |=
require_device_extension(my_data, my_data
->enables.khr_maintenance1, "vkTrimCommandPoolKHR",
VK_KHR_MAINTENANCE1_EXTENSION_NAME);

skip_call |=
parameter_validation_vkTrimCommandPoolKHR(my_data
->report_data, commandPool, flags);

if (!skip_call) {
my_data->dispatch_table.
TrimCommandPoolKHR(device, commandPool, flags
);
}
}
...
VKAPI_ATTR VkResult
VKAPI_CALL DebugMarkerSetObjectTagEXT(VkDevice
device,
...
skip |=
require_device_extension(my_data, my_data
->enables.ext_debug_marker, "vkDebugMarkerSetObjectTagEXT",
VK_EXT_DEBUG_MARKER_EXTENSION_NAME);

skip |=
parameter_validation_vkDebugMarkerSetObjectTagEXT(my_data
->report_data, pTagInfo);
...
validate_result(my_data
->report_data, "vkDebugMarkerSetObjectTagEXT", result);
} else {
result = VK_SUCCESS;
}
}
...
VKAPI_ATTR VkResult
VKAPI_CALL DebugMarkerSetObjectNameEXT(VkDevice
device,
...
skip |=
require_device_extension(my_data, my_data
->enables.ext_debug_marker, "vkDebugMarkerSetObjectNameEXT",
VK_EXT_DEBUG_MARKER_EXTENSION_NAME);

skip |=
parameter_validation_vkDebugMarkerSetObjectNameEXT(my_data
->report_data, pNameInfo);
...
validate_result(my_data
->report_data, "vkDebugMarkerSetObjectNameEXT", result);
} else {
result = VK_SUCCESS;
}
}
...
VKAPI_ATTR void VKAPI_CALL
CmdDebugMarkerBeginEXT(VkCommandBuffer
commandBuffer,
...
skip |=
require_device_extension(my_data, my_data
->enables.ext_debug_marker, "vkCmdDebugMarkerBeginEXT",
VK_EXT_DEBUG_MARKER_EXTENSION_NAME);

skip |=
parameter_validation_vkCmdDebugMarkerBeginEXT(my_data
->report_data, pMarkerInfo);

if (!
skip&& my_data
->dispatch_table.CmdDebugMarkerBeginEXT) {
my_data->dispatch_table.
CmdDebugMarkerBeginEXT(commandBuffer, pMarkerInfo
);
}
}

VKAPI_ATTR void VKAPI_CALL
CmdDebugMarkerInsertEXT(VkCommandBuffer
commandBuffer,
VkDebugMarkerMarkerInfoEXT* pMarkerInfo
) {
bool skip = false;
...
skip |=
require_device_extension(my_data, my_data
->enables.ext_debug_marker, "vkCmdDebugMarkerInsertEXT",
VK_EXT_DEBUG_MARKER_EXTENSION_NAME);

skip |=
parameter_validation_vkCmdDebugMarkerInsertEXT(my_data
->report_data, pMarkerInfo);
...
my_data->dispatch_table.
CmdDebugMarkerInsertEXT(commandBuffer, pMarkerInfo
);
}
}
...
VKAPI_ATTR VkResult
VKAPI_CALL ReleaseDisplayEXT(VkPhysicalDevice
physicalDevice,
VkDisplayKHR display
) {
...
bool skip = false;
skip |=
require_instance_extension(physicalDevice, & instance_extension_enables::ext_direct_mode_display_enabled,
...
validate_result(my_data
->report_data, "vkGetRandROutputDisplayEXT", result);
}
return
result;
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceSurfaceCapabilities2EXT(VkPhysicalDevice
physicalDevice,
VkSurfaceKHR surface,
...
bool skip = false;
skip |=
require_instance_extension(physicalDevice, & instance_extension_enables::ext_display_surface_counter_enabled,
"vkGetPhysicalDeviceSurfaceCapabilities2EXT", VK_EXT_DISPLAY_SURFACE_COUNTER_EXTENSION_NAME);
skip |=
parameter_validation_vkGetPhysicalDeviceSurfaceCapabilities2EXT(my_data
->report_data, surface, pSurfaceCapabilities);
if (!skip) {
result = my_data->dispatch_table
        .GetPhysicalDeviceSurfaceCapabilities2EXT(physicalDevice, surface, pSurfaceCapabilities);
validate_result(my_data
->report_data, "vkGetPhysicalDeviceSurfaceCapabilities2EXT", result);
}
return
result;
}
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPhysicalDeviceExternalImageFormatPropertiesNV(
        VkPhysicalDevice
physicalDevice,
VkFormat format, VkImageType
type,
VkImageTiling tiling, VkImageUsageFlags
usage,
VkImageCreateFlags flags, VkExternalMemoryHandleTypeFlagsNV
externalHandleType,
...
skip |=
require_instance_extension(physicalDevice, & instance_extension_enables::nv_external_memory_capabilities_enabled,
"vkGetPhysicalDeviceExternalImageFormatPropertiesNV",
VK_NV_EXTERNAL_MEMORY_CAPABILITIES_EXTENSION_NAME);

skip |=
parameter_validation_vkGetPhysicalDeviceExternalImageFormatPropertiesNV(
        my_data
->report_data, format, type, tiling, usage, flags, externalHandleType, pExternalImageFormatProperties);
...
validate_result(my_data
->report_data, "vkGetPhysicalDeviceExternalImageFormatPropertiesNV", result);
}
...
VKAPI_ATTR void VKAPI_CALL
CmdProcessCommandsNVX(VkCommandBuffer
commandBuffer,
...
skip |=
require_device_extension(my_data, my_data
->enables.nvx_device_generated_commands, "vkCmdProcessCommandsNVX",
VK_NVX_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME);
skip |=
parameter_validation_vkCmdProcessCommandsNVX(my_data
->report_data, pProcessCommandsInfo);
if (!skip) {
my_data->dispatch_table.
CmdProcessCommandsNVX(commandBuffer, pProcessCommandsInfo
);
}
}

VKAPI_ATTR void VKAPI_CALL
CmdReserveSpaceForCommandsNVX(VkCommandBuffer
commandBuffer,
...
skip |=
require_device_extension(my_data, my_data
->enables.nvx_device_generated_commands, "vkCmdReserveSpaceForCommandsNVX",
VK_NVX_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME);
skip |=
parameter_validation_vkCmdReserveSpaceForCommandsNVX(my_data
->report_data, pReserveSpaceInfo);
if (!skip) {
my_data->dispatch_table.
CmdReserveSpaceForCommandsNVX(commandBuffer, pReserveSpaceInfo
);
}
}

VKAPI_ATTR VkResult
VKAPI_CALL CreateIndirectCommandsLayoutNVX(VkDevice
device,
...
skip |=
require_device_extension(my_data, my_data
->enables.nvx_device_generated_commands, "vkCreateIndirectCommandsLayoutNVX",
VK_NVX_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME);
skip |=
parameter_validation_vkCreateIndirectCommandsLayoutNVX(my_data
->report_data, pCreateInfo, pAllocator,
pIndirectCommandsLayout);
if (!skip) {
result = my_data->dispatch_table
        .CreateIndirectCommandsLayoutNVX(device, pCreateInfo, pAllocator, pIndirectCommandsLayout);
validate_result(my_data
->report_data, "vkCreateIndirectCommandsLayoutNVX", result);
}
return
result;
}
...
VKAPI_ATTR void VKAPI_CALL
DestroyIndirectCommandsLayoutNVX(VkDevice
device,
...
skip |=
require_device_extension(my_data, my_data
->enables.nvx_device_generated_commands, "vkDestroyIndirectCommandsLayoutNVX",
VK_NVX_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME);
#if 0 // Validation not automatically generated
...
if (!skip) {
my_data->dispatch_table.
DestroyIndirectCommandsLayoutNVX(device, indirectCommandsLayout, pAllocator
);
}
}

VKAPI_ATTR VkResult
VKAPI_CALL CreateObjectTableNVX(VkDevice
device,
const VkObjectTableCreateInfoNVX* pCreateInfo,
const VkAllocationCallbacks* pAllocator, VkObjectTableNVX
*pObjectTable) {
VkResult result = VK_ERROR_VALIDATION_FAILED_EXT;
bool skip = false;
layer_data* my_data = get_my_data_ptr(get_dispatch_key(device), layer_data_map);
...
skip |=
require_device_extension(my_data, my_data
->enables.nvx_device_generated_commands, "vkCreateObjectTableNVX",
VK_NVX_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME);
skip |=
parameter_validation_vkCreateObjectTableNVX(my_data
->report_data, pCreateInfo, pAllocator, pObjectTable);
if (!skip) {
result = my_data->dispatch_table.CreateObjectTableNVX(device, pCreateInfo, pAllocator, pObjectTable);
validate_result(my_data
->report_data, "vkCreateObjectTableNVX", result);
}
return
result;
}
...
VKAPI_ATTR void VKAPI_CALL
DestroyObjectTableNVX(VkDevice
device,
...
skip |=
require_device_extension(my_data, my_data
->enables.nvx_device_generated_commands, "vkDestroyObjectTableNVX",
VK_NVX_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME);
#if 0 // Validation not automatically generated
...
if (!skip) {
my_data->dispatch_table.
DestroyObjectTableNVX(device, objectTable, pAllocator
);
}
}

VKAPI_ATTR VkResult
VKAPI_CALL RegisterObjectsNVX(VkDevice
device,
VkObjectTableNVX objectTable, uint32_t
objectCount,
...
skip |=
require_device_extension(my_data, my_data
->enables.nvx_device_generated_commands, "vkRegisterObjectsNVX",
VK_NVX_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME);
skip |=
parameter_validation_vkRegisterObjectsNVX(my_data
->report_data, objectTable, objectCount, ppObjectTableEntries,
pObjectIndices);
if (!skip) {
result = my_data->dispatch_table
        .RegisterObjectsNVX(device, objectTable, objectCount, ppObjectTableEntries, pObjectIndices);
validate_result(my_data
->report_data, "vkRegisterObjectsNVX", result);
}
return
result;
}
...
VKAPI_ATTR VkResult
VKAPI_CALL UnregisterObjectsNVX(VkDevice
device,
VkObjectTableNVX objectTable, uint32_t
objectCount,
...
skip |=
require_device_extension(my_data, my_data
->enables.nvx_device_generated_commands, "vkUnregisterObjectsNVX",
VK_NVX_DEVICE_GENERATED_COMMANDS_EXTENSION_NAME);
skip |=
parameter_validation_vkUnregisterObjectsNVX(my_data
->report_data, objectTable, objectCount, pObjectEntryTypes,
pObjectIndices);
if (!skip) {
result = my_data->dispatch_table
        .UnregisterObjectsNVX(device, objectTable, objectCount, pObjectEntryTypes, pObjectIndices);
validate_result(my_data
->report_data, "vkUnregisterObjectsNVX", result);
}
return
result;
}
...
VKAPI_ATTR void VKAPI_CALL
GetPhysicalDeviceGeneratedCommandsPropertiesNVX(VkPhysicalDevice
physicalDevice,
...
!= NULL);
skip |=
parameter_validation_vkGetPhysicalDeviceGeneratedCommandsPropertiesNVX(my_data
->report_data, pFeatures, pLimits);
if (!skip) {
my_data->dispatch_table.
...
VKAPI_CALL GetDeviceProcAddr(VkDevice
device,
const char* funcName
) {
assert(device);
...
if (
validate_string(data
->report_data, "vkGetDeviceProcAddr", "funcName", funcName)) {
return NULL;
}
...
VKAPI_ATTR PFN_vkVoidFunction
VKAPI_CALL GetInstanceProcAddr(VkInstance
instance,
const char* funcName
) {
...
VKAPI_ATTR PFN_vkVoidFunction
VKAPI_CALL GetPhysicalDeviceProcAddr(VkInstance
instance,
const char* funcName
) {
assert(instance);
...
return nullptr;
return data->dispatch_table.
GetPhysicalDeviceProcAddr(instance, funcName
);
}
    ...
    static const struct
    {
        ...
        PFN_vkVoidFunction proc;
            ...
            {"vkCreateIndirectCommandsLayoutNVX",
             reinterpret_cast<PFN_vkVoidFunction>(CreateIndirectCommandsLayoutNVX)},
            {"vkDestroyIndirectCommandsLayoutNVX",
             reinterpret_cast<PFN_vkVoidFunction>(DestroyIndirectCommandsLayoutNVX)},
    ...
    static const struct
    {
        ...
        PFN_vkVoidFunction proc;
            ...
            {"vkGetPhysicalDeviceDisplayPropertiesKHR",
             reinterpret_cast<PFN_vkVoidFunction>(GetPhysicalDeviceDisplayPropertiesKHR)},
            {"vkGetPhysicalDeviceDisplayPlanePropertiesKHR",
             reinterpret_cast<PFN_vkVoidFunction>(GetPhysicalDeviceDisplayPlanePropertiesKHR)},
            {"vkGetDisplayPlaneSupportedDisplaysKHR",
             reinterpret_cast<PFN_vkVoidFunction>(GetDisplayPlaneSupportedDisplaysKHR)},
...
VKAPI_ATTR VkResult
VKAPI_CALL vkCreateDebugReportCallbackEXT(VkInstance
instance,
const VkDebugReportCallbackCreateInfoEXT* pCreateInfo,
const VkAllocationCallbacks* pAllocator,
...
*pMsgCallback) {
return
parameter_validation::CreateDebugReportCallbackEXT(instance, pCreateInfo, pAllocator, pMsgCallback
);
}
...
return
parameter_validation::EnumerateInstanceLayerProperties(pCount, pProperties
);
}
...
== VK_NULL_HANDLE);
return
parameter_validation::EnumerateDeviceLayerProperties(VK_NULL_HANDLE, pCount, pProperties
);
}
...
== VK_NULL_HANDLE);
return
parameter_validation::EnumerateDeviceExtensionProperties(VK_NULL_HANDLE, pLayerName, pCount, pProperties
);
}
...
return
parameter_validation::GetDeviceProcAddr(dev, funcName
);
}
...
VK_LAYER_EXPORT VKAPI_ATTR
PFN_vkVoidFunction VKAPI_CALL
vk_layerGetPhysicalDeviceProcAddr(VkInstance
instance,
const char* funcName
) {
return
parameter_validation::GetPhysicalDeviceProcAddr(instance, funcName
);
}
...
pVersionStruct->
pfnGetPhysicalDeviceProcAddr = vk_layerGetPhysicalDeviceProcAddr;
}
