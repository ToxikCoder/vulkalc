...
// Declare only
    ...
    VKAPI_CALL EnumerateInstanceExtensionProperties(
...
// Declare only
    ...
    VKAPI_CALL EnumerateInstanceLayerProperties(
            uint32_t* pPropertyCount,
...
// Declare only
    ...
    VKAPI_CALL EnumerateDeviceLayerProperties(
            VkPhysicalDevice physicalDevice,
            uint32_t* pPropertyCount,
            VkLayerProperties* pProperties);
            ...
            VkFence fence)
    {
            ...
            if(pSubmits)
            {
                ...
                for(uint32_t index0 = 0; index0 < submitCount; ++index0)
                {
                    ...
                    if(local_pSubmits[index0].pWaitSemaphores)
                    {
                        ...
                        {
                            local_pSubmits[index0].pWaitSemaphores[index1] = (VkSemaphore) dev_data
                                    ->unique_id_mapping[reinterpret_cast<const uint64_t&>(local_pSubmits[index0]
                                    .pWaitSemaphores[index1])];
                        }
                    }
                    if(local_pSubmits[index0].pSignalSemaphores)
                    {
                        ...
                        {
                            local_pSubmits[index0].pSignalSemaphores[index1] = (VkSemaphore) dev_data
                                    ->unique_id_mapping[reinterpret_cast<const uint64_t&>(local_pSubmits[index0]
        ...
        VkResult result = dev_data->device_dispatch_table
                ->QueueSubmit(queue, submitCount, (const VkSubmitInfo*) local_pSubmits, fence);
        if(local_pSubmits)
    ...
    )
{
    ...
    dev_data->unique_id_mapping[memory_id];
    ...
    dev_data->device_dispatch_table->
...
VKAPI_ATTR VkResult
VKAPI_CALL FlushMappedMemoryRanges(
        VkDevice
device,
...
for (
uint32_t index0 = 0;
index0<memoryRangeCount;
++index0) {
local_pMemoryRanges[index0].
initialize(& pMemoryRanges[index0]);
if (pMemoryRanges[index0].memory) {
local_pMemoryRanges[index0].
memory = (VkDeviceMemory) dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(pMemoryRanges[index0].memory)];
}
}
}
}
VkResult result = dev_data->device_dispatch_table
        ->FlushMappedMemoryRanges(device, memoryRangeCount, (const VkMappedMemoryRange*) local_pMemoryRanges);
...
VKAPI_ATTR VkResult
VKAPI_CALL InvalidateMappedMemoryRanges(
        VkDevice
device,
uint32_t memoryRangeCount,
const VkMappedMemoryRange* pMemoryRanges
...
VkResult result = dev_data->device_dispatch_table
        ->InvalidateMappedMemoryRanges(device, memoryRangeCount, (const VkMappedMemoryRange*) local_pMemoryRanges);
if (local_pMemoryRanges)
delete[]
local_pMemoryRanges;
return
...
memory = (VkDeviceMemory)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(memory)];
}
        ...
        VkFence fence)
{
        ...
        if(pBindInfo)
        {
            ...
            for(uint32_t index0 = 0; index0 < bindInfoCount; ++index0)
            {
                ...
                if(local_pBindInfo[index0].pWaitSemaphores)
                {
                    ...
                    {
                        local_pBindInfo[index0].pWaitSemaphores[index1] = (VkSemaphore) dev_data
                                ->unique_id_mapping[reinterpret_cast<const uint64_t&>(local_pBindInfo[index0]
                                .pWaitSemaphores[index1])];
                    }
                }
                if(local_pBindInfo[index0].pBufferBinds)
                {
                    for(uint32_t index1 = 0; index1 < local_pBindInfo[index0].bufferBindCount; ++index1)
                    {
                        ...
                        {
                            local_pBindInfo[index0].pBufferBinds[index1].buffer = (VkBuffer) dev_data
                                    ->unique_id_mapping[reinterpret_cast<const uint64_t&>(pBindInfo[index0]
                                    .pBufferBinds[index1].buffer)];
                        }
                        if(local_pBindInfo[index0].pBufferBinds[index1].pBinds)
                        {
                            for(uint32_t index2 = 0;
                                index2 < local_pBindInfo[index0].pBufferBinds[index1].bindCount; ++index2)
                            {
                                if(pBindInfo[index0].pBufferBinds[index1].pBinds[index2].memory)
                                {
                                    local_pBindInfo[index0].pBufferBinds[index1].pBinds[index2]
                                            .memory = (VkDeviceMemory)
                                    dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(pBindInfo[index0]
                                            .pBufferBinds[index1].pBinds[index2].memory)];
                                }
                            }
                        }
                    }
                ...
                if(local_pBindInfo[index0].pImageOpaqueBinds)
                {
                    for(uint32_t index1 = 0; index1 < local_pBindInfo[index0].imageOpaqueBindCount; ++index1)
                    {
                        ...
                        {
                            local_pBindInfo[index0].pImageOpaqueBinds[index1].image = (VkImage) dev_data
                                    ->unique_id_mapping[reinterpret_cast<const uint64_t&>(pBindInfo[index0]
                                    .pImageOpaqueBinds[index1].image)];
                        }
                        if(local_pBindInfo[index0].pImageOpaqueBinds[index1].pBinds)
                        {
                            for(uint32_t index2 = 0;
                                index2 < local_pBindInfo[index0].pImageOpaqueBinds[index1].bindCount; ++index2)
                            {
                                if(pBindInfo[index0].pImageOpaqueBinds[index1].pBinds[index2].memory)
                                {
                                    local_pBindInfo[index0].pImageOpaqueBinds[index1].pBinds[index2]
                                            .memory = (VkDeviceMemory)
                                    dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(pBindInfo[index0]
                                            .pImageOpaqueBinds[index1].pBinds[index2].memory)];
                                }
                            }
                        }
                    }
                ...
                if(local_pBindInfo[index0].pImageBinds)
                {
                    for(uint32_t index1 = 0; index1 < local_pBindInfo[index0].imageBindCount; ++index1)
                    {
                        ...
                        {
                            local_pBindInfo[index0].pImageBinds[index1].image = (VkImage) dev_data
                                    ->unique_id_mapping[reinterpret_cast<const uint64_t&>(pBindInfo[index0]
                                    .pImageBinds[index1].image)];
                        }
                        if(local_pBindInfo[index0].pImageBinds[index1].pBinds)
                        {
                            for(uint32_t index2 = 0;
                                index2 < local_pBindInfo[index0].pImageBinds[index1].bindCount; ++index2)
                            {
                                if(pBindInfo[index0].pImageBinds[index1].pBinds[index2].memory)
                                {
                                    local_pBindInfo[index0].pImageBinds[index1].pBinds[index2].memory = (VkDeviceMemory)
                                    dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(pBindInfo[index0]
                                            .pImageBinds[index1].pBinds[index2].memory)];
                                }
                            }
                        }
                    }
                ...
                if(local_pBindInfo[index0].pSignalSemaphores)
                {
                    ...
                    {
                        local_pBindInfo[index0].pSignalSemaphores[index1] = (VkSemaphore) dev_data
                                ->unique_id_mapping[reinterpret_cast<const uint64_t&>(local_pBindInfo[index0]
                                .pSignalSemaphores[index1])];
                    }
                }
    ...
    VkResult result = dev_data->device_dispatch_table
            ->QueueBindSparse(queue, bindInfoCount, (const VkBindSparseInfo*) local_pBindInfo, fence);
    if(local_pBindInfo)
...
fence = (VkFence)
dev_data->unique_id_mapping[fence_id];
dev_data->unique_id_mapping.
erase(fence_id);
lock.
...
index0<fenceCount;
++index0) {
local_pFences[index0] = (VkFence)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pFences[index0])
];
}
}
}
VkResult result = dev_data->device_dispatch_table->ResetFences(device, fenceCount,(
...
std::lock_guard <std::mutex> lock(global_lock);
fence = (VkFence)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(fence)];
}
...
VkResult result = dev_data->device_dispatch_table->WaitForFences(device, fenceCount,(
const VkFence*)local_pFences, waitAll, timeout);
if (local_pFences)
delete[]
local_pFences;
return
...
dev_data->unique_id_mapping[semaphore_id];
dev_data->unique_id_mapping.
erase(semaphore_id);
lock.
...
event = (VkEvent)
dev_data->unique_id_mapping[event_id];
dev_data->unique_id_mapping.
erase(event_id);
lock.
...
std::lock_guard <std::mutex> lock(global_lock);
event = (VkEvent)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(event)];
}
...
dev_data->unique_id_mapping[queryPool_id];
dev_data->unique_id_mapping.
erase(queryPool_id);
lock.
...
VKAPI_ATTR VkResult
VKAPI_CALL GetQueryPoolResults(
        VkDevice
device,
VkQueryPool queryPool,
        uint32_t
...
std::lock_guard <std::mutex> lock(global_lock);
queryPool = (VkQueryPool)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(queryPool)];
}
VkResult result = dev_data->device_dispatch_table
...
buffer = (VkBuffer)
dev_data->unique_id_mapping[buffer_id];
dev_data->unique_id_mapping.
erase(buffer_id);
lock.
...
local_pCreateInfo->
buffer = (VkBuffer)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pCreateInfo
->buffer)];
}
}
}
VkResult result = dev_data->device_dispatch_table
        ->CreateBufferView(device, (const VkBufferViewCreateInfo*) local_pCreateInfo, pAllocator, pView);
...
dev_data->unique_id_mapping[bufferView_id];
dev_data->unique_id_mapping.
erase(bufferView_id);
lock.
...
image = (VkImage)
dev_data->unique_id_mapping[image_id];
dev_data->unique_id_mapping.
erase(image_id);
lock.
...
std::lock_guard <std::mutex> lock(global_lock);
image = (VkImage)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(image)];
}
dev_data->device_dispatch_table->
...
VkResult result = dev_data->device_dispatch_table
        ->CreateImageView(device, (const VkImageViewCreateInfo*) local_pCreateInfo, pAllocator, pView);
if (local_pCreateInfo)
delete
local_pCreateInfo;
if (VK_SUCCESS == result) {
...
dev_data->unique_id_mapping[imageView_id];
dev_data->unique_id_mapping.
erase(imageView_id);
lock.
...
dev_data->unique_id_mapping[shaderModule_id];
dev_data->unique_id_mapping.
erase(shaderModule_id);
lock.
...
VKAPI_ATTR VkResult
VKAPI_CALL CreatePipelineCache(
        VkDevice
device,
...
dev_data->unique_id_mapping[pipelineCache_id];
dev_data->unique_id_mapping.
erase(pipelineCache_id);
lock.
...
VKAPI_ATTR VkResult
VKAPI_CALL GetPipelineCacheData(
        VkDevice
device,
VkPipelineCache pipelineCache,
        size_t
...
std::lock_guard <std::mutex> lock(global_lock);
pipelineCache = (VkPipelineCache)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(pipelineCache)];
}
...
VKAPI_ATTR VkResult
VKAPI_CALL MergePipelineCaches(
        VkDevice
device,
...
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(dstCache)];
if (pSrcCaches) {
local_pSrcCaches = new VkPipelineCache[srcCacheCount];
for (
uint32_t index0 = 0;
index0<srcCacheCount;
++index0) {
local_pSrcCaches[index0] = (VkPipelineCache)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pSrcCaches[index0])
];
}
}
}
VkResult result = dev_data->device_dispatch_table->MergePipelineCaches(device, dstCache, srcCacheCount,(
...
// Declare only
VKAPI_ATTR VkResult
VKAPI_CALL CreateGraphicsPipelines(
        VkDevice
device,
VkPipelineCache pipelineCache,
        uint32_t
...
pipeline = (VkPipeline)
dev_data->unique_id_mapping[pipeline_id];
dev_data->unique_id_mapping.
erase(pipeline_id);
lock.
...
VKAPI_ATTR VkResult
VKAPI_CALL CreatePipelineLayout(
        VkDevice
device,
...
for (
uint32_t index1 = 0;
index1<local_pCreateInfo->
setLayoutCount;
++index1) {
local_pCreateInfo->pSetLayouts[index1] = (VkDescriptorSetLayout)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(local_pCreateInfo
->pSetLayouts[index1])];
}
}
}
...
VkResult result = dev_data->device_dispatch_table
        ->CreatePipelineLayout(device, (const VkPipelineLayoutCreateInfo*) local_pCreateInfo, pAllocator,
                               pPipelineLayout);
if (local_pCreateInfo)
delete
local_pCreateInfo;
...
dev_data->unique_id_mapping[pipelineLayout_id];
dev_data->unique_id_mapping.
erase(pipelineLayout_id);
lock.
...
sampler = (VkSampler)
dev_data->unique_id_mapping[sampler_id];
dev_data->unique_id_mapping.
erase(sampler_id);
lock.
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateDescriptorSetLayout(
        VkDevice
device,
...
for (
uint32_t index2 = 0;
index2<local_pCreateInfo->pBindings[index1].
descriptorCount;
++index2) {
local_pCreateInfo->pBindings[index1].pImmutableSamplers[index2] = (VkSampler)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(local_pCreateInfo
->pBindings[index1].pImmutableSamplers[index2])];
}
}
}
}
...
VkResult result = dev_data->device_dispatch_table
        ->CreateDescriptorSetLayout(device, (const VkDescriptorSetLayoutCreateInfo*) local_pCreateInfo, pAllocator,
                                    pSetLayout);
if (local_pCreateInfo)
delete
local_pCreateInfo;
...
dev_data->unique_id_mapping[descriptorSetLayout_id];
dev_data->unique_id_mapping.
erase(descriptorSetLayout_id);
lock.
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateDescriptorPool(
        VkDevice
device,
...
dev_data->unique_id_mapping[descriptorPool_id];
dev_data->unique_id_mapping.
erase(descriptorPool_id);
lock.
...
VKAPI_ATTR VkResult
VKAPI_CALL ResetDescriptorPool(
        VkDevice
device,
VkDescriptorPool descriptorPool,
...
std::lock_guard <std::mutex> lock(global_lock);
descriptorPool = (VkDescriptorPool)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(descriptorPool)];
}
...
VKAPI_ATTR VkResult
VKAPI_CALL AllocateDescriptorSets(
        VkDevice
device,
...
descriptorPool = (VkDescriptorPool)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pAllocateInfo
->descriptorPool)];
}
if (local_pAllocateInfo->pSetLayouts) {
for (
uint32_t index1 = 0;
index1<local_pAllocateInfo->
descriptorSetCount;
++index1) {
local_pAllocateInfo->pSetLayouts[index1] = (VkDescriptorSetLayout)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(local_pAllocateInfo
->pSetLayouts[index1])];
}
}
}
...
VkResult result = dev_data->device_dispatch_table
        ->AllocateDescriptorSets(device, (const VkDescriptorSetAllocateInfo*) local_pAllocateInfo, pDescriptorSets);
...
for (
uint32_t index0 = 0;
index0<descriptorSetCount;
++index0) {
local_pDescriptorSets[index0] = (VkDescriptorSet)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pDescriptorSets[index0])
];
}
}
}
VkResult result = dev_data->device_dispatch_table->FreeDescriptorSets(device, descriptorPool, descriptorSetCount,(
...
for (
uint32_t index0 = 0;
index0<descriptorWriteCount;
++index0) {
local_pDescriptorWrites[index0].
initialize(& pDescriptorWrites[index0]);
if (pDescriptorWrites[index0].dstSet) {
local_pDescriptorWrites[index0].
dstSet = (VkDescriptorSet)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pDescriptorWrites[index0]
.dstSet)];
}
if (local_pDescriptorWrites[index0].pImageInfo) {
for (
uint32_t index1 = 0;
index1<local_pDescriptorWrites[index0].
descriptorCount;
++index1) {
if (pDescriptorWrites[index0].pImageInfo[index1].sampler) {
local_pDescriptorWrites[index0].pImageInfo[index1].
sampler = (VkSampler)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pDescriptorWrites[index0]
.pImageInfo[index1].sampler)];
}
...
local_pDescriptorWrites[index0].pBufferInfo[index1].
buffer = (VkBuffer)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pDescriptorWrites[index0]
.pBufferInfo[index1].buffer)];
}
}
}
...
descriptorCount;
++index1) {
local_pDescriptorWrites[index0].pTexelBufferView[index1] = (VkBufferView)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(local_pDescriptorWrites[index0]
.pTexelBufferView[index1])];
}
}
}
}
...
for (
uint32_t index0 = 0;
index0<descriptorCopyCount;
++index0) {
local_pDescriptorCopies[index0].
initialize(& pDescriptorCopies[index0]);
if (pDescriptorCopies[index0].srcSet) {
local_pDescriptorCopies[index0].
srcSet = (VkDescriptorSet)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pDescriptorCopies[index0]
.srcSet)];
}
...
dev_data->device_dispatch_table->
UpdateDescriptorSets(device, descriptorWriteCount, (
const VkWriteDescriptorSet*)local_pDescriptorWrites, descriptorCopyCount, (const VkCopyDescriptorSet*)local_pDescriptorCopies);
if (local_pDescriptorWrites)
delete[]
local_pDescriptorWrites;
if (local_pDescriptorCopies)
delete[]
local_pDescriptorCopies;
}

VKAPI_ATTR VkResult
VKAPI_CALL CreateFramebuffer(
        VkDevice
device,
...
local_pCreateInfo->
renderPass = (VkRenderPass) dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(pCreateInfo->renderPass)];
}
...
attachmentCount;
++index1) {
local_pCreateInfo->pAttachments[index1] = (VkImageView)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(local_pCreateInfo
->pAttachments[index1])];
}
}
...
VkResult result = dev_data->device_dispatch_table
        ->CreateFramebuffer(device, (const VkFramebufferCreateInfo*) local_pCreateInfo, pAllocator, pFramebuffer);
if (local_pCreateInfo)
delete
local_pCreateInfo;
if (VK_SUCCESS == result) {
...
dev_data->unique_id_mapping[framebuffer_id];
dev_data->unique_id_mapping.
erase(framebuffer_id);
lock.
...
dev_data->unique_id_mapping[renderPass_id];
dev_data->unique_id_mapping.
erase(renderPass_id);
lock.
...
std::lock_guard <std::mutex> lock(global_lock);
renderPass = (VkRenderPass)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(renderPass)];
}
dev_data->device_dispatch_table->
...
dev_data->unique_id_mapping[commandPool_id];
dev_data->unique_id_mapping.
erase(commandPool_id);
lock.
...
std::lock_guard <std::mutex> lock(global_lock);
commandPool = (VkCommandPool)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(commandPool)];
}
...
VKAPI_ATTR VkResult
VKAPI_CALL AllocateCommandBuffers(
        VkDevice
device,
...
VkResult result = dev_data->device_dispatch_table
        ->AllocateCommandBuffers(device, (const VkCommandBufferAllocateInfo*) local_pAllocateInfo, pCommandBuffers);
if (local_pAllocateInfo)
delete
local_pAllocateInfo;
return
...
local_pBeginInfo->pInheritanceInfo->
renderPass = (VkRenderPass)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pBeginInfo
->pInheritanceInfo->renderPass)];
}
...
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pBeginInfo
->pInheritanceInfo->framebuffer)];
}
}
}
}
VkResult result = dev_data->device_dispatch_table
        ->BeginCommandBuffer(commandBuffer, (const VkCommandBufferBeginInfo*) local_pBeginInfo);
if (local_pBeginInfo)
delete
local_pBeginInfo;
return
...
std::lock_guard <std::mutex> lock(global_lock);
pipeline = (VkPipeline)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(pipeline)];
}
dev_data->device_dispatch_table->
...
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(layout)];
if (pDescriptorSets) {
local_pDescriptorSets = new VkDescriptorSet[descriptorSetCount];
for (
uint32_t index0 = 0;
...
dev_data->device_dispatch_table->
CmdBindDescriptorSets(commandBuffer, pipelineBindPoint, layout, firstSet, descriptorSetCount, (
...
std::lock_guard <std::mutex> lock(global_lock);
buffer = (VkBuffer)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(buffer)];
}
dev_data->device_dispatch_table->
...
index0<bindingCount;
++index0) {
local_pBuffers[index0] = (VkBuffer)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pBuffers[index0])
];
}
}
}
...
std::lock_guard <std::mutex> lock(global_lock);
srcBuffer = (VkBuffer)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(srcBuffer)];
dstBuffer = (VkBuffer)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(dstBuffer)];
}
dev_data->device_dispatch_table->
...
std::lock_guard <std::mutex> lock(global_lock);
srcImage = (VkImage)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(srcImage)];
dstImage = (VkImage)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(dstImage)];
}
dev_data->device_dispatch_table->
...
index0<eventCount;
++index0) {
local_pEvents[index0] = (VkEvent)dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pEvents[index0])
];
}
}
...
for (
uint32_t index0 = 0;
index0<bufferMemoryBarrierCount;
++index0) {
local_pBufferMemoryBarriers[index0].
initialize(& pBufferMemoryBarriers[index0]);
if (pBufferMemoryBarriers[index0].buffer) {
local_pBufferMemoryBarriers[index0].
buffer = (VkBuffer)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pBufferMemoryBarriers[index0]
.buffer)];
}
}
}
...
for (
uint32_t index0 = 0;
index0<imageMemoryBarrierCount;
++index0) {
local_pImageMemoryBarriers[index0].
initialize(& pImageMemoryBarriers[index0]);
if (pImageMemoryBarriers[index0].image) {
local_pImageMemoryBarriers[index0].
image = (VkImage)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pImageMemoryBarriers[index0]
.image)];
}
}
}
...
dev_data->device_dispatch_table->
CmdWaitEvents(commandBuffer, eventCount, (
const VkEvent*)local_pEvents, srcStageMask, dstStageMask, memoryBarrierCount, pMemoryBarriers, bufferMemoryBarrierCount, (const VkBufferMemoryBarrier*)local_pBufferMemoryBarriers, imageMemoryBarrierCount, (const VkImageMemoryBarrier*)local_pImageMemoryBarriers);
if (local_pEvents)
delete[]
local_pEvents;
if (local_pBufferMemoryBarriers)
delete[]
local_pBufferMemoryBarriers;
...
dev_data->device_dispatch_table->
CmdPipelineBarrier(commandBuffer, srcStageMask, dstStageMask, dependencyFlags, memoryBarrierCount, pMemoryBarriers, bufferMemoryBarrierCount, (
const VkBufferMemoryBarrier*)local_pBufferMemoryBarriers, imageMemoryBarrierCount, (const VkImageMemoryBarrier*)local_pImageMemoryBarriers);
if (local_pBufferMemoryBarriers)
delete[]
local_pBufferMemoryBarriers;
if (local_pImageMemoryBarriers)
...
renderPass = (VkRenderPass)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pRenderPassBegin
->renderPass)];
}
...
const VkRenderPassBeginInfo*)local_pRenderPassBegin, contents);
...
dev_data->instance_dispatch_table->
...
VKAPI_CALL GetPhysicalDeviceSurfaceSupportKHR(
        VkPhysicalDevice physicalDevice,
        ...
        VkBool32* pSupported)
{
    ...
    VkResult result = dev_data->instance_dispatch_table
...
VKAPI_CALL GetPhysicalDeviceSurfaceCapabilitiesKHR(
        VkPhysicalDevice physicalDevice,
        VkSurfaceKHR surface,
...
VKAPI_CALL GetPhysicalDeviceSurfaceFormatsKHR(
        VkPhysicalDevice physicalDevice,
        VkSurfaceKHR surface,
...
VKAPI_CALL GetPhysicalDeviceSurfacePresentModesKHR(
        VkPhysicalDevice physicalDevice,
        VkSurfaceKHR surface,
...
dev_data->unique_id_mapping[swapchain_id];
dev_data->unique_id_mapping.
erase(swapchain_id);
lock.
...
// Declare only
VKAPI_ATTR VkResult
VKAPI_CALL GetSwapchainImagesKHR(
        VkDevice
device,
VkSwapchainKHR swapchain,
        uint32_t
...
VKAPI_ATTR VkResult
VKAPI_CALL AcquireNextImageKHR(
        VkDevice
device,
VkSwapchainKHR swapchain,
        uint64_t
...
std::lock_guard <std::mutex> lock(global_lock);
swapchain = (VkSwapchainKHR)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(swapchain)];
semaphore = (VkSemaphore)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(semaphore)];
fence = (VkFence)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(fence)];
}
VkResult result = dev_data->device_dispatch_table
        ...
        const VkPresentInfoKHR* pPresentInfo)
{
    ...
    safe_VkPresentInfoKHR* local_pPresentInfo = NULL;
    {
        ...
        if(pPresentInfo)
        {
            ...
            if(local_pPresentInfo->pWaitSemaphores)
            {
                ...
                {
                    local_pPresentInfo->pWaitSemaphores[index1] = (VkSemaphore)
                    dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(local_pPresentInfo
                            ->pWaitSemaphores[index1])];
                }
            }
            if(local_pPresentInfo->pSwapchains)
            {
                ...
                {
                    local_pPresentInfo->pSwapchains[index1] = (VkSwapchainKHR)
                    dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(local_pPresentInfo
                            ->pSwapchains[index1])];
                }
            }
    ...
    VkResult result = dev_data->device_dispatch_table
            ->QueuePresentKHR(queue, (const VkPresentInfoKHR*) local_pPresentInfo);
...
VKAPI_CALL CreateDisplayModeKHR(
        VkPhysicalDevice physicalDevice,
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateDisplayPlaneSurfaceKHR(
        VkInstance
instance,
...
local_pCreateInfo->
displayMode = (VkDisplayModeKHR) dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(pCreateInfo
        ->displayMode)];
}
}
}
VkResult result = dev_data->instance_dispatch_table
        ->CreateDisplayPlaneSurfaceKHR(instance, (const VkDisplaySurfaceCreateInfoKHR*) local_pCreateInfo, pAllocator,
                                       pSurface);
if (local_pCreateInfo)
delete
local_pCreateInfo;
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateSharedSwapchainsKHR(
        VkDevice
device,
...
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(countBuffer)];
}
dev_data->device_dispatch_table->
...
objectTable = (VkObjectTableNVX) dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(pProcessCommandsInfo
        ->objectTable)];
}
if (pProcessCommandsInfo->indirectCommandsLayout) {
local_pProcessCommandsInfo->
indirectCommandsLayout = (VkIndirectCommandsLayoutNVX) dev_data
        ->unique_id_mapping[reinterpret_cast<const uint64_t&>(pProcessCommandsInfo->indirectCommandsLayout)];
}
if (local_pProcessCommandsInfo->pIndirectCommandsTokens) {
for (
uint32_t index1 = 0;
index1<local_pProcessCommandsInfo->
indirectCommandsTokenCount;
++index1) {
if (pProcessCommandsInfo->pIndirectCommandsTokens[index1].buffer) {
local_pProcessCommandsInfo->pIndirectCommandsTokens[index1].
buffer = (VkBuffer)
dev_data->unique_id_mapping[reinterpret_cast
<const uint64_t&>(pProcessCommandsInfo
->pIndirectCommandsTokens[index1].buffer)];
}
}
}
...
dev_data->device_dispatch_table->
CmdProcessCommandsNVX(commandBuffer, (
const VkCmdProcessCommandsInfoNVX*)local_pProcessCommandsInfo);
...
objectTable = (VkObjectTableNVX) dev_data->unique_id_mapping[reinterpret_cast<const uint64_t&>(pReserveSpaceInfo
        ->objectTable)];
}
if (pReserveSpaceInfo->indirectCommandsLayout) {
local_pReserveSpaceInfo->
indirectCommandsLayout = (VkIndirectCommandsLayoutNVX) dev_data
        ->unique_id_mapping[reinterpret_cast<const uint64_t&>(pReserveSpaceInfo->indirectCommandsLayout)];
}
}
}
dev_data->device_dispatch_table->
CmdReserveSpaceForCommandsNVX(commandBuffer, (
const VkCmdReserveSpaceForCommandsInfoNVX*)local_pReserveSpaceInfo);
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateIndirectCommandsLayoutNVX(
        VkDevice
device,
...
dev_data->unique_id_mapping[indirectCommandsLayout_id];
dev_data->unique_id_mapping.
erase(indirectCommandsLayout_id);
lock.
...
VKAPI_ATTR VkResult
VKAPI_CALL CreateObjectTableNVX(
        VkDevice
device,
...
dev_data->unique_id_mapping[objectTable_id];
dev_data->unique_id_mapping.
erase(objectTable_id);
lock.
...
std::lock_guard <std::mutex> lock(global_lock);
objectTable = (VkObjectTableNVX)
dev_data->unique_id_mapping[reinterpret_cast<uint64_t &>(objectTable)];
}
VkResult result = dev_data->device_dispatch_table
...
VKAPI_ATTR VkResult
VKAPI_CALL UnregisterObjectsNVX(
        VkDevice
device,
VkObjectTableNVX objectTable,
        uint32_t
...
VKAPI_CALL GetPhysicalDeviceSurfaceCapabilities2EXT(
        VkPhysicalDevice physicalDevice,
        VkSurfaceKHR surface,
        VkSurfaceCapabilities2EXT* pSurfaceCapabilities)
{
    layer_data* dev_data = get_my_data_ptr(get_dispatch_key(physicalDevice), layer_data_map);
    {
        ...
        dev_data->unique_id_mapping[reinterpret_cast<uint64_t&>(surface)];
    }
    VkResult result = dev_data->instance_dispatch_table
...
VKAPI_ATTR VkResult
VKAPI_CALL DisplayPowerControlEXT(
        VkDevice
device,
VkDisplayKHR display,
...
VKAPI_ATTR VkResult
VKAPI_CALL RegisterDeviceEventEXT(
        VkDevice
device,
...
VKAPI_ATTR VkResult
VKAPI_CALL RegisterDisplayEventEXT(
        VkDevice
device,
VkDisplayKHR display,
...
VKAPI_ATTR VkResult
VKAPI_CALL GetSwapchainCounterEXT(
        VkDevice
device,
VkSwapchainKHR swapchain,
...
struct { const char* name; PFN_vkVoidFunction pFunc; } procmap[] = {
        ...
        {"vkGetPhysicalDeviceSurfaceCapabilities2EXT",
         reinterpret_cast<PFN_vkVoidFunction>(GetPhysicalDeviceSurfaceCapabilities2EXT)},
